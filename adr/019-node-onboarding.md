...
3. **`declaredMbps` is cleared.** Deregistration resets the declared capacity tier to 0 so that leaving the active set also releases the `bond_required(declaredMbps)` floor on the bond ([ADR 026 § Capacity-bond curve](026-tokenomics.md#capacity-bond-curve)). The bond itself is untouched, so re-registration needs no new funds — `registerNode` is gated only by `minBond` while the tier is 0 — but Step 2.2's `declareMbps` must be repeated to serve at a tier again.
