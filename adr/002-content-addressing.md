...
- The hash serves as the slash evidence primitive: a client submitting a slash claim provides the expected hash and the received bytes; the mismatch is verifiable on-chain. **Resolved:** [ADR 014](014-on-chain-verification.md) specifies a single-round optimistic challenge-response for the PoC (challenger posts a signed `StreamResponse` + 100 TOKEN bond; node has 24 hours to counter) with a production upgrade path using interactive keccak256 Merkle proofs over 1024-byte chunks. See [ADR 004](004-tokenomics.md#challenge-bond) for challenge bond requirements and [ADR 005](005-protocol.md#cdnprobev1--latency-probe) for slashing evidence mechanisms.
