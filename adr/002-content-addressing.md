...
A reserved namespace ID (`namespaceId == 0`) is the **default-open namespace**. Any blob is implicitly claimed under it; no `claimContent` call is required. Operators authorized by the DAO via the default-open allow-list (see [ADR 011 § Default-open allow-list](011-content-takedown.md#default-open-allow-list)) may serve as origin for default-open blobs; cache-only serving by any staked operator remains permissionless. The allow-list is the single global authority over default-open origin behaviour and is seeded with an initial operator set at `OriginAssignment` deployment, so default-open origin authorization is strict set-membership from genesis (no permissive bootstrap phase). Publishers seeking per-content durability guarantees, takedown accountability, or namespace-scoped origin sets opt in by registering and claiming under a non-zero namespace.
