...
- If an origin-backed node loses its backing data (disk failure, misconfiguration), the content is gone from the network unless another node holds the same blob. Operators are responsible for their own backend durability. Additionally, the hash-to-object-key catalog (the mapping from BLAKE3 hashes to origin storage keys) is a critical operational dependency: if the catalog is lost, the node cannot map content hashes to origin storage keys, effectively losing access to all content even if the backing store is intact. Operators SHOULD treat the catalog database with the same durability guarantees as the origin storage itself (regular backups, replication).
