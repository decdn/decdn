...
The QUIC handshake mutually authenticates the client's iroh NodeId and encrypts the channel (TLS 1.3). The session token on the epoch key stream binds the iroh identity to the provider's subscriber account. The technology stack, deployment model, and auth mechanism are provider choices — the CDN protocol only requires that the app server accepts `cdn/keys/v1` connections and that the client possesses the correct epoch key and envelope before issuing a `StreamRequest` to a CDN node.
