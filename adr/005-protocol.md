...
The requester probes all known peers in parallel (or checks the probe cache for recent results), waits up to 500ms, then selects the winner using a composite score: `rate_per_mb × rtt_ms` (lower is better). The 500ms window accommodates inter-continental RTTs. This applies to clients picking nodes and nodes picking peers for a cache miss pull.
