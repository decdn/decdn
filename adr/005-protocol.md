...
| `cdn/client/v1` | payer ↔ delivering node | Paid blob delivery with payment vouchers (client→node, node→node on cache miss). [ADR 030](030-blake3-merkle-verification.md) defines `cdn/client/v2` as a Tier 3 ALPN bump that extends the signed `StreamResponse` field set with `merkle_root` for cryptographic corruption proofs; v1 and v2 coexist via QUIC ALPN negotiation per [ADR 013](013-schema-evolution.md#alpn-version-negotiation). |
