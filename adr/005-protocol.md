...
**Preimage wire format:** `Preimage` carries exactly 32 bytes and no signature. It pays for one `chunk_bytes` of delivery: releasing the preimage at index `k` claims `k` chunks of the epoch. The receiver verifies it by applying `keccak256` to it `k − held` times and comparing against the preimage it already holds, where `held` is the lane's current index — so verification costs exactly as many hashes as the index advances, which is **one** in steady state. The first preimage of an epoch compares against `chain_root` instead, at the same cost. It is the smallest message in the protocol and the only one on the payment path in steady state. There is no acknowledgement — a preimage proves its own index, so the receiver has nothing to confirm and the sender has nothing to reconcile. Preimages MUST arrive in ascending index order on a lane; the QUIC stream provides that ordering, and a node rejects a non-advancing index rather than reordering.
...
enum StreamError {
    ...
    VoucherRejected { reason: VoucherRejectReason, position: Option<LanePosition> }, // Mid-stream payment rejection (carried in a StreamError message, not in the initial StreamResponse) — see VoucherRejected semantics below; `position` is present only on StaleEpoch
}
...
**Lane position on `StaleEpoch`, and why it needs no signature gate.** A signer that still holds its state places itself from the preimages it released and needs nothing from the node. A signer that has *lost* its state — a crash, a fresh process, a device that was offline for a week — cannot, and would otherwise be unable to open a correctly-based epoch on a lane it no longer remembers. So `StaleEpoch` carries the node's position:

```rust
// Attached to a StaleEpoch rejection so a signer that lost its lane state can
// resume. Self-authenticating: see below.
struct LanePosition {
    voucher: Voucher,       // the epoch anchor the node is on — the signer's own signed object
    chain_index: u64,       // highest index the node holds on that epoch
    preimage: [u8; 32],     // the preimage AT that index — the proof
}
```

The echoed `voucher` carries every input the seed derivation needs ([ADR 003 § Client Restart and Lane Resume](003-payments.md#client-restart-and-lane-resume)) — it is the signer's own signed epoch anchor, which is why the node has to echo it rather than name the epoch by a counter. The signer re-derives that epoch's chain from it, hashes `preimage` `chain_index` times, and requires the result to equal the voucher's `chain_root`. That single check is the whole authentication: a node cannot overstate its position, because claiming index `k` means exhibiting preimage `k`, which it can only hold if the signer released it and cannot derive from any lower one. Understating is possible and self-harming — it concedes value the node already earned.

This is what replaces the authenticated watermark bundle a per-interval voucher scheme required. That bundle needed *two* signature gates — the node attaching it only for a correctly-signed voucher, the signer trusting it only under its own prior signature — because a bare cumulative number carries no proof of itself and a hostile upstream could otherwise inflate a signer toward its cap. A preimage is its own proof, so the gates disappear and the payload shrinks to one hash check.
...
| `StaleEpoch` | The new voucher's base is behind what the lane has already earned. A signer holding its own state recomputes the base from its released preimages and reissues. One that has lost its state re-derives the chain from the `LanePosition`'s echoed voucher (above), verifies the attached preimage against it, then resumes at `chain_index + 1` or opens the next epoch at that frontier. **Bounded retries per stream.** |
