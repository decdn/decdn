...
enum StreamError {
    ...
    BlobTooLarge,      // Blob does not fit this node's disk budget; do not retry this node
...
**`BlobTooLarge` enforcement:** A node returns `StreamResponse {ok: false, error: BlobTooLarge}` when a blob does not fit its **disk budget** (`cache.cache_size_mb`). There is no separate blob-size limit. A blob larger than the whole cache can never be admitted, whatever the node evicts, so the refusal is a durable fact about this (node, blob) pair.

The check applies at **admission**, not at delivery. A node that already holds a blob always serves it: the disk is already spent, so refusing to deliver would waste the storage and forgo the payment for it. The check therefore binds on the origin pull-through path, which is the one ingest path with no signed size ahead of the bytes — an origin backend can misstate or omit its length, so the node caps the stream as it reads and aborts on breach.

Paid ingest needs no such cap. A provider signs `total_bytes` before delivery and the receiving node streams that body to disk at `O(chunk-group)` memory, so a large blob costs disk, which the eviction driver already bounds against the same budget.

**Probe interaction:** A node MUST NOT respond `has_blob: true` in a `ProbeResponse` for a blob it will not serve. Advertising `has_blob: true` and then refusing means advertising a blob it cannot serve — a local-reputation failure ([ADR 008](008-reputation.md#adr-008-reputation-system)). Since a resident blob is always servable, a node that holds the blob can always answer `has_blob: true`. For cache-miss pulls the question does not arise: the node already answered `has_blob: false`, so it advertised nothing.

**Retry behavior:**

1. On `ok: false` or connection failure, the requester does **not** retry the same node for the same blob hash. For `BlobTooLarge`, the requester MUST NOT retry the same node — the disk budget is a stable node policy, not a transient condition like `Overloaded`. Since the budget is per-node, a node with more disk may accept the blob.
