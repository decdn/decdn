...
The same two-phase pattern carries `CooperativeCloseAuthExt { last_signature }` after a `CooperativeCloseAuth`: the client's own last-accepted voucher signature, echoed so a client whose persisted watermark lags can prove the node's declared tuple is one it already signed ([ADR 003 § Cooperative close](003-payments.md#adr-003-payment-model)). It is additive and non-wire-breaking in both directions — a node that predates it emits no trailing bytes and the receiver fills `CooperativeCloseAuthExt::default()`; a client that predates it decodes the base auth and ignores the trailing bytes. Neither side is forced to upgrade, and a client that cannot read the echo simply keeps the stricter refusal it had before.
