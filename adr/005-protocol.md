...
**Signer binding:** Connection-level peer identity is authenticated by the iroh QUIC handshake against the registered NodeId (Ed25519). Message-body attribution and on-chain slash evidence both rely on `slash_sig`: verifiers recover the Ethereum address via `ecrecover` (3,000 gas on-chain) and confirm it maps to the expected NodeId via `CapacityBond.nodeIdOf` ([ADR 003 § NodeId-to-Ethereum Binding](003-payments.md#nodeid-to-ethereum-binding)). `slash_sig` is mandatory and non-empty on every `ProbeResponse` and `StreamResponse` — no opt-out. Requesters MUST reject responses with missing or zero-length `slash_sig`, and MUST recover the signature to the peer's registered operator address before acting on the response. A length check alone leaves the quote unattributed: selection reads `has_blob` and `rate_per_mb` off the response, and neither binds the peer to anything until the signature recovers. A response that fails to verify is dropped as requester-local policy — it MUST NOT be scored against the peer, because a signature that does not recover attributes nothing to anyone. See [ADR 014](014-on-chain-verification.md#adr-014-on-chain-verification-for-slashing-evidence) for the EIP-712 type definitions and `SlashJudge` contract interface.
...
A node advertises its configured `rate_per_mb` verbatim in a `ProbeResponse` and a `StreamResponse`. The node never rewrites its own quote to a floor: the delivery floor is not a quote gate. A rate below the floor still sells and settles; `PaymentPool.redeem` clamps only the vote-weight byte credit for sub-floor bytes (see [ADR 003 § Rate-floor enforcement](003-payments.md#rate-floor-enforcement)). The floor is redemption-time contract state, so a node reads it for no wire decision. `MAX_RATE_PER_MB` = 1000 base units per MB (~$1/GB, ~100× the expected market rate) bounds `rate_per_mb` from above at the wire layer — the largest `rate_per_mb` the wire decodes — and the contract caps `deliveryFloor` at the same constant, so the floor can never exceed the wire price. The cap sits near real prices on purpose: a floor set close to a far-above-market ceiling would clamp credited bytes toward zero and suppress ADR-036 vote-weight accrual, so a realistic ceiling bounds that suppression while still catching below-market bytes.

##### Requester-side validation (optional)

Requesters (clients and nodes performing cache-miss pulls) MAY reject `ProbeResponse` or `StreamResponse` messages whose `rate_per_mb` they consider too expensive. This is a local policy decision, not a protocol requirement — the buyer sees the signed rate before it pays anything, which is what makes a governance ceiling unnecessary. This is the only check either side performs on the quoted price itself. Separately, the node checks that each voucher pays the quoted rate for the span it adds, and refuses one that does not as `Underpaid` (see [`VoucherRejected` semantics](#voucherrejected-semantics)).

A node with a cache miss issues a `cdn/dht/v1` FIND_VALUE lookup for the target hash, then probes the returned candidate set concurrently via `cdn/probe/v1` (or checks the probe cache for recent results), collecting responses until a 500ms timeout, then selects the winner using the unified selection score (see [ADR 001, Node Selection Algorithm](001-network.md#node-selection-algorithm)). See [ADR 001, Content Discovery](001-network.md#content-discovery-dht--probe) and [ADR 022](022-content-discovery.md#adr-022--content-discovery-at-scale) for the full discovery flow. The 500ms ceiling accommodates inter-continental RTTs. This is the node-to-node path. A client picks its node by measured RTT from its peer store or the registry ([ADR 037 § Client selection policy](037-regional-proxy-warming.md#client-selection-policy-latency-driven-proxy-preference)).
...
1. **Each stream anchors itself.** A bare preimage does not name its chain, so each stream carries its own `(anchor voucher, chain_root, verified, tip)`, seeded from the `chain_root` voucher that anchors it (`verified = 0`, `tip = chain_root`). The epoch's root voucher MUST precede that stream's preimages for that epoch. The payer sends it immediately before the stream's first reveal, not at stream open. A stream that makes no reveal sends no anchor voucher: its closing voucher is its only voucher. After a rollover, the payer sends the new-root voucher on **each** stream immediately before that stream's next reveal. The payer does not send it on one stream and rely on the node to propagate it. QUIC orders messages within a stream. So each stream reads its old-chain preimages against its own old root before its own roll voucher arrives. A fast stream that has adopted `R₂` cannot strand a sibling still finishing `R₁`. A preimage on a stream with no anchor is rejected `UnanchoredPreimage`. Re-sending a voucher the node already holds is free — an at-or-below-watermark voucher is treated as already-satisfied, not rejected. It pays no whole chunk ([ADR 003 § Concurrent Streams](003-payments.md#concurrent-streams)).
...
enum StreamError {
    ...
    VoucherRejected { reason: VoucherRejectReason, bundle: Option<WatermarkBundle> }, // Mid-stream payment-voucher rejection (carried in a StreamError message, not in the initial StreamResponse) — see VoucherRejected semantics below; `bundle` carries the node's true watermark on a watermark-gated reject
...
// Attached to a regression/exhaustion/Underpaid VoucherRejected so a delegated signer that
...
enum VoucherRejectReason {
    ...
    PoolExhausted,        // Pool's remaining deposit, minus the refundable floor M and already-committed concurrent floor credit across every signer, can no longer fund further credit for this stream. Pool level only — the per-signer floor sub-cap refuses at admission as NotFound and never reaches this enum. Handler-emitted mid-stream after capability ownership is proven; no validation-enum counterpart
    ...
    ChainIndexZero,       // index == 0 — index 0 is the redeem-time settlement case and never travels the wire. The u8 index cannot exceed MAX_CHAIN_LENGTH, so there is no over-large case to report. Handler-emitted; no validation-enum counterpart
    ...
    Underpaid,            // The span the voucher adds over the lane's accepted watermark pays below the quoted rate_per_mb. Carries the node's watermark. Handler-emitted; no validation-enum counterpart
}
```
...
The other `StreamError` variants (`NotFound`, `Overloaded`, `BlobTooLarge`, `InternalError`, `EvictedSinceProbe`, `InsufficientDeposit`) are delivery-side and ride in the initial `StreamResponse { ok: false, error: ... }` (transitioning `AwaitingResponse → Failed`). `VoucherRejected` is the only variant scoped to the mid-stream `StreamError` message; this asymmetry is intentional — each rejection's lifecycle position determines which carrier message is available. `InsufficientDeposit` is the open-time pool-solvency refusal spoken to a proven pool owner; a prober's floor refusal collapses to `NotFound` instead (see [ADR 003 § Pool solvency](003-payments.md#pool-solvency-and-the-refundable-floor-m)).

The first seven `VoucherRejectReason` variants mirror the off-chain validation enums `PoolError` / `VoucherError` (in `crates/incentive/`) one-to-one. The chain variants (`BadPreimage`, `ChainIndexZero`, `UnanchoredPreimage`, `ChunkPriceMismatch`) are handler-emitted and have no validation-enum counterpart. `BadSignature`, `WrongSigner`, `WrongPool`, `WrongProvider`, and `SpendingCapExhausted` avoid an on-chain `redeem` revert; the ordering guards (`AmountRegression`, `BytesRegression`) have no on-chain counterpart — on-chain redemption is cumulative, so a stale voucher simply pays `0` — and exist to keep the node's own off-chain ledger consistent (see [ADR 003 § Redemption and Close](003-payments.md#redemption-and-close) and [ADR 003 § Off-chain Voucher Rejections](003-payments.md#off-chain-voucher-rejections-wire-encoding) for the per-reason mapping). The remaining variants have no validation-enum counterpart and are emitted by the `cdn/client/v1` handler directly: `CapabilityExpired` fires when the signer's capability has passed its expiry — the node holds the clock, so this is handler-direct rather than bridged through `PoolError`; `PoolExhausted` fires mid-stream when the pool's remaining deposit can no longer fund further credit, after the client has already proved capability ownership; and `Underpaid` fires when the span a voucher adds over the lane's accepted watermark pays below the quoted `rate_per_mb`. There is no floor-related reject reason: the delivery floor is a soft floor that `PaymentPool.redeem` enforces by clamping credited bytes, so a voucher priced below the live floor still settles and the node keeps serving (see [ADR 003 § Rate-floor enforcement](003-payments.md#rate-floor-enforcement)). A transient node-side persist-write failure (`PoolError::Store`, surfaced internally as `RetrySignal`) carries no wire reject reason at all: the node aborts the stream cleanly instead, and the signer resends the same voucher on a fresh stream (see [ADR 003 § Off-chain voucher state persistence](003-payments.md#off-chain-voucher-state-persistence)).

**Watermark bundle (self-heal on regression, exhaustion, and underpayment).** A delegated signer — one issued a capped capability by a pool owner — cannot reconstruct its off-chain lane watermark from the contract: a lane records nothing until its first redemption. An `AmountRegression`/`BytesRegression`/`SpendingCapExhausted`/`Underpaid` rejection would otherwise be a dead end. On exactly those four reasons, the node attaches a `WatermarkBundle` carrying its true cumulative `amount`/`bytes_delivered` plus `last_signature` — the `r‖s‖v` of the signer's own last-accepted voucher. Two signature gates keep the bundle from leaking a watermark or corrupting signer state:

- **Node attaches it only when the rejected voucher's signature recovers to the pool's authorized `signer`.** The regression checks fire before the signature check, so the node recovers the signature explicitly at the reject site; a party who merely guessed the chain-derivable `poolId` and sent an unsigned or wrong-key voucher gets no bundle. This keeps a lane's watermark private to the key that signs it. A `ChunkPreimage` carries no signature of its own and gets no bundle: `BadPreimage` is a hash-chain mismatch, which a payment watermark cannot repair, so it stays terminal. The bundle remains on exactly the four self-heal reasons.
...
- **Signer trusts `verified_index` only when `tip` proves it:** `keccak^verified_index(tip) == chain_root`. The signature gate above covers the anchor half of the bundle and the `chain_root`, but no signature covers the index — the voucher it echoes has no index field — and the signer folds `verified_index × chunk_price` into the amount it re-signs. A node reporting 255 where the signer released 3 would otherwise collect 252 chunks it never delivered, authorized by the signer's own key. A preimage is unforgeable, so the deepest one a node can present is the deepest one it was given; hashing it forward to the root the signer's own signature covers turns the index from a claim into a proof. The walk is local, at most 255 keccaks, and needs no round trip. A bundle failing it is not resumable.
...
On `Underpaid`, the passing bundle is normally behind the signer's ledger. The signer holds vouchers that the node never accepted. The signer commits a voucher when its send succeeds and records it as paid, even when the node does not accept it. Every later span then looks short to the node. The signer rebases its ledger down to the bundle and resumes. A bundle ahead of the ledger reseeds instead, as for the other reasons.

The rebase waits for any voucher that is still being sent, so nothing signed from the old anchor commits over the new one. Each rebase starts a new ledger generation, and the signer stamps each voucher with the generation it was signed under. An `Underpaid` for a voucher from an earlier generation is stale: a sibling stream signed it before the rebase. The signer retries the stream and does not change the ledger. A rebase on a stale rejection would re-sign amounts the node has since accepted from the healed anchor, with different bytes, which is a `BytesRegression`. An `Underpaid` for a voucher from the current generation is a new divergence, and the signer rebases again.

The signer records the rebased watermark once with an overwrite, then advances from it, so its next run starts in step with the node. Every later record is a monotone advance. The rebase costs the signer nothing: redemption pays the highest cumulative the node holds, and every voucher the node holds is one the signer signed.
...
| `ChainIndexZero` | The index is `0`, which never travels the wire. A payer bug. Do not retry; surface to caller. (A chain that has run out of indexes is not this reason — the payer rolls to a fresh `chain_root` before it reaches the end.) |
...
| `Underpaid` | The voucher's span over the node's accepted watermark pays below the quoted rate. The signer's ledger is ahead of the node's. If the `WatermarkBundle` authenticates (above), rebase to it and retry. For a voucher signed before the latest rebase, retry without rebasing again. Without a bundle (the lane has no accepted voucher yet), the signed price is below the quote: do not retry; surface to caller. **Bounded retries per stream.** |
...
4. **Time to first byte:** the requester bounds the wait from `StreamRequest` to the first byte with a time budget. On a cache-miss pull the serving node fills from upstream before it emits the first byte, so this latency scales with blob size. The budget is requester-local policy. Its expiry says nothing about the peer and does not affect reputation, the same treatment as an unsigned timeout.
5. **Steady-state throughput:** the requester counts bytes read off the QUIC stream, partial frames included, so the measure is frame-size-independent. It reconnects to the next candidate with `byte_offset` set to the last BLAKE3-verified byte when the throughput over a trailing window falls below a minimum floor. The window and floor are requester-local configuration; the recommended defaults are a 20-second window and a 4 KiB/s floor. The abort is requester-local policy and does not affect reputation. A self-inflicted pause at the credit-window boundary, while the requester still owes the covering proof, does not count as a sender stall.
