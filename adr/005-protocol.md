...
Before signing a `ProbeResponse` containing `rate_per_mb`, the node MUST verify that `rate_per_mb` is at least its locally cached `deliveryFloor`. If the node's configured rate is below the current floor — e.g., governance raised it while the node was running — the node SHOULD raise `rate_per_mb` to the floor and log a warning rather than refusing to respond, keeping the node operational during governance transitions until the operator updates their rate configuration. The clamp only ever raises: there is no governance ceiling, and `MAX_RATE_PER_MB` = 1000 base units per MB (~$1/GB, ~100× the expected market rate) bounds the field above at the wire layer. This is the largest `rate_per_mb` the wire decodes, and it also caps the on-chain `deliveryFloor` (see [ADR 003 § Rate-floor enforcement](003-payments.md#rate-floor-enforcement)) so the floor can never exceed the wire price. The cap sits near real prices on purpose: a floor set close to a far-above-market ceiling would clamp credited bytes toward zero and suppress ADR-036 vote-weight accrual, so a realistic ceiling bounds that suppression while still catching below-market bytes.
