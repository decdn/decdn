...
1. **The deepest preimage wins.** The node ignores `index ≤ verified` without hashing, rejects `index > MAX_CHAIN_LENGTH` before hashing (`ChainIndexTooLarge`), and otherwise accepts when `keccak^(index − verified)(preimage)` reaches the deepest preimage it holds. A fast stream may therefore skip indexes a slower one has not reached.
2. **Each stream anchors its own epoch.** A bare preimage does not name its chain, and `index` is one byte, so the epoch's `chain_root` voucher MUST precede that stream's preimages for that epoch — sent once at stream open and once again on each rollover, never before every preimage. The voucher is cumulative and signed, so a resend is idempotent and dedup is free. A preimage that arrives on a stream with no anchoring voucher is rejected `UnanchoredPreimage`.

Without rule 2 a fast stream can carry a reveal for a chain whose announcing voucher has not landed anywhere yet, and the node has no way to verify it. Per-stream chains would avoid the problem but multiply rollovers, and therefore signatures, which is exactly the cost the chain exists to remove.

Implementation constraint: the payer must have a single voucher-signing and chain-advancing task per lane, aggregating byte counts from all its streams, not independent per-stream payment logic. It serializes the chain index and the rollover decision under a local lock while bytes stream concurrently.
...
enum VoucherRejectReason {
    ...
    ChainIndexTooLarge,   // index > MAX_CHAIN_LENGTH, or index == 0 (index 0 never travels the wire). Rejected before any hashing. Handler-emitted; no validation-enum counterpart
    UnanchoredPreimage,   // Preimage arrived on a stream that has not yet carried the epoch's chain_root voucher, so the node cannot name the chain it belongs to. Handler-emitted; no validation-enum counterpart
    ChainRootReuse,       // Voucher carries a chain_root already seen on another lane, or already retired on this one (ADR 003 § One chain per lane). Handler-emitted; no validation-enum counterpart
}
```
...
| `ChainIndexTooLarge` | The index is past `MAX_CHAIN_LENGTH`, or is `0`. A payer bug: the chain should have rolled. Do not retry on this chain; roll to a fresh `chain_root` with a new signed voucher, then resume from index 1. |
| `UnanchoredPreimage` | This stream has not carried the current epoch's `chain_root` voucher. Not a fatal error. **Send the current root voucher on this stream, then resend the preimage.** The voucher is cumulative, so a resend costs one signature-free duplicate at worst. Bounded retries per stream. |
| `ChainRootReuse` | The node has seen this `chain_root` on another lane or already retired it here. A payer bug with real cost — a reused root lets one node spend another's preimages ([ADR 003 § Cross-lane preimage spend](003-payments.md#cross-lane-preimage-spend)). Derive a fresh independent seed for this lane and re-sign. Do not retry with the same root. |
...
The byte counter is cumulative across all streams sharing a `pool_id`; each stream's delivered bytes contribute to the aggregate counter that triggers the next proof. The coordinator also holds the lane's `chain_root` and the deepest verified chain index, so a preimage arriving on any stream is placed against lane-wide state.
