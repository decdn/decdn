...
enum VoucherRejectReason {
    ...
    WrongToken,           // PoolError::WrongToken — cross-token replay defense (ADR 003)
    StaleNonce,           // PoolError::NonceNotIncreasing — voucher nonce not strictly increasing on this lane
    AmountRegression,     // PoolError::AmountDecreasing — cumulative amount regressed on this lane
...
| `StaleNonce` | Client-side bookkeeping desync (e.g., reconnect after crash). If the reject carries a `WatermarkBundle` that authenticates (above), re-seed to the node's lane watermark and reissue from the next cumulative — this is the self-heal path for a delegated signer that cannot refresh from chain. Otherwise refresh lane state from the node and reissue. **Bounded retries per stream.** |
