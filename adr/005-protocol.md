...
sequenceDiagram
    ...
    C->>R: ProbeResponse {has_blob, is_origin, rate_per_mb, timestamp_us, signature, total_bytes?, slash_sig}

    Note over R: RTT = receive_time - timestamp_us
    Note over R: Score = unified selection score (see ADR 001)
```

`timestamp_us` is a requester-generated microsecond timestamp echoed back. RTT is `receive_time - timestamp_us`. `has_blob` confirms the node has the content. `is_origin` signals the responder's role for this blob — `true` if the responder claims to be an authorized origin for the blob's namespace (see [ADR 002 § Publisher Identity and Namespaces](002-content-addressing.md#publisher-identity-and-namespaces) and [ADR 011 § Origin Assignment Authority](011-content-takedown.md#origin-assignment-authority)); `false` if the responder is serving from cache only. The two flags are independent: a node may have a cached copy (`has_blob: true, is_origin: false`) of content for which it is not the authorized origin, and that response is legitimate — cache-only serving remains permissionless regardless of namespace registration. `rate_per_mb` lets the requester score candidates on both latency and price in a single round-trip. `total_bytes` is an optional unsigned field indicating the blob's total size in bytes. When present, it lets the requester estimate total cost before committing to a node or opening a payment channel. Nodes SHOULD include it when the blob size is known. The field is not covered by the signature — it is not needed for any slashing mechanism and follows the Tier 1 minor evolution pattern from [ADR 013](013-schema-evolution.md#tier-1--minor-no-coordination).

**`is_origin` rules.** A node MUST NOT respond `is_origin: true` for a hash unless **both** of the following hold: (a) the operator is authorized as origin for the hash — i.e., the operator is in *at least one* of the hash's claiming namespaces' authorized sets (`∃ ns ∈ PublisherRegistry.namespaceOf(hash)` such that `OriginAssignment.isAuthorizedOrigin(ns, operator) == true`), or — when `PublisherRegistry.namespaceOf(hash)` is empty — the operator is in the DAO-maintained default-open allow-list (see [ADR 002 § Multi-claim semantics](002-content-addressing.md#multi-claim-semantics) and [ADR 011 § Default-open allow-list](011-content-takedown.md#default-open-allow-list)); see [ADR 016](016-contract-interactions.md). AND (b) the operator is not currently blacklisted (`ContentBlacklist.isOriginBlacklisted(operator) == false`; see [ADR 011](011-content-takedown.md#interaction-with-contentblacklist)). The rule is uniform across all namespaces — for hashes with one or more non-zero claims, authorization is per-namespace `OriginAssignment` membership in any one of them; for hashes with no non-zero claims, authorization is membership in the DAO-maintained default-open allow-list, subject to the bootstrap rule that returns `true` for any active staker until the allow-list has been activated for the first time. Before signing, the responder consults `PublisherRegistry.namespaceOf(hash)`, iterates `OriginAssignment.isAuthorizedOrigin` over the returned set (or falls through to the default-open allow-list when the set is empty), and finally consults `ContentBlacklist.isOriginBlacklisted`. All views are read-cached locally and refreshed on a configurable interval (`assignment_poll_interval`, default 5 minutes); on cache miss the node SHOULD respond `is_origin: false` rather than block on an RPC call. Signing `is_origin: true` without authorization is a **phantom-origin slashing offense**.

`signature` is the candidate node's iroh private key signature over `{hash, has_blob, is_origin, rate_per_mb, timestamp_us}`. `is_origin` is signature-covered because phantom-origin slashing requires cryptographic attribution to the responder. The signed-field set is the v1 baseline; per [ADR 013 § Signed Field Freezing](013-schema-evolution.md#signed-field-freezing), any subsequent change to this set is a Tier 3 ALPN bump. The signature makes the probe response cryptographically attributable and enables three slashing mechanisms: (1) **phantom announcement slashing** — if `has_blob: true` in `ProbeResponse` but the node returns a signed `StreamResponse` with `ok: false` or a redirect for the same hash, the two signed messages are on-chain-verifiable evidence of a phantom announcement (the timeout/non-response case is handled separately — see ADR 003); (2) **rate manipulation slashing** — if `stream_response.timestamp_us >= probe_response.timestamp_us` and `stream_response.timestamp_us - probe_response.timestamp_us < 30_000_000` (30 seconds) and `stream_response.rate_per_mb > probe_response.rate_per_mb`, both signed messages constitute on-chain-verifiable evidence of bait-and-switch; (3) **phantom-origin slashing** — `SlashJudge` evaluates a signed `ProbeResponse` with `is_origin: true` against on-chain state at the response timestamp using the historical views `PublisherRegistry.namespaceOfAt(hash, timestamp_us)` and `OriginAssignment.isAuthorizedOriginAt(namespaceId, operator, timestamp_us)` (defined in [ADR 002 § Contract: PublisherRegistry](002-content-addressing.md#contract-publisherregistry) and [ADR 011 § Historical state and slashing evidence](011-content-takedown.md#historical-state-and-slashing-evidence)). The disjunctive predicate is specified in [ADR 011 § Historical state and slashing evidence](011-content-takedown.md#historical-state-and-slashing-evidence): the response is authorized iff *some* `ns ∈ PublisherRegistry.namespaceOfAt(hash, t)` has `OriginAssignment.isAuthorizedOriginAt(ns, operator, t) == true`, or the set is empty and the operator was in the default-open allow-list at `t`. If that disjunction is false OR `ContentBlacklist` recorded the operator as blacklisted at `t`, the signed response is unauthorized and slashable. For hashes with no non-zero claims at `t`, the default-open branch follows the same checkpoint logic with the bootstrap rule that returns `true` for any timestamp before the default-open allow-list was first activated. Evidence age is bounded by the existing `Max evidence age` parameter ([ADR 009](009-governance.md)) so honest operators are never exposed to indefinite slash risk from old probes. Both `timestamp_us` values are requester-generated (the probe timestamp is echoed in `ProbeResponse`; `StreamResponse` echoes a separate requester timestamp from `StreamRequest`), so the on-chain verifier computes the delta from a single clock with no wall-clock reference needed. **Submitting slash evidence requires a 100 TOKEN challenge bond** — see [ADR 026 §8](026-gauge-boost-tokenomics.md#8-slashing-and-burn). The bond is returned if the challenge succeeds and forfeited if the node successfully counters, preventing zero-cost griefing via fabricated slash claims.
...
> **Relationship to iroh-blobs:** The `cdn/client/v1` protocol wraps iroh-blobs' verified streaming within its own message framing. iroh-blobs provides BLAKE3 tree-hash verification at the chunk level; `ChunkData` messages carry these verified chunks alongside the payment and delivery control messages (`Voucher`, `StreamEnd`, `StreamError`) that iroh-blobs' native transfer protocol does not support. The BLAKE3 content hash in `StreamRequest` is the iroh-blobs hash, and verification uses iroh-blobs' incremental tree-hash mechanism — receivers do not need to buffer the full blob before confirming integrity.
...
All protocol messages use [postcard](https://docs.rs/postcard) — compact, no-std friendly, serde-based. Standard in the iroh ecosystem; avoids introducing a second serialization dependency alongside what iroh already uses internally. Wire framing (varint length prefix), message discrimination (protocol enums per ALPN), and schema evolution rules are specified in [ADR 013](013-schema-evolution.md).
...
- `StreamRequest` includes a requester-generated `timestamp_us` that the node echoes in `StreamResponse`. Rate manipulation is an **immediate offense** ([ADR 014](014-on-chain-verification.md)): two signed messages from the same NodeId — `ProbeResponse{rate=R₁, timestamp_us=T}` and `StreamResponse{rate=R₂, timestamp_us=T+Δ}` — with `Δ < 30s` and `R₂ > R₁` are non-repudiable on-chain evidence. The node's last probe-quoted rate is binding for any stream opened within the 30-second window; legitimate rate increases require honoring old quotes for that window or pausing new connections during the propagation gap. Rate decreases (`R₂ < R₁`) are unconstrained — slashing is one-directional
