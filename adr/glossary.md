...
| **SlashAppeal** | The slash-appeal state machine contract ([ADR 028](028-slashing-appeals.md#adr-028-slashing-appeals-and-dispute-escalation)). A slashed operator (or any party) files an appeal posting a TOKEN bond; the emergency multisig fast-tracks/rejects and the Governor grants/upholds. Holds `SLASH_APPEAL_ROLE` on `CapacityBond` and drives its **escrow-on-slash** settle hooks (`markAppealOpen` / `settleAppealUpheld` / `settleAppealGranted`): slashed TOKEN is held in `CapacityBond` escrow until the appeal resolves, then distributed 50% challenger / 50% burn or refunded to the operator on a granted appeal (which also clears `slashedAtEpoch[op]` per [ADR 036 § Slashing zero-out](036-served-bytes-voting-weight.md#slashing-zero-out)). Replaces the retired SafetyReserve ([ADR 026 § Slashing and burn](026-tokenomics.md#slashing-and-burn)). |
