...
- New field on the operator record: `regionLastChanged: uint256` (Unix timestamp in seconds, matching the existing `firstRegisteredAt`, `registeredAt`, and `lastMultiaddrUpdate` fields in [ADR 001 § Data Structure](001-network.md#data-structure)). Set to `block.timestamp` on `registerNode` and on every successful `updateRegion`. The standing-eligibility check (§ below) reads it with a read-time fallback to `firstRegisteredAt`: `effective = regionLastChanged != 0 ? regionLastChanged : firstRegisteredAt`. This avoids a one-time storage migration — Solidity zero-initializes the field for pre-upgrade records, and the fallback gives the most conservative reading (region has been stable since registration).
- New operator-facing call: `updateRegion(string newRegion)`. Mirrors the operator-callable, no-bond shape of the existing `updateMultiaddrs` ([ADR 019 § NAT and Multiaddr Handling](019-node-onboarding.md#nat-and-multiaddr-handling); cooldown semantics in [ADR 001 § Multiaddr Update Policy](001-network.md#multiaddr-update-policy)) — caller must be the operator's registered Ethereum address. No analogous per-call cooldown is specified for `updateRegion`; the 7-day `REGION_STABILITY_WINDOW` (§ below) is the only timing gate. Emits `RegionUpdated(bytes32 indexed nodeId, string oldRegion, string newRegion)` — the `indexed` qualifier on `nodeId` matches every other `StakingRegistry` event in [ADR 001 § Interface (additions to StakingRegistry)](001-network.md#interface-additions-to-stakingregistry), and the block timestamp is implicit in the event log header so a separate `timestamp` parameter would be redundant.
