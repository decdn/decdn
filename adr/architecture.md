...
The economic model that ties the protocol together. [ADR 026](026-tokenomics.md#adr-026-tokenomics) is the canonical umbrella (supply, the `FeeRouter` six-bucket split, operator economics, slashing-burn, bootstrap, governable parameters); the per-bucket detail ADRs are [ADR 034](034-gauge-boost-voting-escrow.md#adr-034-gauge-boost-and-voting-escrow) (gauge-boost formula + `VotingEscrow`, incl. the per-operator gauge-share cap that defends against wash-trading), [ADR 035](035-delegator-pool.md#adr-035-delegator-pool) (delegator pool), and [ADR 033](033-safety-insurance-reserve.md#adr-033-safety-and-insurance-reserve) (`SafetyReserve`, 3% safety/insurance bucket); [ADR 018](018-liquidity-strategy.md#adr-018-liquidity-strategy-balancer-8020-pol) is the Balancer V3 POL. The launch contract surface is forward-compatible (additive integration via standard `AccessControl` role grants per [ADR 016 § Access Control Matrix](016-contract-interactions.md#5-access-control-matrix)) so future economic-layer products can land as additive top-level contracts without changing existing contracts.

1. [ADR 026 — Tokenomics](026-tokenomics.md#adr-026-tokenomics) (canonical umbrella)
2. [ADR 034 — Gauge Boost and Voting Escrow](034-gauge-boost-voting-escrow.md#adr-034-gauge-boost-and-voting-escrow)
3. [ADR 035 — Delegator Pool](035-delegator-pool.md#adr-035-delegator-pool)
4. [ADR 033 — Safety and Insurance Reserve](033-safety-insurance-reserve.md#adr-033-safety-and-insurance-reserve)
5. [ADR 018 — Liquidity Strategy (Balancer 80/20 POL)](018-liquidity-strategy.md#adr-018-liquidity-strategy-balancer-8020-pol)

### Chapter 5 — Verification & enforcement

How protocol violations are detected, adjudicated, and punished. The slashing schedule lives in [ADR 026 § Slashing and burn](026-tokenomics.md#8-slashing-and-burn); this chapter is the evidence and adjudication path.
...
- **[ADR 008 — Reputation System](008-reputation.md#adr-008-reputation-system)** — Interaction-weighted scoring with gossip propagation; complements the [ADR 034 § per-operator gauge-share cap](034-gauge-boost-voting-escrow.md#per-operator-gauge-share-cap) as off-chain wash-trading signal.
- **[ADR 009 — Governance Model](009-governance.md#adr-009-governance-model)** — Admin key for PoC; ve-weighted Governor + Timelock with safety bounds for production.
...
- **[ADR 014 — On-Chain Verification for Slashing Evidence](014-on-chain-verification.md#adr-014-on-chain-verification-for-slashing-evidence)** — secp256k1 EIP-712 `slash_sig` on `ProbeResponse`/`StreamResponse`; unified `SlashJudge` contract for the three signature-dependent offenses (phantom, rate, blacklist). Content corruption is absorbed at the wire (no on-chain path) — see [ADR 003 § Corrupted delivery](003-payments.md#corrupted-delivery). Wash-trading defense lives in [ADR 034 § Per-operator gauge-share cap](034-gauge-boost-voting-escrow.md#per-operator-gauge-share-cap) (per-operator gauge-share cap), not in `SlashJudge`.
...
- **[ADR 022 — Content Discovery at Scale](022-content-discovery.md#adr-022--content-discovery-at-scale)** — `cdn/dht/v1` Kademlia subset for content discovery (primary from PoC onward); demand surfaced via DHT FIND_VALUE query frequency and local cache-miss timestamps. No discovery fees.
- **[ADR 024 — Account Abstraction and Safe Smart Wallet Support](024-account-abstraction.md#adr-024-account-abstraction-and-safe-smart-wallet-support)** — Universal `SignatureChecker` across all contracts; Safe as the recommended wallet for nodes and clients; session keys via ERC-7579 `smartsessions` deferred to production.
- **[ADR 026 — Tokenomics](026-tokenomics.md#adr-026-tokenomics)** — Canonical economic-model umbrella: 1B fixed supply, the `FeeRouter` six-bucket split (40/40/7/5/5/3), operator economics, slashing-burn, bootstrap, and the governable-parameter bounds. Per-bucket detail is split into [ADR 034](034-gauge-boost-voting-escrow.md#adr-034-gauge-boost-and-voting-escrow) (gauge/ve), [ADR 035](035-delegator-pool.md#adr-035-delegator-pool) (delegator), and [ADR 033](033-safety-insurance-reserve.md#adr-033-safety-and-insurance-reserve) (safety reserve).
- **[ADR 028 — Slashing Appeals and Dispute Escalation](028-slashing-appeals.md#adr-028-slashing-appeals-and-dispute-escalation)** — 30-day post-slash appeal window via `SafetyReserve` restitution; emergency multisig fast-track + 14-day ve-Governor ratification; one accepted appeal per operator per 365 days.
- **[ADR 030 — Node Region Self-Attestation](030-node-region-self-attestation.md#adr-030-node-region-self-attestation)** — Region claims in `NodeAnnounce` and `StakingRegistry` are accepted at face value; the IP-geolocation oracle / third-party attestation path is explicitly rejected. Appeals-standing flipping and reactive blacklist-scope flipping are closed by a 7-day `regionLastChanged` stability window on `StakingRegistry` (governable `[3d, 30d]`; [ADR 011 § Standing](011-content-takedown.md#standing) path 2 and [§ Regional Scope](011-content-takedown.md#regional-scope)). Latency-vs.-claim reputation penalty from [ADR 001 § Consequences](001-network.md#consequences) is the canonical soft mitigation for residual pre-positioned misdeclaration.
- **[ADR 031 — ContentBlacklist Appeal-Contract Surface](031-content-blacklist-appeals-contract.md#adr-031-contentblacklist-appeal-contract-surface)** — Pins the contract surface for [ADR 011 § Blacklist Entry Appeals](011-content-takedown.md#blacklist-entry-appeals): per-appeal storage layout, event-topic ordering, state machine, and integration with `ContentBlacklist` core (suspension flag, `_removeHashRegional`, permissionless cleanup).
...
- **[ADR 033 — Safety and Insurance Reserve](033-safety-insurance-reserve.md#adr-033-safety-and-insurance-reserve)** — The 3% `FeeRouter` safety/insurance bucket: `SafetyReserve` payout categories, the four-gate spending controls, epoch-FIFO cross-category payout ordering with permissionless queued disbursement, interface stability, and the `ISafetyReserve` contract surface. Split out of [ADR 026 § Safety and insurance reserve (3% bucket)](026-tokenomics.md#5-safety-and-insurance-reserve-3-bucket); the appeal-surface that feeds it is [ADR 032](032-safety-reserve-appeals-contract.md#adr-032-safetyreserve-appeal-surface-contract-surface).
- **[ADR 034 — Gauge Boost and Voting Escrow](034-gauge-boost-voting-escrow.md#adr-034-gauge-boost-and-voting-escrow)** — The 40% gauge bucket: Curve-style ve-weighted gauge-boost formula, degenerate-input fallbacks, the per-operator gauge-share cap (5% default) that is the canonical wash-trading defense, and the non-transferable `VotingEscrow` contract (historical checkpointing, lock ownership). Split out of [ADR 026 §3–§4](026-tokenomics.md#3-gauge-boost-formula).
- **[ADR 035 — Delegator Pool](035-delegator-pool.md#adr-035-delegator-pool)** — The 7% delegator bucket: the USDC→TOKEN buy-and-distribute pipeline (`DelegatorBuyer`), its distinction from buyback-and-burn, the TOKEN-denomination rationale, and MEV/slippage handling. Split out of [ADR 026 § Delegator pool — USDC → TOKEN conversion](026-tokenomics.md#6-delegator-pool--usdc--token-conversion).
