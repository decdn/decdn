...
- **[ADR 003 — Payment Model](003-payments.md#adr-003-payment-model)** — Off-chain vouchers backed by a shared on-chain payment pool (USDC, fixed at deployment): one deposit, many capped signers, node-addressed vouchers, per-lane redemption. Metering rides a per-lane PayWord hash chain, so signatures are O(1) per transfer and the meter ticks once per 1 MiB chunk. Market-driven rates within governance-set bounds.
...
- A payment pool amortizes on-chain costs across every node and signer it backs; per-MB payments are off-chain vouchers extended by a hash chain, redeemed per node
