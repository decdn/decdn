...
- **[ADR 026 — Tokenomics](026-tokenomics.md#adr-026-tokenomics)** — Canonical economic-model umbrella: 1B fixed supply, the `FeeRouter` six-bucket split (40/40/7/5/5/3), operator economics, slashing-burn, bootstrap, and the governable-parameter bounds.
...
- **[ADR 033 — Safety and Insurance Reserve](033-safety-insurance-reserve.md#adr-033-safety-and-insurance-reserve)** — The `FeeRouter` safety/insurance bucket: `SafetyReserve` payout categories, the four-gate spending controls, epoch-FIFO cross-category payout ordering with permissionless queued disbursement, interface stability, and the `ISafetyReserve` contract surface. Split out of [ADR 026 § Safety and insurance reserve (3% bucket)](026-tokenomics.md#safety-and-insurance-reserve-3-bucket); the appeal-surface that feeds it is [ADR 032](032-safety-reserve-appeals-contract.md#adr-032-safetyreserve-appeal-surface-contract-surface).
- **[ADR 034 — Gauge Boost and Voting Escrow](034-gauge-boost-voting-escrow.md#adr-034-gauge-boost-and-voting-escrow)** — The 40% gauge bucket: Curve-style ve-weighted gauge-boost formula, degenerate-input fallbacks, the per-operator gauge-share cap that is the canonical wash-trading defense, and the non-transferable `VotingEscrow` contract.
- **[ADR 035 — Delegator Pool](035-delegator-pool.md#adr-035-delegator-pool)** — The 7% delegator bucket: the USDC→TOKEN buy-and-distribute pipeline (`DelegatorBuyer`), its distinction from buyback-and-burn, the TOKEN-denomination rationale, and MEV/slippage handling.
