...
- **[ADR 026 — Tokenomics](026-tokenomics.md#adr-026-tokenomics)** — Canonical economic-model umbrella under the v2.1 work-token rewrite: 1B fixed supply, 13-group allocation, the `FeeRouter` four-bucket split (60/25/10/5), the `CapacityBond` lock-to-capacity curve, capacity-shortfall slashing, Operator Service Emissions, operator-only governance, slashing-burn, bootstrap, and governable-parameter bounds.
