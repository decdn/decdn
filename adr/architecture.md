...
- **[ADR 014 — On-Chain Verification for Slashing Evidence](014-on-chain-verification.md)** — secp256k1 EIP-712 `slash_sig` on `ProbeResponse`/`StreamResponse`; unified `SlashJudge` contract for the three signature-dependent offenses (phantom, rate, blacklist). Content corruption is absorbed at the wire (no on-chain path) — see [ADR 003 §Corrupted delivery](003-payments.md#corrupted-delivery). Distinct-client diversity gating and wash-trading defense live in [ADR 027](027-distinct-client-receipts.md) and [ADR 026 §3](026-gauge-boost-tokenomics.md#per-operator-gauge-share-cap), not in `SlashJudge`.
...
- **[ADR 027 — Distinct-Client Diversity Gating](027-distinct-client-receipts.md)** — Gauge-pool eligibility gated on distinct-client funded-channel count, computed inline in `FeeRouter.routeSettlement` from settled-voucher state. Paired with the [ADR 026 §3](026-gauge-boost-tokenomics.md#per-operator-gauge-share-cap) per-operator gauge-share cap. Required for mainnet launch.
