...
Off-chain stablecoin payment channels for per-MB delivery, with on-chain settlement. Client-side architecture and smart-wallet support are included here because client trust boundaries and key management hang off the payment path.
...
- **Stablecoin behavior stability.** Allowlisted stablecoins are assumed not to change behavior post-approval (e.g., a proxy-upgradeable token adding fee-on-transfer). Changed token semantics could break settlement arithmetic or trap funds. Mitigation: launch is USDC-only, and every added stablecoin is allowlisted and vetted per [ADR 003 § Stablecoin Vetting Checklist](003-payments.md#stablecoin-vetting-checklist).
