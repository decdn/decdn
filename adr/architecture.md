...
- ~~Delegated voucher signer (hardware wallet HW key + session hot key)~~: decided — [ADR 003 Amendment](003-payments.md#amendment-delegated-voucher-signer) adds `setDelegate` / `clearDelegate` to `StablePaymentChannel`; hot key derivation is in [ADR 012](012-client.md)
