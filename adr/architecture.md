...
- **[ADR 034 — Gauge Boost and Voting Escrow](034-gauge-boost-voting-escrow.md#adr-034-gauge-boost-and-voting-escrow)** — The 40% gauge bucket: Curve-style ve-weighted gauge-boost formula, degenerate-input fallbacks, the per-operator gauge-share cap (5% default) that is the canonical wash-trading defense, and the non-transferable `VotingEscrow` contract (historical checkpointing, lock ownership). Split out of [ADR 026 § Gauge-boost formula – § Voting escrow (`VotingEscrow`)](026-tokenomics.md#gauge-boost-formula).
- **[ADR 035 — Delegator Pool](035-delegator-pool.md#adr-035-delegator-pool)** — The 7% delegator bucket: the USDC→TOKEN buy-and-distribute pipeline (`DelegatorBuyer`), its distinction from buyback-and-burn, the TOKEN-denomination rationale, and MEV/slippage handling. Split out of [ADR 026 § Delegator pool — USDC → TOKEN conversion](026-tokenomics.md#delegator-pool--usdc--token-conversion).
