...
- **[ADR 026 — Tokenomics](026-tokenomics.md#adr-026-tokenomics)** — Canonical economic-model umbrella under the v2.2 no-emission work-token rewrite: 1B fixed supply, 11-group allocation, the `FeeRouter` four-bucket split (60/25/10/5), the `CapacityBond` lock-to-capacity curve, capacity-shortfall slashing, the bounded one-shot Genesis Bond Credits program (in place of any ongoing service emission), App Incentives demand-side bucket, operator-only served-bytes-weighted governance per [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight), slashing-burn, bootstrap, and governable-parameter bounds.
