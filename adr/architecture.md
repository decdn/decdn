...
- **App server as trusted infrastructure.** The app server holds all blob encryption keys (`K_blob`) for encrypted content ([Appendix: Encrypted Content Publishing](appendix-encrypted-content-publishing.md)). Compromise of the app server key store exposes all content. Mitigation: epoch key rotation limits the blast radius for epoch-level access control, but does not provide forward secrecy for stored blob keys — this is an accepted trade-off documented in the encrypted-content-publishing appendix.
