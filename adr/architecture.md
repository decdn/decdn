...
The economic model that ties the protocol together. [ADR 026](026-tokenomics.md) is the canonical umbrella (supply, the `FeeRouter` six-bucket split, operator economics, slashing-burn, bootstrap, governable parameters); the per-bucket detail ADRs are [ADR 034](034-gauge-boost-voting-escrow.md) (gauge-boost formula + `VotingEscrow`, incl. the per-operator gauge-share cap that defends against wash-trading), [ADR 035](035-delegator-pool.md) (delegator pool), and [ADR 033](033-safety-insurance-reserve.md) (`SafetyReserve`, 3% safety/insurance bucket); [ADR 018](018-liquidity-strategy.md) is the Balancer V3 POL. The launch contract surface is forward-compatible (additive integration via standard `AccessControl` role grants per [ADR 016 §5](016-contract-interactions.md#5-access-control-matrix)) so future economic-layer products can land as additive top-level contracts without changing existing contracts.

1. [ADR 026 — Tokenomics](026-tokenomics.md) (canonical umbrella)
2. [ADR 034 — Gauge Boost and Voting Escrow](034-gauge-boost-voting-escrow.md)
3. [ADR 035 — Delegator Pool](035-delegator-pool.md)
4. [ADR 033 — Safety and Insurance Reserve](033-safety-insurance-reserve.md)
5. [ADR 018 — Liquidity Strategy (Balancer 80/20 POL)](018-liquidity-strategy.md)
...
5. [ADR 032 — SafetyReserve Appeal-Surface Contract Surface](032-safety-reserve-appeals-contract.md) — contract-implementation pin for [ADR 028](028-slashing-appeals.md)'s appeal flow
6. [ADR 031 — ContentBlacklist Appeal-Contract Surface](031-content-blacklist-appeals-contract.md) — contract-implementation pin for [ADR 011](011-content-takedown.md)'s blacklist-entry appeals
...
11. [Operator Protocol-Upgrade Runbook](appendix-operator-upgrade-path.md) — sequenced operator actions for each [ADR 013](013-schema-evolution.md) tier (Tier 1/2 checklists; Tier 3 rolling-upgrade procedure; client and governance coordination)
...
- **[ADR 008 — Reputation System](008-reputation.md)** — Interaction-weighted scoring with gossip propagation; complements the [ADR 034 § per-operator gauge-share cap](034-gauge-boost-voting-escrow.md#per-operator-gauge-share-cap) as off-chain wash-trading signal.
...
- **[ADR 026 — Tokenomics](026-tokenomics.md)** — Canonical economic-model umbrella: 1B fixed supply, the `FeeRouter` six-bucket split (40/40/7/5/5/3), operator economics, slashing-burn, bootstrap, and the governable-parameter bounds. Per-bucket detail is split into [ADR 034](034-gauge-boost-voting-escrow.md) (gauge/ve), [ADR 035](035-delegator-pool.md) (delegator), and [ADR 033](033-safety-insurance-reserve.md) (safety reserve).
...
- **[ADR 032 — SafetyReserve Appeal-Surface Contract Surface](032-safety-reserve-appeals-contract.md)** — Pins the contract surface for [ADR 028](028-slashing-appeals.md): per-appeal storage layout, escrow accounting, all six Solidity event signatures (`SlashAppealOpened` / `FastTracked` / `Rejected` / `Ratified` / `Reversed` / `Lapsed`), and a permissionless `cleanupExpiredAppeal` entry point.
- **[ADR 033 — Safety and Insurance Reserve](033-safety-insurance-reserve.md)** — The 3% `FeeRouter` safety/insurance bucket: `SafetyReserve` payout categories, the four-gate spending controls, epoch-FIFO cross-category payout ordering with permissionless queued disbursement, interface stability, and the `ISafetyReserve` contract surface. Split out of [ADR 026 §5](026-tokenomics.md#5-safety-and-insurance-reserve-3-bucket); the appeal-surface that feeds it is [ADR 032](032-safety-reserve-appeals-contract.md).
- **[ADR 034 — Gauge Boost and Voting Escrow](034-gauge-boost-voting-escrow.md)** — The 40% gauge bucket: Curve-style ve-weighted gauge-boost formula, degenerate-input fallbacks, the per-operator gauge-share cap (5% default) that is the canonical wash-trading defense, and the non-transferable `VotingEscrow` contract (historical checkpointing, lock ownership). Split out of [ADR 026 §3–§4](026-tokenomics.md#3-gauge-boost-formula).
- **[ADR 035 — Delegator Pool](035-delegator-pool.md)** — The 7% delegator bucket: the USDC→TOKEN buy-and-distribute pipeline (`DelegatorBuyer`), its distinction from buyback-and-burn, the TOKEN-denomination rationale, and MEV/slippage handling. Split out of [ADR 026 §6](026-tokenomics.md#6-delegator-pool--usdc--token-conversion).
