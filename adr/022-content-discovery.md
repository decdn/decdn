...
3. Schedule re-publish at `T + 45 minutes`, where `T` is the local wall-clock at which step 2 was last performed for this blob. Re-publish reuses the same step 1–2 sequence; missed slots (e.g., due to local downtime) re-fire at the next scheduler tick rather than back-filling.
