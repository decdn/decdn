...
The gate governs the protocol-driven *speculative acquisition decision* only. The cache role remains permissionless: any bonded operator may serve any hash they have, and the cache-miss pull-through path used to fulfill an in-flight `cdn/client/v1` `StreamRequest` from a paying customer is unaffected by these knobs (it has its own selection logic and is driven by an active paid request, not by speculative popularity inference).
