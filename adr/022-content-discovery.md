...
The on-chain origin set is also the directory of last resort if the DHT returns no providers: resolve namespaces via `PublisherRegistry.namespaceOf(hash)` and union the operator-address sets via `OriginAssignment.getOrigins(namespaceId)` for each ([ADR 011 § Origin Assignment Authority](011-content-takedown.md#origin-assignment-authority)). Operator addresses are mapped to NodeIds via `StakingRegistry.getActiveNodes` (returning `(operator, nodeId, lastSettlementAt)` tuples — see [ADR 016 § Off-Chain Read API](016-contract-interactions.md#off-chain-read-api-client--node-bootstrap)); the requester then probes those NodeIds directly. This fallback is uncommon — under normal operation the DHT contains entries for every actively-serving authorized origin — but provides a deterministic recovery path during DHT churn or bootstrap. For default-open content (`namespaceId == 0`) the on-chain directory is `OriginAssignment.getOrigins(0)` — the DAO-maintained default-open allow-list — resolved to NodeIds the same way as for registered namespaces. During the bootstrap window before the allow-list is first activated (`defaultOpenAllowlistActive == false`), `getOrigins(0)` is empty and the DHT path is the only discovery channel; once activated, the allow-list provides the same fallback as registered namespaces.
