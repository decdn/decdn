...
- Nodes publish `(hash → NodeId, coverage)` records **once they hold at least one verified byte of a blob** — a voluntary, self-interested advertisement to attract paying clients. No economic incentive exists to publish records for blobs you don't hold (false records attract probes that reveal the lie, degrading reputation and earnings).
...
/// genuine disjointness between holders. An empty bitmap means "holds nothing";
/// an all-ones bitmap means a full holder.
struct Coverage {
...
When a node verifies its first byte of blob H — on any verified partial, not only on `Complete`:
...
17. A `BatchStoreRequest` consumes between 1 and `n` tokens from each rate-limit bucket: stage 1 charges 1 token before deserialization; stage 2 charges up to `n - 1` more after reading `n = entries.len()`, bounded by remaining budget. The first `k` admitted hashes pass per-hash processing; the remaining `n - k` are marked `false` in the ack without further processing.
...
22. A `cdn/probe/v1` response carries the holder's live coverage bitmap as an unsigned field; the signed set `{hash, has_blob, rate_per_mb, timestamp_us}` is unchanged, and `has_blob` is true iff the node holds ≥1 verified byte of `H`. No contract change is required.
23. A probe response with `has_blob: false` and a non-empty coverage bitmap is malformed: the requester rejects it and scores a protocol violation. A holder cannot earn by advertising coverage while signing `has_blob: false`.
