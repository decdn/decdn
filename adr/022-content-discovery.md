...
**Origin-only discovery.** A requester that needs the canonical origin for a hash in a registered namespace (e.g., a cache-miss pull where only an authorized origin is acceptable) discovers it through the standard DHT path and then validates the role at probe time. The DHT does not discriminate origin vs cache providers — `StoreRequest` is the same wire format regardless of role — so any cached or origin holder may publish a record. The requester then relies on the `is_origin` field in the probe response (see [ADR 005 § cdn/probe/v1](005-protocol.md#cdnprobev1--latency-probe)) to confirm role.

If the DHT returns no origin-confirmed providers, the requester may consult on-chain state as a directory of last resort: resolve the namespace via `PublisherRegistry.namespaceOf(hash)` and read the authorized operator-address set via `OriginAssignment.getOrigins(namespaceId)` ([ADR 011 § Origin Assignment Authority](011-content-takedown.md#origin-assignment-authority)). Operator addresses are mapped to NodeIds via `StakingRegistry.getActiveNodes` (returning `(operator, nodeId, lastSettlementAt)` tuples — see [ADR 016 § Off-Chain Read API](016-contract-interactions.md#off-chain-read-api-client--node-bootstrap)); the requester then probes those NodeIds directly and confirms `is_origin: true` at the probe step. This fallback is uncommon — under normal operation the DHT contains entries for every actively-serving authorized origin — but provides a deterministic recovery path during DHT churn or bootstrap. For default-open content (`namespaceId == 0`) the on-chain directory of last resort is `OriginAssignment.getOrigins(0)` — the DAO-maintained default-open allow-list — resolved to NodeIds the same way as for registered namespaces. During the bootstrap window before the allow-list is first activated (`defaultOpenAllowlistActive == false`), `getOrigins(0)` is empty and the DHT path is the only discovery channel; once activated, the allow-list provides the same fallback as registered namespaces.
