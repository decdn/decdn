...
#### Origin authorization for cache warming

The cache role is permissionless by default: any bonded operator may serve any hash it holds, and the cache-miss pull-through path used to fulfill an in-flight `cdn/client/v1` `StreamRequest` from a paying customer pulls and warm-caches unconditionally, driven by an active paid request rather than any speculative inference.

An operator who does not want the reactive path to relay or warm *unclaimed* content MAY opt it into an authorized-origin restriction via the cache-side `pull_through_require_authorized_origin` knob (default `false`, so the permissionless default is unchanged; the gate is specified alongside the seed-leech caps in [ADR 037 § Seed-leech caps](037-regional-proxy-warming.md#seed-leech-caps)). When set, a pull-through initiates for a hash only if at least one operator is currently authorized as origin for the hash's namespace via `OriginAssignment.getOrigins(namespaceId)`; a `namespaceId == 0` request has no authorized origins, so an enabled gate refuses it (see [ADR 011 § Origin Assignment Authority](011-content-takedown.md#origin-assignment-authority)). `OriginAssignment` state is kept current via subscription to `AssignmentActivated` and `AssignmentRevoked` events against the local registry cache (the same pattern used for blacklist polling and channel-state queries); on RPC unavailability the gate fails closed until the cache recovers. The gate refuses only the *initiation* of a pull (and the cache-warming write it would drive) — a range the node already holds is served regardless, since refusing already-held content stays the job of [`ContentBlacklist`](011-content-takedown.md#adr-011-content-takedown-and-hash-blacklisting). The same `OriginAssignment` resolution — hash → namespace → authorized origin → active NodeId — also backs the FIND_VALUE last-resort origin fallback (§ [FIND_VALUE Flow](#find_value-flow-cache-miss--dht-lookup)).

A publisher whose `OriginAssignment` has not yet been ratified (timelock per [ADR 009](009-governance.md#adr-009-governance-model)) does not get gated cache-tier propagation during the wait — content is served from their own configured origin until ratification. For a truly-unclaimed hash (`namespaceId == 0`, which has no authorized origins) an operator nonetheless wants resident, the operator MUST explicitly pin (see [appendix-blob-cache-eviction.md § Operator pinning overrides LRU](appendix-blob-cache-eviction.md#operator-pinning-overrides-lru)).
...
| `NodeAnnounce` gossip | Unchanged. Carries node-level metadata only (region, load); content discovery derives from DHT FIND_VALUE traffic, and local demand is served reactively (see Proxy warming row). No new gossip message types. |
...
17. A `BatchStoreRequest` consumes between 1 and `n` tokens from each rate-limit bucket: stage 1 charges 1 token before deserialization; stage 2 charges up to `n - 1` more after reading `n = hashes.len()`, bounded by remaining budget. The first `k` admitted hashes pass per-hash processing; the remaining `n - k` are marked `false` in the ack without further processing.
18. A `BatchStoreRequest` with `hashes.len() > 256` is rejected by closing the stream with `MALFORMED_MESSAGE` (`0x03`); the publisher MUST split larger publish sets across multiple batches.
19. A publisher whose `BatchStoreRequest` to a given receiver results in stream-close-without-ack MUST fall back to per-hash `StoreRequest` for subsequent publications to that receiver; the publisher SHOULD cache the negative result for some bounded duration to avoid repeated drop-and-fallback cycles.
