...
[ADR 026](026-gauge-boost-tokenomics.md) and [ADR 027](027-distinct-client-receipts.md) introduce three new gas-cost surfaces that the L2 selection in this ADR must be re-validated against before mainnet deployment. All three are economic, not architectural — they do not invalidate the Arbitrum One choice above, but they must be quantified on the chosen L2 with measured (not estimated) gas numbers prior to mainnet launch.

1. **`FeeRouter` byte-counter overhead per settlement.** Every `settleChannel` call routes through `FeeRouter.routeSettlement(operator, bytesDelivered, amount)`, which increments an operator's per-epoch `bytes_delivered` counter (ADR 026 §2 epoch mechanics). This is **5–15K gas of overhead** on top of the existing settlement transaction. At 100K settlements/year for a medium operator, the overhead alone is roughly **$8–$1,500/yr** at typical L2 gas prices — the spread is wide, and the actual cost on Arbitrum One must be measured against current fee markets rather than estimated. The aggregate per-settlement gas (including this overhead) must remain within operator-affordability bounds — cite the operator P&L cases in [ADR 026 §7](026-gauge-boost-tokenomics.md) as the affordability reference.

2. **MMR-append + byte-counter combined cost per `commitEpochReceiptRoot`.** [ADR 027 §4](027-distinct-client-receipts.md#per-epoch-summary) pins a per-(`operator`, `epochId`) rolling MMR accumulator on `FeeRouter`: each `commitEpochReceiptRoot` call appends one leaf and merges peaks bottom-up via O(log N) `keccak256` hashes — bounded above by within-peak depth + bagging-fold depth (~21 hashes at the reference scale of ~30k receipts/operator/month). The combined per-call cost — MMR append plus the [ADR 027 §4 Batch shape](027-distinct-client-receipts.md#batch-shape) leaf-count and per-(`operator`, `epochId`) storage updates — must be measured against Arbitrum One fee markets and must remain within operator-affordability bounds at the [ADR 026 §7](026-gauge-boost-tokenomics.md#7-operator-economics-and-minimum-stake) operator P&L scale. The plain-hash O(N) alternative was rejected for L2-prohibitive cost; the MMR's O(log N) shape is what makes the receipt path viable on the chosen L2.

3. **Per-epoch keeper-call gas economics.** The design requires **two TWAP-protected USDC→TOKEN swap keeper calls per epoch**: one for `BuybackBurner` (5% buyback-and-burn flow, inflow source per [ADR 018](018-liquidity-strategy.md)) and one for the delegator-pool USDC→TOKEN conversion (7% delegator-pool flow per [ADR 026 §6](026-gauge-boost-tokenomics.md)). At a 1-week epoch length, that is **52+ swap executions per year minimum**, both routed through the Balancer V3 80/20 TOKEN/USDC pool with TWAP windows, `minOut` bounds, and per-epoch liquidity caps. Per-epoch keeper costs must be sized at S2/S3 scale (the regimes where liquidity caps bind per [ADR 026 §8](026-gauge-boost-tokenomics.md) and the economic-model spec) and confirmed not to be cost-prohibitive against the inflow each call routes.

4. **Combined L2-selection validation gate.** Before mainnet deployment, the selection criteria in this ADR MUST additionally verify that:
   - Aggregate per-settlement gas (USDC-equivalent per settlement, including the `FeeRouter` byte-counter overhead) is within the operator-affordability bounds set by the operator P&L cases in [ADR 026 §7](026-gauge-boost-tokenomics.md).
   - Per-`commitEpochReceiptRoot` MMR-append cost is within operator-affordability bounds at the [ADR 026 §7](026-gauge-boost-tokenomics.md#7-operator-economics-and-minimum-stake) operator P&L scale and remains O(log N) under the chosen L2's storage-cost schedule.
