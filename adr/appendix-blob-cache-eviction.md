...
- Enforcement is after the fact, and since #1678 that is the whole admission model: a blob is admitted if it fits `cache.cache_size_mb`, and the [§ Eviction driver loop](#eviction-driver-loop) reconciles the rest once the footprint crosses high-water. Nothing reserves disk before a fill starts, so a burst of concurrent fills can overshoot the budget until the driver catches up.
