...
- Enforcement is after the fact. The [§ Eviction driver loop](#eviction-driver-loop) reclaims once the footprint crosses high-water; nothing reserves disk before a fill starts. A burst of concurrent fills can therefore overshoot the budget until the driver catches up. `decdn_cache_fill_in_flight_bytes` and `decdn_cache_pending_reclaim_bytes` measure that exposure (#1678); an up-front admission reservation is deferred until they show it is real.
- Reclaim lags eviction. `release_for_eviction` drops the protecting tag, and the bytes return only on the next iroh-blobs GC sweep (`cache.gc_interval_sec`). Sweeping on demand at the high-water crossing is blocked upstream ([#520](https://github.com/decdn/decdn/issues/520)): iroh-blobs 0.103 keeps `gc_run_once` in a private module.
