...
Under v2.2 ([ADR 009 § Production](009-governance.md#production-operator-weighted-dao-governance), [ADR 026 § Governance](026-tokenomics.md#governance)), DAO voting weight is `declared_capacity_Mbps × age_ramp(months_bonded)`, sourced from `CapacityBond.capacityAt × age_ramp`. The capacity figure is operator-asserted at registration time and only loosely tied to actual delivery — capacity-shortfall slashing ([ADR 026 § Capacity-shortfall slashing](026-tokenomics.md#capacity-shortfall-slashing)) is forward-looking and slow (4-week rolling probe window). A deeply-bonded but lightly-serving operator carries a vote weight that does not reflect their real contribution to the network.

The on-chain raw material to fix this already exists. [ADR 016 § Contract: FeeRouter](016-contract-interactions.md#contract-feerouter) populates `bytesPerEpoch[operator][epoch]` inline on every `routeSettlement`. Today that counter is labeled analytics-only; this ADR promotes it to be the canonical voting-weight source.
...
- **[ADR 016 — Smart Contract Interaction Model](016-contract-interactions.md#adr-016-smart-contract-interaction-model):** Contract Inventory `DecdnGovernor` row updated to read FeeRouter for vote weight. `classDiagram` `FeeRouter` gains `totalBytesPerEpoch`, `bytesInWindow`, `totalBytesInWindow`; `Governor` edges retargeted. `Contract: FeeRouter` section gains the new mappings, getters, and `setWindowEpochs`; the "analytics-only" comment is removed. `Contract: CapacityBond` section gains `slashedAtEpoch`. Cross-Contract Call Graph + Complete Call Table add `Governor → FeeRouter: bytesInWindow / totalBytesInWindow`. Deployment Order step 13 (DecdnGovernor) lists `FeeRouter` as a non-zero constructor input.
...
- **`docs/superpowers/specs/2026-05-27-remove-operator-emissions-design.md`:** The ADR rebase row for [ADR 009](009-governance.md#adr-009-governance-model) is updated to "amended; voting source moved per this ADR."
...
- Sibling spec: [`2026-05-27-served-bytes-voting-design.md`](../docs/superpowers/specs/2026-05-27-served-bytes-voting-design.md)
