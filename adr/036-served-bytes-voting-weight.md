...
This argument **depends on a minimum price-per-byte being enforced** — otherwise the attacker pays ~0 USDC per byte and the 40% non-refunded cost vanishes. That floor is the `PaymentPool.deliveryFloor` settlement check ([ADR 003 § Rate-floor enforcement](003-payments.md#rate-floor-enforcement)): `redeem` rejects any claim whose cumulative `amount / bytesDelivered` is below `deliveryFloor`, so each stamped byte costs `>= deliveryFloor` real USDC. The floor binds the **resolved** claim — the epoch base plus `chainIndex × chunkPrice` over `chainIndex × chunkBytes` ([ADR 003 § Redemption and Close](003-payments.md#redemption-and-close)) — and a node additionally refuses an epoch whose `chunkPrice / chunkBytes` is already below the floor, so the chain cannot dilute the price as the index grows. Without that floor the attack is near-free (`amount = 1` wei for `bytesDelivered = 2^200`), which the cost estimate below assumes away.
