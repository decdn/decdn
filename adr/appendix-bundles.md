...
such layer. Hand-authoring `path → hash` JSON is error-prone for any
non-trivial origin and a non-starter as a public publishing workflow,
so the `decdn bundle` subcommand ships a deterministic generator
alongside the on-disk format spec it produces — the format gets
validated by its own generator, and publishers never have to write a
bundle file by hand.

The deterministic-emit requirement (see [Determinism](#determinism))
is what makes single-hash distribution viable: a bundle file is just
bytes, so it has its own BLAKE3 like any other blob. **Publishers
announce a single bundle hash**, clients fetch the bundle blob first,
parse it, then fetch every entry by hash. No out-of-band file
distribution, no separate channel for handing out the JSON.
...
makes the hash function explicit on the wire and leaves room for a
later `<algorithm>:<hex>` tag if a different hash function ever needs
to live at this layer. **This is a bundle-format convention**, not a
project-wide rename: on-wire fields elsewhere in the codebase carry
bare hex (e.g. `decdn node evict <hash>`), and that stays unchanged.
Bundles are publisher-side and free to set their own conventions.
...
- **Sort.** `entries` is sorted by `path.as_bytes()` ascending. Bytewise
  on UTF-8 paths is the same as Unicode codepoint order under UTF-8's
  prefix-free encoding, and matches every other content-addressed sort
  in the workspace.
...
- **Single-line compact.** No whitespace between tokens, no indentation,
  no internal newlines. Pretty-printing is a `jq .` away when humans
  need to read it; the bytes themselves are content-addressed.
- **No trailing newline.** The bundle file's last byte is the closing
  `}`. Trailing whitespace is the most common source of byte-identical
  drift across editors and shells; omitting it removes the foot-gun.

## Bundle as blob

A bundle file is just bytes — it has a BLAKE3 like any other blob.
Origins serve the bundle bytes as one regular blob; publishers
announce the bundle hash; clients (eventually) fetch the bundle hash,
parse the JSON, then fetch every `entries[].hash` separately. The
bundle is leaf data, not a manifest in the protocol sense.
...
- `DECDNMAN` is a **transport-side, intra-file** binary format —
  one logical blob split into ordered chunks with shared total-size
...
nothing prevents an entry's `hash` from being a `DECDNMAN` manifest
blob hash for a chunked single file inside a bundled directory.

## Relationship to ADR 002 / ADR 013

- [ADR 002](002-content-addressing.md) declares multi-file an
  application concern. This appendix is that application concern,
  layered above the protocol — the network sees only the bundle blob
  and (later) each entry blob, never the bundle's structure.
- [ADR 013](013-schema-evolution.md) bounds *wire-format* evolution
  (varint framing, ALPN tiers, `#[serde(default)]` rules for signed
  payloads). Bundles are a **file format**, never on the wire, so
