...
**No per-relationship rate ramp — trust never gates delivery rate.** The security bound is the reserved floor `M`, not a per-relationship earned window. A node does **not** throttle a pool based on its history with that pool. A first-time pool and an established pool follow the same rules. Each stream ramps its own [credit window](#credit-window) from the floor of one chunk as that stream pays. The ramp resets on every new stream; it never carries over from the pool's or signer's past. Admission is the separate question: the per-signer floor sub-cap does read a signer's accumulated `dead_charge`, so a signer's past can refuse a stream, but it never slows one that is admitted. A fresh stream and an established stream both start at the floor and ramp identically. Trust affects only redemption *cadence* (`t`, a gas concern), never delivery rate. The only caveat is fan-out *width*, not per-stream rate: a fresh pool spread across very many nodes at once can reopen the bounded tail below. We accept this residual; it is never a reason to slow any single stream. This removes the cold-start penalty of a dynamic per-relationship window, which throttled every fresh `(node, pool)` pair until it earned history.

**Per-signer floor isolation.** The floor accumulator has two levels. Admission checks both. The per-pool ceiling is the solvency bound: the sum of un-vouchered floor credit across every signer on a pool never exceeds `remaining − M`. This holds however many signer identities draw on the pool. Underneath it, each `(pool, signer)` lane holds at most a **share** of the same headroom. `dead_charge` accrues per signer and rolls up into the pool total, so a signer that opens and abandons streams progressively locks *itself* out first. The sub-cap bounds each identity's draw. It does not guarantee a slice. For `N` signers at share `f` the caps sum to `N · f · (remaining − M)`. Once `N > 1/f`, the pool ceiling binds before every signer reaches its cap. The claim that holds in all cases is the weaker one: no single signer draws more than its share, so one misbehaving key cannot take the whole pool's floor budget. The contract's per-signer `spending_cap` does not do this work: it bounds *settled* spend, and the floor is by definition un-vouchered. This gates admission on un-vouchered exposure, not delivery rate: an admitted stream ramps identically whatever its signer's history, which is the property [No per-relationship rate ramp](#pool-solvency-and-the-refundable-floor-m) states.

**Key rotation is the recovery path.** `dead_charge` is permanent and clears only when the pool is reclaimed on-chain, and it counts against the signer's cap. Under a bare share — the default, with the window ceiling disabled — an owner top-up raises the headroom and so raises the cap, which lets a flaky-but-honest signer recover. Under an armed window ceiling it does not: a key that accrues `k` windows of abandoned-stream loss over the pool's lifetime stays refused. The protocol already supplies the remedy — capabilities carry an expiry and are issued per session (see [Revocation](#revocation)), so the publisher issues the client a fresh key and the spent lane retires with the old one. The node keys `dead_charge` by signer, so a new key starts with a clean cap. This is deliberate: it needs no decay schedule, no split live and dead budgets, and no per-signer history for the node to keep.

The cap is **node policy**, like `M`. It needs no pool-owner cooperation and no protocol field, because the floor is credit the node itself fronts — capping it per counterparty is the node bounding its own bad debt. It throttles **unpaid credit, not paid throughput**. A signer that pays its vouchers releases that stream's reservation, so its share recycles and a stream already under way is never slowed. What the share bounds is how many streams one signer holds un-vouchered at once.

**The unit is the credit window, not a slice of the deposit.** A signer's honest need does not scale with the pool's size. Every admission reserves at most one ramp-start credit window, and a paying stream releases that reservation as soon as it covers it. Honest need is therefore `concurrent un-vouchered streams × one window`, whether the pool holds ten dollars or a hundred thousand. The cap is `clamp(share, one window, k · one window)`. The share is a fraction of `remaining − M`. `k` is a node-local window count.

Both clamps earn their place, but only the lower one is always armed. The **lower** clamp keeps small pools usable: without it, a pool with less headroom than the share assumes gives every signer a sub-window cap, and that pool then serves nobody. With it, the node admits a lone signer's first stream on any pool. The **upper** clamp is what keeps the bound meaningful on a large pool, and it is **opt-in**: `k` defaults to `0`, which disables it. The reason is the interaction with `dead_charge` below. An admitted stream that fails without delivering folds a full window into that permanent total, so a client retrying a cold node burns one window per attempt, and a ceiling of `k` turns `k` transient warm-up failures into a permanent lockout for that session key. No constant escapes this, because the burn is bounded only by the pool's headroom. An operator who serves wide-fan-out publishers arms the ceiling; the default trades the fan-out bound for availability on ordinary cold starts. A bare share is loose in both directions there: one session key could hold millions of windows, and a constant `10_000 / share_bps` keys would still strand the whole floor — which is exactly the case a shared pool exists for, one publisher delegating many session keys. With the window ceiling, the damage one compromised key does is constant, and the number of keys needed to strand the floor scales with the deposit (`(remaining − M) ÷ (k · window)`).

**Residuals (honest):**

- **Fan-out.** `M` gives a zero tail only up to fan-out `N = M/t`. A pool fanned wider than the `M` it reserves reopens a bounded tail. We accept this residual rather than build machinery against it: each node's exposure to a fresh pool is only its own small redemption threshold `t`. Enlarging the aggregate tail forces the attacker to fan across proportionally many nodes and to physically pull real bytes from each — a real egress cost. Self-dealing is separately taxed by the FeeRouter cut. Sizing `M` for the expected fan-out closes this residual in the common case: honest clients are sticky and do not fan a brand-new pool across hundreds of nodes at once. A node also bounds a second, separate tail: the **un-vouchered floor** bytes it fronts across every stream on one pool before that stream's first voucher lands. A floor-credit accumulator tracks this live, un-vouchered exposure. It works at two levels: per pool and per signer. It refuses a new admission when either level is full. The pool level is `remaining − M`. The signer level is one signer's share of that, clamped below at one credit window and, when the operator arms the optional node-local window count, above at that (see **Per-signer floor isolation** above). The accumulator persists durably across a restart, per signer, so a crash never resets it. On one node, aggregate un-vouchered floor exposure against one pool therefore never exceeds `remaining − M`, no matter how many concurrent or sequential streams draw on it, and no single signer on that pool exceeds that clamped share. One residual survives *inside* a pool: a signer identity is free to mint, so an attacker holding the owner key can still reach the pool ceiling by spraying fresh signers. The sub-cap protects the **named co-tenants of a shared pool** — the case the pool exists for, one publisher delegating many session keys — and the pool ceiling remains the bound against identity fan-out. The larger residual that survives is **cross-node fan-out**: each node bounds only its own exposure to the shared `remaining − M`. `K` uncoordinated nodes serving the same pool can together reach up to `K × (remaining − M)` — the same shape of residual this ADR already accepts for the vouchered tail above. Closing it fully needs cross-node coordination the protocol does not have. A small additional residual survives a crash inside a lane's idle window, before stream-end reconcile persists its charge. It is bounded by the count of lanes withholding at the moment of the crash, times one floor — far smaller than the fan-out residual it sits beside. A clean shutdown does not add to it: stream-end reconcile queues each charge to a serial persist worker, and shutdown waits for that queue to drain after the last stream returns. A charge that still fails to reach disk is counted, so the residual is visible rather than silent. A stream that ends abnormally — a client disconnect, a rejected voucher, or a mid-stream stop — charges its **full reserved floor**, not the proportional bytes it happened to take. This conservative direction is what keeps the sequential bound tight: a cache-miss fill aborted before a single byte reaches the client still charges a floor, so an attacker who fronts the node's upstream USDC and then disconnects cannot repeat the pattern for free. The cost of the conservative choice is that an honest client that disconnects inside its first chunk also charges a full floor. `dead_charge` only grows; it clears only when the pool is reclaimed on-chain, and then for every signer on it at once. A long-lived pool whose clients occasionally disconnect therefore accrues a permanent per-node, per-signer floor charge and is eventually refused despite a healthy on-chain deposit — the owner tops the pool up to restore its budget, exactly as for the un-vouchered floor bound itself.
...
The per-lane bound is one chunk, but the maximum **free floor** a node ever fronts against one pool is bounded per pool, not per lane: a floor-credit accumulator caps the sum of every stream's un-vouchered exposure against that pool to `remaining − M`, and caps any one signer's draw to a share of that same headroom, so opening many lanes against the same under-funded pool does not multiply the free bytes an attacker collects, nor let one signer take the whole pool's free budget from its co-tenants (see [Pool solvency and the refundable floor `M`](#pool-solvency-and-the-refundable-floor-m)).
