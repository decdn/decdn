...
- Fee discount mechanic from [ADR 004](004-tokenomics.md) (10× stake → 1.5% fee) is **removed** under [ADR 026](026-gauge-boost-tokenomics.md); operator return is differentiated through ve-locked gauge boost rather than a discounted skim percentage.
...
The "Fee percentage" bound from [ADR 004](004-tokenomics.md) is removed — `StablePaymentChannel` no longer holds a fee parameter. Bucket-share bounds (40/40/7/5/5/3 + `boostFloor`) are owned by `FeeRouter` per [ADR 026 §11](026-gauge-boost-tokenomics.md#11-governable-parameters-with-safety-bounds).
...
**PoC note:** The `BuybackBurner` contract is deployed with the same interface, but `executeBuyback` is not called during the PoC. The 20% buyback allocation in [ADR 004](004-tokenomics.md#buybackburner-contract) is superseded by the 5% same-tx burn bucket in [ADR 026 §2](026-gauge-boost-tokenomics.md#2-feerouter-split-40407553) — under v3 the inflow source is `FeeRouter` (per-settlement same-tx transfer), not manual treasury transfer. USDC accumulates in the contract without being swapped during the PoC.

### FeeRouter Integration

Under [ADR 026](026-gauge-boost-tokenomics.md), `StablePaymentChannel.settleChannel` does not split fees inline. The full operator-bound USDC balance is forwarded to a `FeeRouter` contract, which applies the canonical six-bucket split (40% node base / 40% gauge boost / 7% delegator pool / 5% buyback-and-burn / 5% treasury / 3% safety reserve — full table and bounds in [ADR 026 §2](026-gauge-boost-tokenomics.md#2-feerouter-split-40407553) and [§11](026-gauge-boost-tokenomics.md#11-governable-parameters-with-safety-bounds)). This ADR specifies the `FeeRouter` interface only as it relates to the settlement path; the gauge formula, ve-escrow mechanics, and bucket disbursement schedule live in ADR 026 and the design spec ([`docs/superpowers/specs/2026-04-18-tokenomics-v2-gauge-boost-design.md`](../docs/superpowers/specs/2026-04-18-tokenomics-v2-gauge-boost-design.md) §2.2 / §5).
...
**Node-to-node cache-miss paid pulls bypass the router entirely.** When node B pulls a blob from origin-backed node A and pays via a payment channel, that channel's settlement is an internal cost-recovery flow between two operators — not net protocol revenue. The fee skim would double-charge the same revenue (once when B pays A, again when B's clients pay B for the same bytes). Per [ADR 026 §2](026-gauge-boost-tokenomics.md#2-feerouter-split-40407553):
