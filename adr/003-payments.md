...
- **Voucher** — signed by the authorized `signer` over `{ pool_id, signer, provider, cumulative, bytes_delivered, nonce, token }`. It is **node-addressed**: it names the payee `provider`. `provider` is mandatory because `pool_id` no longer encodes the payee; without it the contract could not attribute a payment or keep independent per-node ordering, and one node could redeem a voucher meant for another.
...
**The window `c`** is a per-node cap on *unredeemed* value against one pool, tracked from the node's own vouchers — not a time interval. A node serves at most `c` before it must redeem to continue, so its exposure to a pool never exceeds `c`. Overdraft is handled by refusing to serve, so no eager redemption is needed to defend and there is no griefing. A drain is not an immediate loss: redemption is partial and retriable, so a node that hits an empty pool collects what it can now and the rest after a top-up — the value is only truly lost if the owner closes and reclaims without ever refilling, still bounded by `c`.
...
3. **Check payee.** Require `provider == msg.sender`. Redemption is cumulative, not nonce-gated (see below), so no on-chain nonce check is needed.
...
**Batch redemption.** `redeemMany([...])` redeems across many lanes (and pools) in one transaction, each entry applied with the single-`redeem` arithmetic above. An entry that would pay `0` — drained pool, already-paid or stale voucher, expired capability, or cap reached — is **skipped, not reverted**, so one empty lane never sinks the batch; it emits one `PoolRedeemed` per paid entry and returns the total paid. Only a structurally-invalid entry — a bad voucher signature, or `provider != msg.sender` — reverts the whole call, since that is caller error, not transient pool state.
...
| `StaleNonce` | `voucher.nonce ≤` the node's off-chain accepted nonce | none — an off-chain ordering guard; on-chain a stale voucher pays `0` and reverts `NothingToRedeem` ([Voucher Nonce Convention](#voucher-nonce-convention)) |
| `AmountRegression` | `voucher.amount <` the node's accepted cumulative | none on-chain (`redeem` pays `cumulative − paid`, so a regressed voucher pays `0`); the reject keeps the node's own ledger consistent |
...
The self-enforcing stop is sufficient. Maximum loss is one voucher interval at the negotiated cadence: default cadence (1 MB × market rate ≈ $0.00001) is negligible; 100 MB at market rate is ~$0.001; the wire ceiling (1024 MB) at the ceiling rate is ~$1.024 — still negligible relative to pool deposits. Nodes serving high-value content can unilaterally enforce smaller intervals regardless of what was negotiated.

The stop is per-lane and mechanical. The node pauses the moment the unpaid balance on a lane reaches that lane's [credit window](#credit-window) and waits for the next higher-nonce voucher on the same lane; it never fronts more than one window against one lane. The pause keys on the lane, not the counterparty. A client that stalls or crashes mid-stream and reconnects is treated no differently from a first request: it resumes on the same lane under the same window and pays only for the bytes it vouchers, never for the interrupted window. The node's own cost of the interruption — re-reading a held blob, or dropping and later re-pulling an unfinished cache-miss fill — is bounded to that one window either way, and no black mark follows the client into its next request. No history accumulates against a client across connections, and none needs to: a fresh signer from the same party gets its own window and is bounded the same way. The bound is a property of each lane, so a new key buys an attacker nothing it did not already have.
...
| Lifecycle | `redeem(poolId, signer, provider, cumulative, bytesDelivered, nonce, voucherSig, capability)` | Provider-only (`provider == msg.sender`): register the signer on first use (verify the owner `capability`), then pay `min(desired, capRoom, remaining)` of a cumulative voucher — the still-unpaid portion, capped by the signer's remaining cap and the pool balance; routes the paid amount through `FeeRouter` same-tx. Reverts `NothingToRedeem` if nothing is payable (writes no state, so it is retried). No dispute window. See [Redemption and Close](#redemption-and-close). |
| Lifecycle | `redeemMany(entries[])` | Provider-only: apply `redeem` to each entry; **skip** (do not revert) any entry that would pay `0` — drained pool, stale/already-paid voucher, expired capability, or cap reached — so one empty lane never sinks the batch. Reverts only on a bad signature or `provider != msg.sender`. Returns the total paid. |
...
**`redeem` behavior:** `redeem(poolId, signer, provider, cumulative, bytesDelivered, nonce, voucherSig, capability)` pays a node against a monotone voucher **while the pool is `Open` or in the grace window**. It is safe without a dispute window because a voucher is a signed, cumulative claim by a capped signer and a node redeems only its own lane. The residual is the signing key itself — a compromised key can authorize up to its `spending_cap`, retired by the capability's `expiry` (see [Redemption while the pool is Open](#redemption-while-the-pool-is-open-no-dispute-window)).
...
- **Voucher validation.** Verify `voucherSig` against `signer` (see [EIP-712 Voucher Signature](#eip-712-voucher-signature)); require `block.timestamp < authorized[poolId][signer].expiry` and the [rate floor](#rate-floor-enforcement) on `cumulative` / `bytesDelivered`. There is no on-chain nonce gate — redemption is cumulative.
...
**Tracking owed vs. paid.** A node needs no bookkeeping beyond what it already keeps, and it never guesses the paid side. *Paid* is authoritative on-chain: `watermark[poolId][signer][provider].amount` is the cumulative USDC actually paid on the lane, readable by `eth_call` and echoed as `newPaidCumulative` in `PoolRedeemed`. *Owed* is the highest voucher cumulative the node has accepted, which it already persists per lane for replay-safety ([Off-chain voucher state persistence](#off-chain-voucher-state-persistence)). Unredeemed value on a lane is `owed − paid`. After each `redeem` the node updates its paid figure from the tx result — *not* by assuming its voucher cleared, which a drained pool would violate — sums `owed − paid` across its lanes to enforce the redemption credit window, and keeps a lane pending while `owed > paid` (retrying after a top-up). To avoid gas on a doomed `redeem`, it can first read `getPool` (`deposit − totalRedeemed`, the pool balance) and `getAuthorization` (`cap − spent`, the signer's room) and wait for a `PoolToppedUp` event when the pool is dry rather than submit a call that would revert `NothingToRedeem`. All of this survives a restart: paid is re-read from chain, owed from the persisted per-lane store.
...
`PoolOpened` indexes `poolId` and `owner`, so an owner lists its pools via `eth_getLogs(topics=[PoolOpened, *, paddedOwnerAddress])`; an indexer keys on `poolId`. `redeem`, `signer`, and `provider` are indexed on `PoolRedeemed`, so a node can enumerate the lanes it has been paid on.
...
**No pool expiry.** A pool has no lifetime cap and never expires; it stays `Open` until the owner closes it. It configures no duration parameter and has no time-based reclaim. Owner funds are recoverable only by the owner (`closePool` then `reclaim`), so they are never stranded, and a node's serving window is bounded by each signer's capability `expiry` — the deadline `redeem` enforces per lane.
...
bytes32 constant VOUCHER_TYPEHASH = keccak256(
    "Voucher(bytes32 poolId,address signer,address provider,uint256 amount,uint256 bytesDelivered,uint256 nonce,address token)"
);
```
...
bytes32 voucherDigest = keccak256(abi.encodePacked(
    "\x19\x01",
    DOMAIN_SEPARATOR,
    keccak256(abi.encode(VOUCHER_TYPEHASH, poolId, signer, provider, amount, bytesDelivered, nonce, token))
));
...
The voucher `nonce` is an **off-chain** field: it orders vouchers within a `(poolId, signer, provider)` lane and lets a node detect a dropped `VoucherAck` without desynchronizing. On-chain redemption does not use it — settlement is purely cumulative (`redeem` pays `cumulative − paid`), so ordering and replay are handled by the monotone paid watermark, not the nonce. Nonces start at **1** (0 is the sentinel for "no voucher yet" in the node's off-chain store). Two consequences:

- A node advances its off-chain accepted nonce per accepted voucher, and rejects a voucher whose nonce does not exceed it (`StaleNonce`, [ADR 005](005-protocol.md#adr-005-wire-protocol)) — an ordering guard on its own delivery, not a settlement gate.
- Because the on-chain lane records only cumulative paid amount, the same voucher can be re-presented to `redeem` after a top-up to collect an earlier drained-pool shortfall (its nonce is unchanged; only the unpaid `cumulative − paid` remains).
...
```
{poolId, signer, provider, amount, bytesDelivered, nonce, token, signature}
```
...
- **Cumulative, monotonic per lane.** Like `amount` and `nonce`, `bytesDelivered` is strictly non-decreasing across vouchers within a `(poolId, signer, provider)` lane. `redeem` computes the byte delta against the lane's last redeemed cumulative.
