...
Client opens many channels with a dust deposit and never streams, on the theory that this forces nodes to track and eventually close stale channels.

**Not a real vector: nothing forces the node to act.** A channel that never streams produces no voucher, and seller-side channel state is created only when a voucher is accepted — after bytes flow. So the node never even tracks an un-streamed channel; there is nothing to enumerate, watch, or close. A channel that streamed only dust leaves the node holding a dust voucher, and closing is a call the node makes to *collect what it is owed*, not a maintenance obligation. If a `closeChannel` would cost more gas than the voucher recovers, the rational node simply does not close and loses nothing: the deposit is the client's own money, which the client alone recovers via `reclaimExpired` (at the client's own gas cost) after channel expiry. The only party who spends anything in this scenario is the attacker, on `openChannel` gas (~$0.05 each) with the deposit fully refundable, for no effect on any node. There is no defender cost to exploit, so no channel-count cap or inactivity filter is warranted.

A voucher-less close (`closeChannelWithoutVoucher`, canonically specified in [Fee Routing on Disputed Closes](#fee-routing-on-disputed-closes)) is available to either party, but its purpose is client failure-recovery rather than griefing defense: it lets a client who funded a channel escape the 90-day `reclaimExpired` wait when a node fails before the first voucher boundary — a routine ops failure with no malicious actor. Restricting the path to providers would create a structural liquidity-lock on every node-failure event, contrary to the intended failure-mode posture. The path is permissionlessly disputable if the closing party actually signed a voucher off-chain, which also closes the repudiation surface below.
