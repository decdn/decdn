...
The model uses two signed objects, one unsigned object, and an on-chain sharded register.
...
- **Voucher** — signed by the authorized `signer` over `{ pool_id, signer, provider, cumulative, bytes_delivered, chain_root, chain_length, chunk_bytes, chunk_price }`. It is **node-addressed**: it names the payee `provider`. `provider` is mandatory because `pool_id` no longer encodes the payee; without it the contract could not attribute a payment or keep independent per-node ordering, and one node could redeem a voucher meant for another. A voucher opens one **chain epoch** on one lane. The signer issues it once per epoch, not once per delivery interval.
- **Preimage** — unsigned, 32 bytes, released by the signer for each delivered chunk. To pay for chunk `k` of the epoch the signer releases `p_k`, where `k` applications of `keccak256` to `p_k` give `chain_root`. The node verifies it with one hash against the preimage it already holds. A preimage proves its own index, so it is the receipt: the node acknowledges nothing and persists nothing to accept it.

**The signature cadence and the metering cadence are independent.** The voucher carries the signature and the on-chain claim. The preimage carries the per-chunk payment. One voucher covers `chain_length` chunks, so a lane signs once per epoch and meters once per chunk. This is what lets the meter be much finer than a signed-per-interval scheme permits. At the default `chunk_bytes` of 64 KiB a chunk costs one hash on each side and no round trip, where a signed voucher costs a signature, a verification, and an acknowledgement.

The mechanism operates at two tiers, both on the same pool primitive:

- **Client → node**: a client opens a pool and issues capped capabilities to one or more signers (its own key, or per-device or per-session keys it delegates). Each signer opens a chain epoch with each node it fetches from and streams preimages, and a node redeems its own lane on-chain.
- **Node → node**: when a node pulls content from another node (typically an origin-backed node), it pays from its own pool via the same mechanism. The origin-backed node is paid wholesale; the pulling node recoups this by serving multiple clients from its cache at a markup.

A node redeems its lane on-chain at any time while the pool is open. It submits the current voucher and the highest preimage it holds. Redeeming a signed, monotone claim needs no dispute window (see [Redemption and Close](#redemption-and-close)). The owner closes the pool to reclaim the unspent remainder. A redemption grace window (default 48 hours, governable within 48h–72h — see [ADR 009](009-governance.md#adr-009-governance-model)) lets each node redeem outstanding payments before the owner reclaims. Reclaim returns the deposit minus the total redeemed across all lanes.
...
- Metering cadence: one preimage per `chunk_bytes` delivered (default 64 KiB, node-advertised lane policy — see [Chunk Cadence](#chunk-cadence))
- Signature cadence: one voucher per `chain_length` chunks (default 1024, so one signature per 64 MiB at the default chunk size)
...
A signed voucher per delivery interval is expensive. It costs a signature, a verification, and an acknowledgement round trip, and the node must know the accepted state is durable before it serves further bytes. That cost forces a coarse interval, and a coarse interval is what the payer can withhold. The chain epoch removes the cost, so the meter can be fine.

**`chunk_bytes` is lane policy, not a negotiation.** The node advertises `chunk_bytes` in `StreamResponse` ([ADR 005](005-protocol.md#chunk-cadence)). The signer builds its chain to that value and commits to it in the voucher's `chunk_bytes` field. The value is then fixed for the epoch, and it is a property of the lane rather than of a stream. Streams that join the lane later inherit it. There is nothing to reconcile between concurrent streams and nothing to renegotiate mid-epoch; a signer that wants a different chunk size ends the epoch and issues a new voucher.

**One chain per lane, never shared.** A signer MUST build an independent chain for every `(provider, epoch)`, from a fresh seed. A preimage carries no payee — it is a bearer token for its chain — so two lanes sharing a root let either node spend the other's preimages, at the signer's expense (see [Chain-root reuse across lanes](#chain-root-reuse-across-lanes)). Independence is what makes the signed `provider` field sufficient: the field stops a *voucher* from being redeemed by the wrong node, and a per-lane chain stops a *preimage* from being worth anything there.

**Defaults.** `chunk_bytes` defaults to 64 KiB and `chain_length` to 1024, so one epoch covers 64 MiB and one signature. Both are node-side and signer-side policy with no governance counterpart, exactly as the delivery interval was: the contract verifies the chain arithmetic the voucher commits to, and never observes what cadence was used during delivery.

**Bounds.** `chunk_bytes` MUST be non-zero. `chain_length` MUST be non-zero and MUST NOT exceed `MAX_CHAIN_LENGTH` (1024). The ceiling bounds the on-chain walk at redemption: a node proves index `k` by hashing forward from the index it last redeemed, so the worst-case walk is one full chain (see [Redemption and Close](#redemption-and-close)). A peer that offers or commits to an out-of-range value commits a protocol error: the message is rejected, not clamped.

**Node sovereignty is unchanged.** A node stops delivering when the unpaid balance reaches its [credit window](#credit-window). The threshold is node-local and needs no wire field, exactly as before.

**Risk per chunk.** The payer's exposure stays at zero: a preimage pays for bytes already delivered. The node's exposure per unpaid chunk is `chunk_bytes` of egress:

| `chunk_bytes` | Floor rate ($0.000001/MB) | Market rate ($0.00001/MB) | Ceiling rate ($0.001/MB) |
| --- | --- | --- | --- |
| 64 KiB (default) | $0.00000006 | $0.0000006 | $0.00006 |
| 1 MiB | $0.000001 | $0.00001 | $0.001 |
| 16 MiB | $0.000016 | $0.00016 | $0.016 |

The default is 64× finer than the 4 MiB interval it replaces, so the amount a payer can withhold falls by the same factor. Even the worst case in the table (16 MiB at ceiling rate) exposes $0.016 — small against the reserve `M` a node keeps against a pool (see [Pool solvency and the refundable floor `M`](#pool-solvency-and-the-refundable-floor-m)).
...
The chunk is the *billing* granularity, not the *delivery* granularity. A delivering node streams within a **credit window**: it keeps sending chunks while the unpaid balance `delivered − paid` stays within `credit_window` bytes, and it pauses a stream only when the next chunk would cross that bound.

**The window is a transport buffer, not an economic mechanism.** Preimages travel in one direction and carry no round trip. The node acknowledges nothing, so payment never blocks delivery and the window exists only to keep bytes in flight while the next preimage crosses the link. This is the change the chain epoch makes: a signed-voucher scheme had to size the window against `RTT + durable-commit latency` or lose the link to idle time once per interval, and that sizing constraint is what made the window an economic parameter with a tuning story. A preimage costs one hash and no acknowledgement, so the window is now sized like any other in-flight buffer.

**Exposure is one-sided.** The payer's exposure stays at zero: a preimage pays for bytes already delivered, so the payer never pays for bytes it has not received. This holds as long as the signer builds an independent chain per lane — a shared `chainRoot` is the one way a payer can give the property away, and it gives away the whole cap (see [Chain-root reuse across lanes](#chain-root-reuse-across-lanes)). The node's credit exposure is exactly the window — unbilled egress already on the wire — bounded there and nowhere else. This is the same bounded-credit shape a node already fronts on the upstream leg of a cache-miss pull (`pull_ahead_bytes`, [ADR 037](037-regional-proxy-warming.md#adr-037-latency-driven-proxy-warming-for-regional-locality)), and the downstream credit is strictly the cheaper of the two: egress it has already served, versus speculative USDC it pays an upstream provider. On the fused cache-miss serve path a single window bounds both quantities at once, since every pulled chunk is forwarded downstream immediately.

**Node-local policy, floored at one chunk.** `credit_window` is node configuration, not a wire or governance parameter — like `chunk_bytes` it has no on-chain counterpart and is never negotiated. It is floored at one chunk so a stream can always make progress (deliver a full chunk, then recoup it). The self-enforcing threshold from [Chunk Cadence](#chunk-cadence) is "pause once the unpaid balance reaches the credit window"; a node can always choose a smaller window, down to one chunk, and pause sooner.

**Takedown latency.** The in-flight takedown check ([ADR 011 § On Blacklist Event](011-content-takedown.md#on-blacklist-event)) runs after each accepted preimage. A takedown that lands mid-stream is therefore first observed within one credit window of egress in the worst case, and within one chunk in steady state. Both are bounded and negligible against the takedown compliance window, so the takedown guarantee is unchanged — and the finer chunk cadence tightens the steady-state figure by the same factor it tightens the withholding bound.

**Defaults.** The window defaults to 8 MiB against a default `chunk_bytes` of 64 KiB, which is 128 chunks of headroom. Window and chunk size move independently because they bound different things: the window bounds credit exposure and how far delivery may run ahead of payment, while `chunk_bytes` bounds the amount a payer can withhold and the per-lane preimage-processing rate. A preimage costs one hash and no durable commit, so the per-lane processing ceiling is no longer a reason to coarsen the chunk. At these values a single stream stays link-bound at any realistic round trip.
...
When multiple streams from the **same signer to the same node** run at once, they share that lane's **single chain epoch** (per `(pool_id, signer, provider)`). Streams to different nodes, or from different signers, are independent lanes. Two rules govern one lane:
...
2. **Preimage routing.** Preimages travel on any active stream in the lane. The node credits them against the lane-wide chain index regardless of which stream carries the message.

**Example:** A signer has stream A (blob X) and stream B (blob Y) on the same lane at a 64 KiB chunk size. After receiving 64 KiB in total (for example 44 KiB from A and 20 KiB from B), the signer releases the next preimage on either stream.

`chunk_bytes` is a lane property fixed by the epoch's voucher ([Chunk Cadence](#chunk-cadence)), so a stream that joins mid-epoch inherits it. There is no per-stream cadence to reconcile, no lane-wide minimum to compute, and no obligation to flush when a stream joins. The signer needs one chain-index counter per lane, not per-stream payment logic.
...
4. **Verify the chain and resolve the claim.** Require `chainIndex <= voucher.chainLength` and `voucher.chainLength <= MAX_CHAIN_LENGTH`. Apply `keccak256` to `preimage` exactly `chainIndex` times and require the result to equal `voucher.chainRoot`; revert `BadPreimage` otherwise. The resolved claim is then `claimed = voucher.cumulative + chainIndex × voucher.chunkPrice` over `claimedBytes = voucher.bytesDelivered + chainIndex × voucher.chunkBytes`. A `chainIndex` of `0` is legal and settles the epoch base alone.
5. **Compute the payable amount.** With `w = watermark[poolId][signer][provider]`: `desired = claimed − w.amount` (the still-unpaid portion this voucher and preimage authorize), `capRoom = cap − spent`, and `remaining = deposit − totalRedeemed`. Then `paid = min(desired, capRoom, remaining)`. The floor check ([Rate-floor enforcement](#rate-floor-enforcement)) applies to `claimed` / `claimedBytes` here.
6. **Pay, or revert if nothing is payable.** If `paid == 0`, revert `NothingToRedeem` — the call writes **no** state and moves no funds, so the node simply retries later (after a top-up, or with a higher preimage). Otherwise: `bytesDelta = claimedBytes − w.bytesDelivered`; `bytesPaid = mulDiv(bytesDelta, paid, desired)` (the **paid-proportional** byte count — `== bytesDelta` when `paid == desired`); advance `w.amount += paid`, `w.bytesDelivered += bytesPaid`, `spent += paid`, `totalRedeemed += paid`; `safeTransfer` `paid` USDC to the `FeeRouter`; then call `FeeRouter.routeSettlement(provider, bytesPaid, paid)`.

**The chain walk is bounded by construction.** The contract always hashes forward from the submitted preimage to `chainRoot`, never from a stored intermediate, so it stores no chain state and needs no root-matching branch. The cost is `chainIndex` hashes, capped at `MAX_CHAIN_LENGTH` (1024). Making the chain longer to amortize signatures further would raise this walk linearly, which is what fixes the ceiling at 1024 rather than at an arbitrary value.

**The walk is not free, and the implementation decides how expensive it is.** Measured against the current `PaymentChannel` on this repo's Foundry setup (`solc` 0.8.28, optimizer on, 200 runs):
...
**Epoch rollover is safe because payment is cumulative.** A signer ends an epoch by issuing the next voucher with `cumulative` set to the frontier the previous epoch reached, and a fresh `chainRoot`. The node accepts it only if that base is at least what the lane has already earned. No sequencing rule is needed on-chain: if a node redeems the old voucher and then the new one, the new one's `desired = claimed − w.amount` is already net of the old payment, and a node that redeems only the new voucher collects the same total. Redeeming a superseded epoch after a newer one simply computes `paid == 0` and reverts `NothingToRedeem`.

**The rate floor binds the chunk, not only the total.** A node MUST reject a voucher whose `chunkPrice / chunkBytes` implies a rate below the live delivery floor, because every index step would then drag the epoch's effective rate down and the redemption would revert once the base no longer compensates. Checking the increment at accept time turns an unredeemable epoch into an immediate, diagnosable rejection instead of a surprise at settlement.

**Partial redemption is retry-safe.** The lane watermark tracks the cumulative amount **actually paid**, not the claim the node presented. When a drained pool covers only part of `desired`, `w.amount` advances by just `paid`, so re-presenting the *same* voucher and preimage after the owner tops up collects the remainder — `desired = claimed − w.amount` is still positive. A node never loses an over-committed claim to a transient drain; it loses value only if the owner closes and reclaims without ever topping up, bounded by the reserve `M` the node keeps against the pool ([Pool solvency and the refundable floor `M`](#pool-solvency-and-the-refundable-floor-m)). Because payment is `claimed − paid`, replay is automatic: an already-paid or stale claim — a lower `chainIndex`, or a superseded epoch — computes `paid == 0` and reverts, moving nothing. `min(desired, capRoom, remaining)` is the solvency backstop — the pool never goes negative, and no signer can cause more than `cap` USDC to leave the pool, since `spent` is the cumulative USDC actually paid on its behalf and `paid` is capped by `cap − spent`.
...
**Batch redemption.** `redeemMany([...])` redeems across many lanes (and pools) in one transaction, each entry applied with the single-`redeem` arithmetic above. An entry that would pay `0` — drained pool, already-paid or stale claim, expired capability, or cap reached — is **skipped, not reverted**, so one empty lane never sinks the batch; it emits one `PoolRedeemed` per paid entry and returns the total paid. Only a structurally-invalid entry — a bad voucher signature, a preimage that does not hash to `chainRoot`, or `provider != msg.sender` — reverts the whole call, since that is caller error, not transient pool state. Each entry carries its own chain walk, so a batch's gas is the sum of its entries' `chainIndex` values; a node batching many lanes at high indices should size the batch against the block gas limit.
...
Each reason below maps to an on-chain `redeem` outcome the node avoids by rejecting early, so the node's acceptance rule and the contract's redemption rule check the same facts: a payment the node accepts is a payment it can redeem. `ChainExhausted` is the one whose on-chain counterpart is an absence rather than a revert — a finished epoch is not invalid, it is simply spent, so the node stops serving instead of refusing something the contract would reject. One further reason, `RateFloorRaised`, is emitted by the handler rather than by this validation path and is documented in [ADR 005 § VoucherRejected semantics](005-protocol.md#voucherrejected-semantics).
...
| `BadChainParams` | `chain_length` is zero or above `MAX_CHAIN_LENGTH`, `chunk_bytes` is zero, or `chunk_price / chunk_bytes` is below the live delivery floor | `redeem` bounds `chainLength` and applies the floor to the resolved claim; an out-of-range epoch is unredeemable from its first index |
| `StaleEpoch` | a new voucher's `cumulative` is below the lane's earned frontier | `redeem` pays `claimed − paid`, so a regressed epoch base pays `0` and reverts `NothingToRedeem` |
| `BadPreimage` | the preimage does not hash to `chain_root` at the claimed index, or the index is not above the lane's current index | `redeem` reverts `BadPreimage` on a chain-walk mismatch, and pays `0` on a non-advancing index |
...
- The preimage is the payment receipt; the BLAKE3 hash is the delivery receipt. Together they provide mutual protection: the signer doesn't release a preimage for bytes that fail hash verification; the node stops delivering if preimages stop arriving
- **The signature cadence and the metering cadence are independent.** One signature opens an epoch of `chain_length` chunks, so the meter can be as fine as the node wants without paying per-chunk cryptography. This is the property that lets the default chunk fall from 4 MiB to 64 KiB, and the amount a payer can withhold falls with it (see [Chunk Cadence](#chunk-cadence))
- **The per-chunk path holds no state.** A preimage proves its own index, so accepting one costs a single hash: no signature, no acknowledgement, no round trip, and nothing to commit durably. Payment stops competing with delivery for the link, and the credit window stops being an economic parameter (see [Credit Window](#credit-window))
- Maximum risk per chunk at the default cadence (64 KiB) is about $0.0000006 at market rate — negligible, and 64× tighter than the interval it replaces (see [Chunk Cadence](#chunk-cadence))
...
- The hash chain is a construction in the money path. It is well studied (Rivest–Shamir PayWord, 1996) and the on-chain half is one bounded `keccak256` loop, but it is more than a signature check and it carries its own audit surface. The chain-walk bound (`MAX_CHAIN_LENGTH`) is a hard ceiling for a reason: without it a redemption's gas is unbounded.
- **Redemption gets more expensive on-chain.** A full-depth walk adds ~99k gas to a ~114k warm redemption — measured, not estimated (see [Redemption and Close](#redemption-and-close)). The off-chain saving is paid for partly in on-chain gas, and the trade only holds because redemption is rare and one call settles every prior epoch. A deployment that wants a cheaper redemption lowers `MAX_CHAIN_LENGTH` and accepts proportionally more signatures.
- The rate is fixed for an epoch. `chunkPrice` and `chunkBytes` are signed into the voucher, so a rate change takes effect at the next epoch rather than the next chunk. This costs the signer one signature and matches the existing rule that the `StreamResponse` rate binds for the stream.
- The node absorbs a sub-chunk remainder at the end of a transfer: a blob that ends mid-chunk yields no preimage for the final partial chunk. The loss is under one `chunk_bytes` (about $0.0000006 at the default), and it is the same class of bounded per-lane loss the node already accepts under [Preimage withholding](#preimage-withholding).
- Off-chain lane state is no longer durably committed before acceptance, so an unclean restart can re-open replay of up to one credit window of already-delivered bytes. The bound equals the credit the node already extends by serving, and the on-chain watermark floors the catastrophic case (see [Off-chain voucher state persistence](#off-chain-voucher-state-persistence)).
...
Client receives bytes but stops releasing preimages, getting content for free up to the last paid chunk.

The self-enforcing stop is sufficient, and the chain epoch tightens it. Maximum loss is one `chunk_bytes` at the lane's cadence: the 64 KiB default at market rate is about $0.0000006, and even a 16 MiB chunk at the ceiling rate is about $0.016. The bound is 64× tighter than the 4 MiB signed-voucher interval it replaces, because a preimage costs one hash instead of a signature and an acknowledgement, so there is no overhead reason to coarsen the chunk. Nodes serving high-value content can set a smaller `chunk_bytes` still.

The stop is per-lane and mechanical. The node pauses the moment the unpaid balance on a lane reaches that lane's [credit window](#credit-window) and waits for the next preimage on the same lane; it never fronts more than one window against one lane. The pause keys on the lane, not the counterparty. A client that stalls or crashes mid-stream and reconnects is treated no differently from a first request: it resumes on the same lane under the same window and pays only for the chunks it releases preimages for, never for the interrupted window. The node's own cost of the interruption — re-reading a held blob, or dropping and later re-pulling an unfinished cache-miss fill — is bounded to that one window either way, and no black mark follows the client into its next request. No history accumulates against a client across connections, and none needs to: a fresh signer from the same party gets its own window and is bounded the same way. The bound is a property of each lane, so a new key buys an attacker nothing it did not already have.

That bound applies to a *funded* lane. A request whose pool cannot cover even the first credit window is refused before the node signs a success `StreamResponse`, so it is never served the free chunk at all — the seller-side pre-flight deposit guard of [ADR 037 § Implementation status](037-regional-proxy-warming.md#implementation-status-856), which fronts both the cache-miss and the direct-serve paths.

#### Voucher withholding (epoch starvation)

Client releases preimages to the end of an epoch but never issues the next voucher, so the node cannot be paid for further bytes.

This is bounded and self-correcting. The node stops serving the lane when the chain is exhausted and returns `ChainExhausted` ([Off-chain Voucher Rejections](#off-chain-voucher-rejections-wire-encoding)). Nothing is lost: every chunk of the finished epoch is already covered by a preimage the node holds and can redeem. The attacker gains no free bytes, only the end of its own service. A signer that wants to keep fetching must open the next epoch, which costs it one signature.

#### Chain-root reuse across lanes

Two different things are called "reuse" here, and they point in opposite directions.

**Presenting one node's voucher at another node achieves nothing.** The root is a signed field of a voucher that names `provider`, and `redeem` requires `provider == msg.sender`. A node holding a voucher that names someone else cannot redeem it, and it rejects the stream with `WrongProvider` rather than serving. A client gains nothing by trying, so this direction is not an attack.

**Sharing one `chainRoot` across two lanes is a client-side footgun, and it costs the client.** A preimage is a bearer token for whatever chain it belongs to — it carries no payee. So if a signer opens node A and node B on the *same* root, every preimage it releases to A is also a valid claim on B's lane. A hostile or merely opportunistic A hands preimage `k` to B, and B redeems at index `k` having delivered a fraction of it. The signer pays for chunks it never received, bounded only by `chainLength × chunkPrice` per extra lane and ultimately by its own `cap`. This is precisely the zero-exposure property (see [Credit Window](#credit-window)) being given away, and no node can detect it: a node sees only its own lane and cannot know another voucher shares its root.

> **Requirement.** A signer MUST derive an independent chain for every `(provider, epoch)` — a fresh seed per lane, not a fresh index into a shared one. Chains for two lanes MUST NOT be derivable from each other. This is the client-side rule that keeps payer exposure at zero; the contract cannot enforce it, because it never sees two vouchers together.
...
EIP-712 typed data over `{poolId, signer, provider, amount, bytesDelivered, chainRoot, chainLength, chunkBytes, chunkPrice}` binds the voucher to a specific pool, signer, and payee. `provider` is what stops one node from redeeming a voucher meant for another, and it is what stops a released preimage from paying at any other node. The EIP-712 domain separator (see [EIP-712 Voucher Signature](#eip-712-voucher-signature)) further binds each voucher to a specific chain and contract deployment, preventing replay across different chains, contract upgrades, or test vs production environments. The contract settles in a single token fixed at deployment, so a voucher can only ever pay in that token — cross-token replay does not arise, and the voucher carries no token field. Resubmission after redemption is neutralized by the cumulative accounting: `redeem` pays `claimed − paid`, so an already-redeemed claim pays `0` and moves no funds. A replayed preimage is neutralized the same way — it resolves to a claim the lane has already been paid.

#### Off-chain voucher state persistence

The on-chain protections in [Replay attack on vouchers](#replay-attack-on-vouchers) constrain only what the contract accepts at redemption. They do not prevent the **delivering node** from re-delivering bytes off-chain for a payment it already honoured: a node that loses its lane state re-accepts an earlier claim the signer resubmits, and serves the bytes again.

**Persistence is best-effort, and the chain is the floor.** The node does not block acceptance on a durable write. Three rules replace the former durable-commit ordering:

1. **Accept on verification; persist asynchronously.** A voucher or preimage advances in-memory lane state as soon as it verifies. The node writes the lane frontier to a buffered local store and flushes it on a **byte bound**: it flushes before the accepted-but-unflushed byte count on a lane would exceed one [credit window](#credit-window). The replay surface after an unclean restart is therefore at most one credit window of re-delivered bytes — the same quantity the node already fronts, knowingly, every time it serves. The bound is in bytes, in the same unit as the credit window, which is what makes that equality exact rather than approximate; bounding by elapsed time instead would let the surface grow with the link rate.
2. **The local store is a lane index and a fast floor, not the source of truth.** On the first claim for a lane the node does not know, it reads `watermark[poolId][signer][provider]` with one `eth_call` and seeds from it. The lane floor is `max(local, chain)`. A lane that was never redeemed has no on-chain record and floors at zero, which is correct: nothing has been paid, so nothing can be replayed for value. Seeding is lazy and per-lane, so a node needs no enumeration — which matters because a pool names no provider and is therefore not node-enumerable ([ADR 016 § Off-Chain Read API](016-contract-interactions.md#off-chain-read-api-client--node-bootstrap)).
3. **A failed persist write is a metric, not a wire rejection.** In-memory state has already advanced and the payment is already honoured, so telling the payer to retry would be wrong. The node records the failure and continues; a store that stays unavailable widens the replay surface, which the node's own monitoring must catch.

**Why the ordering rule was expensive.** Requiring a durable commit before the acknowledgement put an fsync on the acknowledgement path. That forced the credit window to cover `RTT + commit latency`, and forced batching machinery to amortize the commit across several vouchers without ever acknowledging ahead of it. The chain epoch removes the acknowledgement entirely and this section removes the ordering rule, so neither the window nor any batching exists to hide a commit. The remaining local write is an optimization: it keeps the common restart cheap, and the chain bounds the uncommon one.

Storage backend and trait shape are implementation concerns; the Rust implementation exposes a `PoolStateStore` seam in `crates/incentive` with a `redb`-backed persistent implementation in `crates/node`. The protocol fixes only the floor rule and the byte bound above.
...
The contract stores **no chain state**. A redemption carries its own voucher and preimage, and the contract hashes forward from the preimage to the voucher's `chainRoot` to resolve the claim. The chain is therefore a way to compress signatures off-chain, not a new piece of on-chain bookkeeping: the register keeps the same two mappings it would keep without it.
...
> **Terminology:** `poolNonce` is the pool creation counter and the only nonce in the payment path. Vouchers carry none — see [Voucher ordering](#voucher-ordering).
...
| Lifecycle | `redeem(voucher, voucherSig, chainIndex, preimage, capability)` | Provider-only (`voucher.provider == msg.sender`): register the signer on first use (verify the owner `capability`), verify the preimage against `voucher.chainRoot` at `chainIndex`, resolve `claimed = voucher.cumulative + chainIndex × voucher.chunkPrice`, then pay `min(desired, capRoom, remaining)` — the still-unpaid portion, capped by the signer's remaining cap and the pool balance; routes the paid amount through `FeeRouter` same-tx. Reverts `BadPreimage` on a chain-walk mismatch and `NothingToRedeem` if nothing is payable (writes no state, so it is retried). No dispute window. See [Redemption and Close](#redemption-and-close). |
| Lifecycle | `redeemMany(entries[])` | Provider-only: apply `redeem` to each entry; **skip** (do not revert) any entry that would pay `0` — drained pool, stale/already-paid claim, expired capability, or cap reached — so one empty lane never sinks the batch. Reverts only on a bad signature, a bad preimage, or `provider != msg.sender`. Returns the total paid. Gas scales with the sum of the entries' `chainIndex` values. |
| Lifecycle | `closePool(poolId)` | Owner-only: start the grace window so nodes may redeem outstanding claims before reclaim; sets status `Closing`; emits `PoolCloseInitiated`. |
...
| View | `watermark(poolId, signer, provider) → Lane` | Read a lane's paid `{amount, bytesDelivered}`. A node seeds an unknown lane's floor from this on first contact (see [Off-chain voucher state persistence](#off-chain-voucher-state-persistence)). |
| View | `MAX_CHAIN_LENGTH() → uint256` | Ceiling on `voucher.chainLength` (1024), which bounds the redemption chain walk. |
...
bytes32 constant VOUCHER_TYPEHASH = keccak256(
    ...
    "bytes32 chainRoot,uint256 chainLength,uint256 chunkBytes,uint256 chunkPrice)"
);
```

`signer` and `provider` are both in the signed payload: `signer` binds the voucher to the authorized key `redeem` validates against, and `provider` binds it to a single payee, so one node cannot redeem another node's voucher. `amount` and `bytesDelivered` are the epoch's **base** cumulatives — what the lane had earned when the epoch opened — and the chain fields extend them.

The four chain fields are all signed, and each is load-bearing. `chainRoot` commits to the epoch's preimage chain, so releasing `p_k` is an unforgeable claim on `k` chunks. `chainLength` bounds the epoch and therefore bounds the redemption walk. `chunkBytes` and `chunkPrice` fix what one index step is worth in bytes and in USDC; without them in the signed payload a node could reinterpret the same released preimages at a higher price. Binding `chainRoot` to `provider` stops a *voucher* from being redeemed by any node but the one the epoch was opened with. It does not by itself confine the *preimages*: a preimage names no payee, so confining it depends on the signer building an independent chain per lane, which is a client-side requirement the contract cannot check (see [Chain-root reuse across lanes](#chain-root-reuse-across-lanes)).

The voucher carries no epoch identifier for fee routing: per-operator-epoch attribution is derived by `FeeRouter.routeSettlement` at redemption time as `epoch = uint64(block.timestamp / EPOCH_LENGTH)`, so a redemption's bytes credit the epoch its transaction lands in. A node redeems whenever it chooses within the capability `expiry` (the per-signer redemption deadline) and the grace window, so the practical bound on epoch-shifting is that `expiry` — a second-order effect on emission share that shrinks as the active-operator set grows. (That fee-routing epoch is unrelated to the lane's chain epoch; the two never interact.)
...
bytes32 voucherDigest = keccak256(abi.encodePacked(
    ...
    keccak256(abi.encode(VOUCHER_TYPEHASH, poolId, signer, provider, amount, bytesDelivered,
                         chainRoot, chainLength, chunkBytes, chunkPrice))
));
...
Vouchers carry **no nonce**. Ordering works on two axes, and neither needs a sequence field:

- **Across epochs**, the ordering key is the voucher's cumulative base. A node accepts a new epoch only if its base is at least the frontier the lane has already earned, and rejects a regressed base (`StaleEpoch`, [ADR 005](005-protocol.md#adr-005-wire-protocol)). A replayed epoch therefore re-opens nothing the lane has moved past.
- **Within an epoch**, the ordering guard is the chain index, which is self-proving and needs no counter agreement: a preimage either hashes to `chainRoot` at a higher index than the node holds, or it does not.

On-chain redemption is likewise cumulative (`redeem` pays `claimed − paid`), so an already-redeemed or stale claim pays `0`, and the same voucher and preimage can be re-presented after a top-up to collect an earlier drained-pool shortfall — only the unpaid `claimed − paid` remains.
...
```
{poolId, signer, provider, amount, bytesDelivered,
 chainRoot, chainLength, chunkBytes, chunkPrice, signature}
```

During delivery over `cdn/client/v1`, `{signature, amount, chainRoot, chainLength, chunkBytes, chunkPrice}` are transmitted on the wire; the remaining fields are derived from stream context — `poolId` from the `StreamRequest`, `signer` the bound client key, `provider` the delivering node, and `bytesDelivered` the node's per-lane cumulative byte counter. The chain fields must be explicit because the node cannot derive them: they are the signer's commitment to what one index step is worth. See [ADR 005](005-protocol.md#adr-005-wire-protocol) for wire protocol details.
...
- **Cumulative, monotonic per lane.** Like `amount`, `bytesDelivered` is strictly non-decreasing across epochs within a `(poolId, signer, provider)` lane. `redeem` computes the byte delta against the lane's last redeemed cumulative.
- **Extended by the chain index, not re-derived.** The voucher's `bytesDelivered` is the epoch base; the resolved claim is `bytesDelivered + chainIndex × chunkBytes` ([Redemption and Close](#redemption-and-close)). Within an epoch the byte count therefore follows the released preimages exactly, with no separate attestation to keep in step. The signer sizes `chunkPrice` against `rate_per_mb` as `chunkPrice = ⌈chunkBytes × rate_per_mb / 1_048_576⌉` (1 MB = 1,048,576 bytes per [ADR 005](005-protocol.md#adr-005-wire-protocol)), and the node rejects an epoch whose implied rate is below the live floor (see [Chunk Cadence](#chunk-cadence)).
