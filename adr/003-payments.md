...
- **Fan-out.** `M` reserves against a maximum *aggregate* bandwidth `B`; a pool drawn wider or faster than `B` can accrue in-flight past the reserve before nodes stop, reopening the tail. Bounded by **rate-anomaly clamping** (below); not driven to zero by `M` alone.
...
**How rate-anomaly clamping works.** A node cannot see other lanes' *unredeemed* in-flight — but it does not need to, because **redemptions are public**. Every `PoolRedeemed` is indexed by `poolId`, so any node watching a pool sums all lanes' redemptions to observe the pool's *aggregate* draw — `d(totalRedeemed)/dt ÷ price ≈ aggregate served bandwidth` across every node serving it — alongside the public `remaining` balance. When that observed aggregate exceeds the `B` the pool's `M` is sized for, the node refuses to keep serving it. Because every node applies the same test to the same public signal, they clamp **together** without any one of them knowing the fan-out `N`; they react to the shared `totalRedeemed` rate, not to a count of peers. Two things make the signal trustworthy: an honest node serving the pool *will* redeem (it wants its money), so its draw shows up on-chain — the only draw a party can hide is service by its **own** nodes to itself, which is not a free-ride; and the redemption stream trails actual serving by at most one threshold `t` per lane, kept small on unproven pools so the anomaly surfaces fast (a declared fan-out hint lets nodes agree on `B` before the first redemptions land). The residual gap is exactly that lag: an attacker who ramps aggregate bandwidth past `B` within one detection delay `Δ` overshoots `M` — but only by physically pulling `B·Δ` of real bytes at real egress cost, a bound independent of fan-out width.
...
- Maximum risk per voucher interval at default cadence (1 MB) is $0.00001 at market rate — negligible. At the wire ceiling (1024 MB) and ceiling rate ($0.001/MB), worst-case risk is $1.024 per interval — small against the reserve `M` a node keeps against a pool (see [Voucher Interval Negotiation](#voucher-interval-negotiation))
...
- The tail is structurally zero for bounded fan-out: the reserved `M` covers all in-flight vouchers. It reopens only past fan-out `N = M/t`, bounded by rate-anomaly clamping; self-dealing is separately bounded by `M` and taxed by the FeeRouter cut.
