...
**`chunk_price` is `rate_per_mb`.** `CHUNK_BYTES` equals `BYTES_PER_MB`, so a chunk costs exactly what the node advertises per MB. The payer signs that price into the voucher, which fixes it for the chain that voucher opens. The chain therefore introduces no second price unit, and its payability floor is the `deliveryFloor` the contract already enforces ([Rate-floor enforcement](#rate-floor-enforcement)).
...
A closing voucher sets `chain_root = 0` and redeems at `chain_index = 0` with a zero preimage. Those zeros satisfy the ordinary `keccak^k(preimage) == chain_root` check. At any higher index the same check needs a value that hashes to `0`, and keccak preimage resistance makes that infeasible. The voucher is therefore **sealed at its `amount`**, and nothing can redeem more against it.

A real root is `keccak^255(seed)` and is never `0`, so `0` is a safe sentinel. The schema keeps one fixed shape, the field stays mandatory, and the contract gains no branch.
...
`chunk_bytes` is three things at once: the price quantum, the meter resolution, and the bound on what a payer can withhold. USDC's 6 decimals put a hard floor under it. A chunk that prices below one base unit ($0.000001) rounds to zero, and a meter that ticks zero cannot charge.

| `chunk_bytes` | At the $0.01/GB target | At the $0.001/GB floor | Rounding per tick | Verdict |
| --- | --- | --- | --- | --- |
| 100 KB | 1 base unit | 0 base units | one whole unit | unpayable at the floor rate |
| **1 MiB** | **10 base units** | **1 base unit** | **~10%** | **chosen** |
| 10 MiB | 100 base units | 10 base units | ~1% | payable, but a 10 MiB withholding bound |

**1 MiB sits about ten times above the single-unit floor.** It prices to roughly ten base units at the target rate, and it stays payable at one base unit down to the governance floor. Because `CHUNK_BYTES` equals `BYTES_PER_MB`, the payability floor is the existing `deliveryFloor` ([Rate-floor enforcement](#rate-floor-enforcement)), and the chain adds no constant of its own.

The meter is therefore ~1 MiB-grained. That is four times finer than the 4 MiB voucher interval it replaces, and much coarser than the 1024-byte BLAKE3 leaf. The token sets that limit, not the hash: hashes are cheap enough to tick far finer, but USDC cannot price the tick. A coarser chunk helps only a network that runs near the floor price, and any change breaks every signature made under the old value, so it is an operator and governance decision rather than a knob.

#### One chain per lane

A released preimage is a **bearer proof**. It names no payee. Its binding to a payee comes entirely from the voucher whose `chain_root` it satisfies. Three rules follow:
...
- A node **MUST** reject a `chain_root` it has already seen on another lane, or already retired on this one. A root is one-shot per lane: a new root supersedes the old one, and the retired chain's index stops advancing.

The reason is direct. One root in two vouchers makes every preimage released to the first provider a valid extension of the second provider's voucher. The second node then claims chunks it never delivered, and the payer pays twice for one tick. Per-lane watermarks bound each claim independently, but they cannot see the collision: each claim is valid under its own signature, and the contract never holds the two together. The rule is therefore payer-side. Its adversarial form is in [Cross-lane preimage spend](#cross-lane-preimage-spend).
...
**The wire sets the bound, not gas.** A released preimage travels as `preimage ‖ index`: 32 bytes and a one-byte index, 33 bytes flat, with no length prefix and no varint ([ADR 005 § Payment quantum and credit window](005-protocol.md#payment-quantum-and-credit-window)). 255 is the longest chain whose releasable indices fit that byte with no offset. A 1024-chain amortizes signatures four times further, at the cost of a wider tag and a proportionally longer worst-case walk. 255 buys a fixed-width message and a walk capped at 255 hashes.

**The byte is the index.** `chain_root = keccak^255(s)`, and the releasable preimages are indices `1..=255`. The wire byte carries the index as itself: no offset on send, no increment on receipt, and no place for an off-by-one to live. Index `0` is the settlement case at redemption and never travels the wire.

**Rollover.** When a lane exhausts its chain, the payer rolls it. It signs a new voucher whose `amount` folds in the exhausted chain's full value (`amount_old + 255 × chunk_price`), whose `bytes_delivered` folds in `255 × CHUNK_BYTES`, and whose `chain_root` is a fresh independent root. The accepted index resets to 0. One signature per 255 MiB keeps signatures O(1) per transfer at any transfer size. A node MUST reject an index above the bound rather than hold a proof the contract refuses to walk.
...
**The window ramps with the stream's own payment.** The credit window for a stream is `min(credit_max, max(chunk_bytes, paid / credit_ramp_divisor))`, where `paid` is that stream's own cumulative confirmed payment. A stream starts at the floor of one chunk and grows its window in proportion to `paid` as it pays, up to the `credit_max` ceiling. A stream that never pays stays pinned at the floor; nothing about the window depends on any other stream, signer, or the node's history with the counterparty.
...
- **Fan-out.** `M` gives a zero tail only up to fan-out `N = M/t`. A pool fanned wider than the `M` it reserves reopens a bounded tail. We accept this residual rather than build machinery against it: each node's exposure to a fresh pool is only its own small redemption threshold `t`. Enlarging the aggregate tail forces the attacker to fan across proportionally many nodes and to physically pull real bytes from each — a real egress cost. Self-dealing is separately taxed by the FeeRouter cut. Sizing `M` for the expected fan-out closes this residual in the common case: honest clients are sticky and do not fan a brand-new pool across hundreds of nodes at once. A node also bounds a second, separate tail: the **un-vouchered floor** bytes it fronts across every stream on one pool before that stream's first voucher lands. A per-pool floor-credit accumulator tracks this live, un-vouchered exposure and refuses new admissions once it reaches the pool's own `remaining − M`. The accumulator persists durably across a restart, so a crash never resets it. On one node, aggregate un-vouchered floor exposure against one pool therefore never exceeds `remaining − M`, no matter how many concurrent or sequential streams draw on it. The residual that survives is **cross-node fan-out**: each node bounds only its own exposure to the shared `remaining − M`. `K` uncoordinated nodes serving the same pool can together reach up to `K × (remaining − M)` — the same shape of residual this ADR already accepts for the vouchered tail above. Closing it fully needs cross-node coordination the protocol does not have. A small additional residual survives a crash inside a lane's idle window, before stream-end reconcile persists its charge. It is bounded by the count of lanes withholding at the moment of the crash, times one floor — far smaller than the fan-out residual it sits beside. A stream that ends abnormally — a client disconnect, a rejected voucher, or a mid-stream stop — charges its **full reserved floor**, not the proportional bytes it happened to take. This conservative direction is what keeps the sequential bound tight: a cache-miss fill aborted before a single byte reaches the client still charges a floor, so an attacker who fronts the node's upstream USDC and then disconnects cannot repeat the pattern for free. The cost of the conservative choice is that an honest client that disconnects inside its first chunk also charges a full floor. `dead_charge` only grows; it clears only when the pool is reclaimed on-chain. A long-lived pool whose clients occasionally disconnect therefore accrues a permanent per-node floor charge and is eventually refused despite a healthy on-chain deposit — the owner tops the pool up to restore its budget, exactly as for the un-vouchered floor bound itself.
...
2. **One chain, one index, shared by every stream.** `chain_root` and the chain index are lane-wide state, not per-stream. A stream that joins a live lane continues the running chain and its index. It MUST NOT open a second root ([One chain per lane](#one-chain-per-lane)).
...
**Rule 1 — the deepest preimage wins.** The node keeps the deepest preimage it has verified (`tip`) and that preimage's index (`verified`). On a reveal at `index`:

- `index ≤ verified` — already covered. The node ignores it and hashes nothing.
- `index > MAX_CHAIN_LENGTH` — past the end of the chain. The node rejects it before it walks anything, so an over-long skip costs no hashes.
- otherwise — the node accepts it when `keccak^(index − verified)(preimage) == tip`, then adopts it as the new `tip`.

Placement is therefore by index, never by arrival order, and a fast stream may skip indexes a slower one has not yet reached. Within one chain epoch, cross-stream interleaving is free.

**Rule 2 — each stream anchors its own epoch.** The `chain_root` a preimage verifies against arrives in a signed voucher, and `index` is one byte. A bare reveal therefore does not name its own chain. On a single sequential stream the voucher naturally precedes its chunks, but concurrent streams break that: a fast stream can carry a reveal for a **new** chain before the voucher announcing that chain lands anywhere. The node cannot verify it and rejects it. QUIC gives per-stream ordering, so the fix is per-stream:

> **On each stream, the epoch's `chain_root` voucher precedes that stream's preimages for that epoch.**

The payer sends the current root voucher once at stream open, and once again on each rollover — not before every preimage, which would delete the point of a signature-free tick. The voucher is cumulative and signed, so the node keeps the maximum: resends are idempotent and dedup is free. A stream that joins late prefixes the *current* root voucher before its first preimage, under the same rule, and a stream may skip a resend of a voucher the node already holds.

This keeps one shared chain and one global counter. Per-stream chains would multiply rollovers and therefore signatures, which is the cost the chain exists to avoid. The payer is one process, so it serializes the counter and the rollover decision under a local lock while bytes stream concurrently. The node needs no rendezvous.
...
| `ChainIndexTooLarge` | `index > MAX_CHAIN_LENGTH`, or `index == 0` on the wire | redemption reverts `ChainIndexTooLarge`; the node rejects before it walks anything |
...
| `ChainRootReuse` | the voucher carries a `chain_root` the node has seen on another lane, or already retired on this one | none on-chain — a payer-side discipline failure the node refuses locally (see [One chain per lane](#one-chain-per-lane)) |
...
- A released preimage is a **bearer proof**: it names no payee, so a payer that reuses one `chain_root` across lanes pays twice for one tick. The defence is a payer-side MUST (an independent seed per lane) plus node-side root-reuse rejection, not a contract check — the contract never sees two vouchers together (see [Cross-lane preimage spend](#cross-lane-preimage-spend))
- The signed voucher carries two more fields (`chainRoot`, `chunkPrice`) and redemption carries two more parameters (`chainIndex`, `preimage`), widening a lane from 160 to 288 static calldata bytes. On the common closing path those bytes are zeros and compress to almost nothing, so a cooperative redemption stays near the baseline; the bounded walk is confined to mid-chain abandonment
...
The self-enforcing stop is sufficient. Maximum loss is one chunk (1 MiB): at market rate (~$0.00001) it is negligible, and at the ceiling rate (~$0.001) it is still negligible relative to pool deposits. The bound is four times tighter than the 4 MiB voucher interval it replaces, because the meter now ticks per chunk. A node cannot pause below one chunk — the credit window is floored there — so the chunk is the withholding bound as well as the price quantum.
...
The per-lane bound is one chunk, but the maximum **free floor** a node ever fronts against one pool is bounded per pool, not per lane: a per-pool floor-credit accumulator caps the sum of every stream's un-vouchered exposure against that pool to `remaining − M`, so opening many lanes against the same under-funded pool does not multiply the free bytes an attacker collects (see [Pool solvency and the refundable floor `M`](#pool-solvency-and-the-refundable-floor-m)).
...
**The node-side backstop is root-reuse rejection.** A node MUST reject a `chain_root` it has already seen on another lane, or already retired on this one (`ChainRootReuse`). That catches the part the node can see and keeps its own ledger one-shot per root. It needs no on-chain counterpart: a replayed root is already neutralized by `claimed − paid`, so the guard exists to keep the node's off-chain ledger consistent, exactly as `AmountRegression` does.
...
A node holds each lane's voucher watermark in memory. The watermark is `(last_amount, last_bytes_delivered, chain_root, highest_index)`, tracked per `(poolId, signer, provider)` lane. The last two fields are the chain state: the root the lane is currently metering against, and the deepest index the node has verified under it. The node MUST advance a lane's watermark as soon as it accepts a voucher or a preimage for that lane, before it delivers any further bytes for the lane. An absent entry is semantically identical to a never-seen lane (`last_amount == 0`). A record exists once the node has accepted the lane's first voucher. The node drops an entry only after it redeems the lane's full cumulative and observes the redemption on-chain.
...
**Every USDC counter and both lane watermarks are `uint64`.** At USDC's six decimals that ceiling is about $18.4 trillion, roughly four hundred times the token's entire supply, and 18.4 exabytes for a byte count — so the width bounds nothing a real pool reaches, while each struct fits in a single storage slot. The values are bounded by construction rather than by a check: `spent <= cap`, `totalRedeemed <= deposit`, and both watermarks only ever advance toward a presented `uint64` cumulative. The EIP-712 typehashes still declare `uint256`, because a narrower field hashes to the same 32-byte word — so the widths change no signature a client produces.
...
- **Claim resolution.** Require `chainIndex <= MAX_CHAIN_LENGTH`, hash `preimage` forward `chainIndex` times, and require the result to equal `chainRoot` (revert `BadPreimage`). Then `claimed = cumulative + chainIndex × chunkPrice` over `claimedBytes = bytesDelivered + chainIndex × CHUNK_BYTES`, and the [rate floor](#rate-floor-enforcement) applies to that pair. A sealed voucher (`chainRoot = 0`, `chainIndex = 0`, zero preimage) resolves to exactly `cumulative` through the same check. Redemption is cumulative; the voucher carries no nonce.
...
Redemption enforces the per-byte floor on the voucher's **cumulative** `amount` and `bytesDelivered` before advancing the lane watermark:

```
require:  amount * BYTES_PER_MB >= bytesDelivered * deliveryFloor      (BYTES_PER_MB = 1_048_576, ADR 005)
```

evaluated as `bytesDelivered <= Math.mulDiv(amount, BYTES_PER_MB, deliveryFloor)`. Both operands are `uint64`, so the widened product cannot overflow, and a byte count wider than the field is not expressible on the wire at all. Because `deliveryFloor >= 1` the divisor is non-zero. The check runs on the cumulative `amount` / `bytesDelivered`, and uses **zero tolerance** — the floor sits 10× below the expected market rate, so honest traffic clears it by ≥10× and needs no rounding headroom (the off-chain 1% tolerance applies to the *advertised* `rate_per_mb`, not this floor).
...
Two 32-byte words instead of a `bytes` blob is what makes `LaneVoucher` a **static** ABI struct: an array of it carries no per-element offset, no length word and no padding. PayWord adds four fields to that struct — `chainRoot`, `chunkPrice`, `chainIndex`, and `preimage` — so a lane costs 288 static calldata bytes instead of 160, and the struct stays static. The added width is mostly zero bytes on the common path (a closing voucher carries a zero root and a zero preimage), which is what keeps a cooperative redemption at roughly the pre-PayWord cost on a compressing L2 — see [Chain walk and redemption cost](#chain-walk-and-redemption-cost). Recovery also skips the `EXTCODESIZE` an ERC-1271 check would do on a cold signer address.
...
- **Extended by the chain at redemption.** A claim resolves over `claimedBytes = bytesDelivered + chainIndex × CHUNK_BYTES`, and the rate floor applies to the extended pair, so bytes proved by preimage carry the same per-byte price obligation as bytes proved by signature ([Redemption and Close](#redemption-and-close)).
- **Routed at redemption.** Forwarded as the paid-proportional byte count (`bytesPaid`) to `FeeRouter.routeSettlement` on each redemption; a fully-paid draw forwards the whole lane byte delta.
...
The router does not validate `bytesDelivered` against any oracle of physical delivery — the value is whatever the signer signed. The defense is twofold. **Structurally**, per-byte revenue requires real client USDC inflow rather than self-attested byte counts (a redemption pays only what the pool holds, and forwards paid-proportional bytes). **Quantitatively**, redemption enforces the `deliveryFloor` per-byte price floor (see [Rate-floor enforcement](#rate-floor-enforcement)), so a voucher cannot decouple a large `bytesDelivered` from a tiny `amount` — claiming `B` bytes costs `>= B / 1_048_576` base units. Without the floor, the pool balance alone would be insufficient: a signer could stamp arbitrarily many bytes at `amount = 1`. Governance vote weight is sourced from the same floor-bound per-byte counter ([ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight)), so both wash-trade revenue and vote-buying are bound by proportional real USDC.
