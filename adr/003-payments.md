...
The per-lane bound is one chunk, but the maximum **free floor** a node ever fronts against one pool is bounded per pool, not per lane: a floor-credit accumulator caps the sum of every stream's un-vouchered exposure against that pool to `remaining − M`, and caps any one signer's share of that sum, so opening many lanes against the same under-funded pool does not multiply the free bytes an attacker collects, nor let one signer take the whole pool's free budget from its co-tenants (see [Pool solvency and the refundable floor `M`](#pool-solvency-and-the-refundable-floor-m)).
