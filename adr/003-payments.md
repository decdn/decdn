...
**The index type sets the bound, not gas.** The chain index travels in a single byte, and 255 is the longest chain whose releasable indices fit that byte with no offset. The message body is `preimage ‖ index` — 32 bytes plus that one — and the frame around it adds the varint length prefix and message discriminant of [ADR 013](013-schema-evolution.md#adr-013-schema-evolution) like every other frame ([ADR 005 § Payment quantum and credit window](005-protocol.md#payment-quantum-and-credit-window)); the bound comes from the index type, not from the body size. A 1024-chain amortizes signatures four times further, at the cost of a wider tag and a proportionally longer worst-case walk. 255 gives a fixed-width message and a walk capped at 255 hashes.

**The byte is the index.** `chain_root = keccak^255(s)`, and the releasable preimages are indices `1..=255`. The wire byte carries the index as itself. There is no offset on send and no increment on receipt, so no off-by-one is possible. Index `0` is the settlement case at redemption and never travels the wire.

**Rollover.** A payer rolls a chain when it exhausts it, and may roll earlier — `ChainIndexTooLarge` and `RateFloorRaised` both force one mid-chain. The fold is the same either way and is stated in terms of the frontier actually reached, never a flat 255: the new voucher's `amount` is `amount_old + verified_index × chunk_price` and its `bytes_delivered` is `bytes_old + verified_index × CHUNK_BYTES`, with a fresh independent `chain_root`. The index resets to 0.
...
| `ChainIndexTooLarge` | `index == 0` on the wire — index 0 is the redeem-time settlement case and carries no chunk | none on-chain — `chainIndex = 0` is the *normal* settlement path and never reverts. The wire index is a `u8`, so `index > MAX_CHAIN_LENGTH` cannot be encoded and needs no runtime check |
...
- The signed voucher carries two more fields (`chainRoot`, `chunkPrice`) and redemption carries two more parameters (`chainIndex`, `preimage`), widening a lane from 5 words to 9. On the common closing path three of the four are zero and `chunkPrice` is zero too on a sealed voucher, so the addition compresses to almost nothing and a cooperative redemption stays near the baseline; the bounded walk is confined to mid-chain abandonment
...
- **Claim resolution.** Require `chainIndex <= MAX_CHAIN_LENGTH`, hash `preimage` forward `chainIndex` times, and require the result to equal `chainRoot` (revert `BadPreimage`). The bound is a real check here, not on the wire: calldata carries `chainIndex` in a full word, so a caller can present a value the one-byte wire index could never encode. Then `claimed = cumulative + chainIndex × chunkPrice` over `claimedBytes = bytesDelivered + chainIndex × CHUNK_BYTES`, and the [rate floor](#rate-floor-enforcement) applies to that pair. A sealed voucher (`chainRoot = 0`, `chainIndex = 0`, zero preimage) resolves to exactly `cumulative` through the same check. Redemption is cumulative; the voucher carries no nonce.
