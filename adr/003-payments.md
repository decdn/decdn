...
- `settleChannel` → requires status `Closing` and `block.timestamp >= disputeDeadline`. Callable by any address. Refunds `deposit - claimedAmount` to the client and, if `claimedAmount > 0`, transfers `claimedAmount` USDC to the configured `FeeRouter` and invokes `FeeRouter.routeSettlement(channel.provider, channel.client, claimedBytes, claimedAmount, voucher.epochId)` in the same transaction. Sets status to `Closed`, emits `ChannelSettled`. **No fee is computed or skimmed inside this contract** — the router applies the [ADR 026](026-gauge-boost-tokenomics.md) six-bucket split and pays the operator's 40% base share same-tx (see [FeeRouter Integration](#feerouter-integration)). Distinct-client diversity gating runs inline in `routeSettlement` per [ADR 027 §1](027-distinct-client-receipts.md#1-feerouter-per-epoch-bookkeeping); the per-(operator, epoch) `distinctClientCount` and `bytesPerEpoch` views are populated as a side effect of this call.
...
`StablePaymentChannel.settleChannel` MUST invoke `FeeRouter.routeSettlement(address operator, address client, uint256 bytesDelivered, uint256 amount, uint64 epochId)` in the same transaction as the USDC `safeTransferFrom` to the router. The router pays the operator's 40% base share in that transaction, dispatches the 5% / 5% / 3% same-tx legs, and updates the per-(operator, epoch) `distinctClientCount` and `bytesPerEpoch` views inline per [ADR 027 §1](027-distinct-client-receipts.md#1-feerouter-per-epoch-bookkeeping). The full `IFeeRouter` interface (claim paths, epoch views, diversity-gating views) is canonical in [ADR 016](016-contract-interactions.md).
...
The `epochId` field is consumed by `FeeRouter.routeSettlement` for distinct-client diversity bucketing per [ADR 027 §2](027-distinct-client-receipts.md#2-epoch-attribution); the client sets it at signing time. `FeeRouter` validates that `epochId` is current or recent (within `MAX_EPOCH_LAG`, default 4 epochs) and rejects future-dated values.
...
`bytesDelivered` is a cumulative byte count signed alongside `amount` and `nonce`. It is the canonical settlement-record byte count carried in the `Voucher`, forwarded to `FeeRouter.routeSettlement`, and aggregated into `bytesPerEpoch[operator][epochId]` for gauge eligibility per [ADR 027 §1](027-distinct-client-receipts.md#1-feerouter-per-epoch-bookkeeping). The [ADR 026 §3](026-gauge-boost-tokenomics.md#3-gauge-boost-formula) gauge formula's `bytes_i` reads `bytesPerEpoch` directly. Properties:
