...
**No per-relationship rate ramp — trust never gates delivery rate.** The security bound is the reserved floor `M`, not a per-relationship earned window. A node does **not** throttle a pool based on its history with that pool. A first-time pool and an established pool follow the same rules. Each stream ramps its own [credit window](#credit-window) from the floor of one chunk as that stream pays. The ramp resets on every new stream; it never carries over from the pool's or signer's past. Admission is the separate question: the per-signer abandonment bucket does read a signer's recent abandonment, so a burst can refuse a stream, but it never slows one that is admitted. A fresh stream and an established stream both start at the floor and ramp identically. Trust affects only redemption *cadence* (`t`, a gas concern), never delivery rate. The only caveat is fan-out *width*, not per-stream rate: a fresh pool spread across very many nodes at once can reopen the bounded tail below. We accept this residual; it is never a reason to slow any single stream. This removes the cold-start penalty of a dynamic per-relationship window, which throttled every fresh `(node, pool)` pair until it earned history.

**Per-signer floor isolation.** Two separate concerns govern the floor, and admission checks both against the signer, because minting a signer identity is free while opening a pool costs money — so every behavioral defense keys to the signer, and the pool is the money envelope over all of them.

The first concern is **live concurrency**: the un-vouchered floor a signer's in-flight streams hold right now, delivered ahead of payment and released as each stream pays. The per-pool ceiling bounds the sum of live floor across every signer to `remaining − M` — the hard solvency envelope, and the one bound a free-to-mint signer cannot evade, because an owner spraying fresh signers still sums under it. Under it, one signer's live floor is capped at an **absolute** `k · one window`. This carries no memory: it is released on payment, so it never penalizes a signer for quitting.

The second concern is **abandonment rate**, and it is *not* a permanent tally. Abandonment is normal — a viewer stops a video, a download is cancelled, a laptop sleeps — so a single abandon must cost nothing lasting. Each `(node, signer)` pair holds a refilling **leaky-bucket allowance**: a stream that abandons debits the bucket by the REAL un-recouped floor it left behind (`delivered − paid` for a direct-serve hit; the reserved window for a cache-miss fill, whose fronted upstream USDC the downstream tail under-measures), and the bucket refills over time. Occasional abandonment never drains it; only a *burst* does, and a drained bucket soft-throttles that one signer on that one node until it refills. It is node-local and signer-isolated: it never rolls into the pool total, never touches a co-tenant, and the throttled signer just pulls from another node. The contract's per-signer `spending_cap` does neither job: it bounds *settled* spend, and the floor is by definition un-vouchered. All of this gates admission on un-vouchered exposure, not delivery rate: an admitted stream ramps identically whatever its signer's history.

**Recovery is refill, not rotation.** The abandonment bucket refills on a wall-clock rate, so an honest but flaky signer recovers on its own with no key rotation, no decay schedule the node has to run, and no split live/dead budgets. A burst-abandoner waits out the refill or moves to another node. Rotation still works — the node keys the bucket by signer, so a fresh session key starts with an empty bucket — but it is a convenience, not the recovery path. Because nothing is permanent, a long-lived shared pool no longer accretes a floor tax from ordinary disconnects, which is what a real "many viewers, most of whom stop mid-stream" workload produces.
...
| `PoolExhausted` | the pool's remaining deposit, minus the refundable floor `M` and already-committed concurrent floor credit across every signer, can no longer fund further credit | none on-chain — a node-side mid-stream solvency stop, not a voucher defect. The per-signer gates (live cap and abandonment bucket) never report this reason: they refuse at admission, before the node signs a success response, as a wire-level `NotFound` |
