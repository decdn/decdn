...
- `reclaimExpired` → requires status `Open` and `block.timestamp >= expiresAt`. Returns the full deposit to the client (no fee deducted — no voucher was submitted). Sets status to `Closed`, emits `ChannelExpiredReclaimed`. Callable by the client or the provider. Regardless of caller, the full deposit is returned to `channel.client` — the provider cannot claim funds via this path. This ensures abandoned channels where the client is absent can be cleaned up by the provider to free on-chain state. **Status interaction note:** `reclaimExpired` is deliberately unavailable for channels in status `Closing`. Once `closeChannel` is called, the channel enters the dispute/settle flow governed by `disputeDeadline`, regardless of whether `expiresAt` has also passed. This prevents a scenario where a party bypasses the dispute window by waiting for `expiresAt` instead of `disputeDeadline`. Implementers MUST enforce `require(channel.status == ChannelStatus.Open)` in `reclaimExpired` and MUST NOT add an `expiresAt` fallback path for `Closing` channels.
