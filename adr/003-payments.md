...
`chain_length` is not a voucher field. The chain index is a `u8`, so the index space is `0..=255` — 256 slots, exactly one byte. Index 0 is the root itself and meters nothing, which leaves `1..=255` as the releasable preimages: `MAX_CHAIN_LENGTH = 255` is the highest index, and at 1 MiB per chunk one chain meters 255 MiB.
