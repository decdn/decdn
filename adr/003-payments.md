...
**Signer authority.** `cooperativeClose` is the one lifecycle entry point the pinned `voucherSigner` may call. Its authority there is bounded but not zero: the call also requires `providerCloseSig`, verified against `ch.provider`, so a compromised signing key cannot force settlement on its own — but combined with a provider waiver it can force settlement at the current watermark. It redirects no funds: the refund still pays `ch.client` and the settlement still pays `ch.provider`. The asymmetry elsewhere is deliberate — the signer is **not** a party to `closeChannel` or `closeChannelWithoutVoucher` (client or provider only), to `topUp` (client only), or to `withdraw` (provider only). The delegate's on-chain authority is confined to what it can already exercise by signing a voucher.
...
// `voucherSigner` takes a fourth slot of its own: every packed slot is already
// at 28 bytes, so a 20-byte address cannot join one.
struct Channel {
