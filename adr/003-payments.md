...
Client reuses one `chain_root` for several nodes, so that preimages released to one node also pay at another.

Not possible. The root is a signed field of a voucher that names `provider`, and `redeem` requires `provider == msg.sender`. A preimage presented at a node the voucher does not name buys nothing, because that node has no voucher binding the root to itself. Reusing a root across two vouchers to two different nodes is equally useless to the attacker: each node can only claim against the voucher it holds, and each voucher's `cumulative` is drawn against the same per-signer `cap`, so the cap bounds the total either way.
