...
1. **Periodic redeem sweep.** A node redeems its outstanding vouchers on a fixed interval, capped well inside the grace window (at most 6 hours against the 48-hour floor), so a pool that closes between sweeps is still swept — many times over — before the owner can `reclaim`. The sweep watches no close event: any interval short of the window is sufficient, and the shorter default keeps earnings flowing regardless of closes. A node does not force sub-threshold dust through the sweep — a lane that never earned enough to justify its own redemption gas is left for the owner to reclaim rather than redeemed at a loss.
