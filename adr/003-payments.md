...
4. **Compute the payable amount.** With `w = watermark[poolId][signer][provider]`: `desired = cumulative − w.amount` (the still-unpaid portion this voucher authorizes), `capRoom = cap − spent`, and `remaining = deposit − totalRedeemed`. Then `paid = min(desired, capRoom, remaining)`. The floor clamp ([Rate-floor enforcement](#rate-floor-enforcement)) applies to `cumulative` / `bytesDelivered` here; it caps the credited byte count and never blocks the payment.
...
- **Voucher validation.** Verify `voucherSig` against `signer` over `{poolId, signer, msg.sender, cumulative, bytesDelivered}` (see [EIP-712 Voucher Signature](#eip-712-voucher-signature)); require `block.timestamp < authorized[poolId][signer].expiry`, then apply the [rate-floor clamp](#rate-floor-enforcement) to the credited `bytesDelivered`. A bad signature or an expired capability stops the voucher; a sub-floor rate does not — it only caps the credited bytes. Redemption is cumulative; the voucher carries no nonce.
