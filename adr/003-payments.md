...
1. **Accept on verification; persist asynchronously.** A voucher or preimage advances in-memory lane state as soon as it verifies. The node writes the lane frontier to a buffered local store and flushes it on a **value bound**: it flushes before the un-flushed accepted value would exceed one [credit window](#credit-window). The replay surface after an unclean restart is therefore at most one credit window of re-delivered bytes — the same quantity the node already fronts, knowingly, every time it serves. Bounding the flush by value rather than by time is what keeps that equality true at any link rate.
...
Storage backend and trait shape are implementation concerns; the Rust implementation exposes a `PoolStateStore` seam in `crates/incentive` with a `redb`-backed persistent implementation in `crates/node`. The protocol fixes only the floor rule and the value bound above.
