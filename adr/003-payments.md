...
- **Default-open content** is governed by the DAO-maintained default-open allow-list ([ADR 011 § Default-open allow-list](011-content-takedown.md#default-open-allow-list)); the allow-list enforces its own redundancy floor (default 10), so a single withholding origin is mitigated the same way as registered namespaces.
