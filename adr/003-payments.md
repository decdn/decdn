...
- Payment quantum: `chunk_bytes` is 1 MiB (1,048,576 bytes), a protocol constant. One chunk is the price quantum, the meter tick, and the withholding bound at the same time (see [Chunk sizing and the payability floor](#chunk-sizing-and-the-payability-floor))
- Signature cadence: one signature for a small transfer, two for a large cooperative one, and one more per chain rollover. It is never one per chunk (see [Two payment resolutions](#two-payment-resolutions))
...
The meter ticks once per `chunk_bytes` delivered. `chunk_bytes` is **1 MiB (1,048,576 bytes), a protocol constant**. It is not negotiated, and governance does not tune it.

**Why it is a constant, not a parameter.** The chunk size is baked into every claim: redemption resolves `claimed_bytes = bytes_delivered + chain_index × CHUNK_BYTES` ([Redemption and Close](#redemption-and-close)). A node and a payer that disagree on the chunk size disagree on what a released preimage is worth, and the contract has no way to learn which value they used. Fixing the constant removes the disagreement instead of negotiating it away. Changing the value later invalidates every voucher signed against a live chain, so it is a protocol-version change and not a knob.

**One chunk is one MiB because the payment token says so.** USDC carries 6 decimals, so a chunk must price to at least one base unit or the meter cannot tick at all. 1 MiB is about 10× that floor at the expected market rate. The full sizing argument is in [Chunk sizing and the payability floor](#chunk-sizing-and-the-payability-floor).

**`chunk_price` is per-chunk and is signed.** Because `CHUNK_BYTES` equals `BYTES_PER_MB`, `chunk_price` is the node's advertised `rate_per_mb` in the same units. The payer signs it into the voucher, so the price of a chunk is fixed for the chain the voucher opens.

**Node sovereignty is unchanged, and it needs no wire field.** A node stops delivering when the unpaid balance on a lane reaches its [credit window](#credit-window). The threshold is node-local policy. A node that wants tighter exposure sets a smaller window; it never has to agree a cadence with the payer to get one.

The chunk cadence is complemented by per-node `max_blob_size` limits ([ADR 005](005-protocol.md#error-handling-and-retry-semantics)): the cadence bounds what a payer can withhold, while `max_blob_size` allows nodes to refuse blobs that would create unacceptable resource pressure (cache exhaustion, extended origin pulls) at any cadence.

### Hash-chain metering (PayWord)

**PayWord is not a new object.** There is one voucher, and its cumulative `amount` is the settlement anchor. The hash chain is an optional, signature-free way to advance that anchor between signatures. Redemption is one formula:

```
claimed = voucher.amount + chain_index × chunk_price
```

At `chain_index = 0` the formula settles exactly `amount` and walks nothing. That case is identical in shape to a voucher without a chain. At `chain_index > 0` the node exhibits a **preimage** that proves the chunks it streamed past the last signed amount, and the contract walks at most `MAX_CHAIN_LENGTH` hashes to check it. The type system and the contract need only an amount and an optional chain that extends it.

**Mechanics.** The payer picks a random seed `s` and computes `chain_root = keccak^255(s)`, where `N = MAX_CHAIN_LENGTH = 255`. It signs that root into the voucher. After the node delivers chunk `k` and the payer verifies those bytes, the payer releases `keccak^(N−k)(s)`. The node applies `keccak256` to the released value `k` times and compares the result to `chain_root`. The root itself never travels on the wire, because the signed voucher already carries it.

**A preimage costs one hash.** Accepting one needs no signature, no acknowledgement, no round trip, and no durable commit before the node delivers further bytes. A preimage also proves its own index: nobody can compute `keccak^(N−k−1)(s)` from `keccak^(N−k)(s)` without the seed, so holding a deeper preimage is itself the receipt for the chunks below it.

#### Two payment resolutions

The chain is a bulk meter and the signature is a fine adjustment. The design runs both, rather than forcing either to do the other's job.

| Path | Resolves | Exact to | Cost |
| --- | --- | --- | --- |
| Preimage (bulk) | whole chunks streamed past the anchor | `chunk_bytes` (1 MiB) | one hash; no signature, no acknowledgement, nothing to persist |
| Voucher `amount` (exact, remainder, sub-chunk) | any residual, including a partial trailing chunk | 1 USDC base unit | one signature |

A partial trailing chunk, or a whole transfer smaller than `chunk_bytes`, settles through a fresh **amount-voucher** — a voucher with a zero root, redeemed at index 0. It does not settle through a preimage, because a preimage always advances the claim by one whole chunk. For example, 1.5 MB settles as one preimage for the first MiB plus a closing amount-voucher for exactly 1.5 MB, which is 15 base units at the market rate. The redemption walks one hash.

| Transfer shape | Signatures | Chain walk at redeem |
| --- | --- | --- |
| Small (fits the credit window) | 1 — deliver, then one closing amount-voucher | none |
| Large, cooperative close | 2 — an opening voucher that carries `chain_root`, then a zero-root closing voucher for the exact total | none |
| Large, payer stops mid-stream | 1 — the opening voucher; the node walks the highest preimage it holds | ≤ `MAX_CHAIN_LENGTH` (255) hashes |

**The chain is the unilateral fallback. The amount-voucher is the cooperative close.** Signatures are therefore O(1) per transfer — one or two, plus one per rollover — and never one per chunk. The node redeems the best proof it holds, and the contract's `claimed − paid` accounting makes that choice idempotent: presenting the weaker proof costs the node a retry and nothing else.

#### The sealed voucher (zero-hash root)

A settlement voucher sets `chain_root = 0` and redeems at `chain_index = 0` with a zero preimage. Those zeros satisfy the ordinary `keccak^k(preimage) == chain_root` check. At any index above 0 the same check would need a preimage that hashes to `0`, and keccak preimage resistance makes that infeasible. The voucher is therefore **sealed at its `amount`**: nothing can redeem more against it.

A legitimate `chain_root = keccak^255(seed)` is never `0`, so `0` is a safe sentinel. The schema stays fixed-shape, the field stays mandatory, and the contract gains no branch.

A voucher that carries a **real** root also settles exactly `amount` when it is redeemed at index 0 — the node passes the root itself as the preimage, which it already holds from the voucher. Both root shapes reach "settle the signed amount" through the same check, so nothing in the contract tests `chainRoot == 0`.

#### Chunk sizing and the payability floor

`chunk_bytes` is the price quantum, the meter granularity, and the withholding bound at the same time. USDC's 6 decimals put a hard floor under it: a chunk that prices below one base unit ($0.000001) rounds to zero, and a meter that ticks zero cannot charge.

| `chunk_bytes` | At target $0.01/GB | At floor $0.001/GB | Rounding per tick | Verdict |
| --- | --- | --- | --- | --- |
| 100 KB | 1 base unit | 0 base units | a whole unit | Unpayable at the floor rate |
| **1 MiB** | **10 base units** | **1 base unit** | **~10%** | **Chosen** |
| 10 MiB | 100 base units | 10 base units | ~1% | Payable, but a 10 MiB withholding bound |

**`chunk_bytes` is fixed at 1 MiB.** That is about 10× the single-unit floor, so a chunk prices to roughly 10 base units at the target rate and stays payable at 1 base unit down to the governance floor. Because `CHUNK_BYTES` equals `BYTES_PER_MB`, `chunk_price` is the advertised `rate_per_mb` in the same units, and the payability floor is exactly the existing `deliveryFloor` ([Rate-floor enforcement](#rate-floor-enforcement)). The chain introduces no second price constant.

The meter is therefore ~1 MiB-grained. That is finer than the 4 MiB voucher interval it replaces. It is coarser than a per-wire-chunk meter, and the reason is the payability floor rather than the cost of a hash — hashes are cheap enough to tick far finer, but the token cannot price the tick. Finer chunks lose payability at low prices. Coarser chunks only matter for a network that runs near the floor price, so a future change is an operator and governance call, and it breaks every signature made under the old value.

#### One chain per lane

A released preimage is a **bearer proof**. It carries no payee field of its own. Its binding to a payee comes entirely from the voucher whose `chain_root` it satisfies.

- A signer **MUST** derive an independent seed and `chain_root` for every `(pool_id, signer, provider)` lane.
...
- A node **MUST** reject a `chain_root` it has already seen on a different lane, or already retired on this lane. A lane's root is one-shot: a new root supersedes the old one, and the old chain's index stops advancing.

The reason is direct. If one root appeared in vouchers to two providers, every preimage the signer released to the first would also be a valid extension of the second's voucher. The second node could claim chunks it never delivered, and the signer would pay twice for one tick. The on-chain watermark bounds each lane independently, but it cannot detect this, because the two claims are legitimate under their own vouchers and the contract never sees them together. The rule is therefore a payer-side MUST, and its adversarial framing is in [Cross-lane preimage spend](#cross-lane-preimage-spend).
...
`chain_length` is not a voucher field. It is always `MAX_CHAIN_LENGTH = 255`. At 1 MiB per chunk, one chain meters 255 MiB.

When a lane exhausts its chain, the payer rolls it. The payer signs a new voucher whose `amount` folds in the exhausted chain's full value (`amount_old + 255 × chunk_price`) and whose `chain_root` is a fresh independent root, and the accepted index resets to 0. Rollover costs one signature per 255 MiB, which keeps the O(1)-per-transfer property at any transfer size. A node MUST reject an index above the bound rather than accept a proof the contract refuses to walk ([Off-chain Voucher Rejections](#off-chain-voucher-rejections-wire-encoding)).

**Why 255.** The **wire** sets this bound, not gas. A released preimage travels as `preimage ‖ index`: 32 bytes plus a one-byte index, 33 bytes flat, with no length prefix and no varint. 255 is the largest chain whose releasable indices fit one byte **directly**. A longer chain amortizes signatures further — 1024 chunks is about 1 GiB per signature — but it costs a wider wire tag and a proportionally longer worst-case walk. 255 is the deliberate trade: about 4× more signatures than a 1024-chain, in exchange for a fixed-width 33-byte message and a walk capped at 255 hashes.

**The byte is the index.** `chain_root = keccak^255(s)`, and releasable preimages are indices `1..=255`. The wire byte carries the index as itself. There is no offset tag, no `−1` on send, and no `+1` on receipt, so there is no place for an off-by-one to live. Index `0` is the settlement case at redeem — a sealed voucher, or a real-root voucher whose root is passed as its own preimage — and it never appears on the wire.
...
The chunk is the *billing* granularity, not the *delivery* granularity. A delivering node streams within a **credit window**: it keeps sending chunks while the unpaid balance `delivered − paid` stays within `credit_window` bytes and pauses a stream only when the next chunk would cross that bound — not at every chunk boundary. Between one chunk and the window, delivery and payment run concurrently. The node sends chunks ahead of the proofs that pay for them, and the payer releases a preimage per chunk while it keeps receiving.

Decoupling the two rates is what keeps single-stream throughput link-bound rather than round-trip-bound. Preimages travel one way and carry no acknowledgement, so payment never blocks delivery; the window exists to keep bytes in flight while the next preimage crosses the link, not to amortize a per-tick round trip. A window of several chunks therefore keeps the link saturated.

**Exposure is one-sided.** The payer's exposure stays at zero: a preimage pays for chunks already delivered and hash-verified, so the payer never pays for bytes it has not received. This holds as long as the signer builds an independent chain per lane — a shared `chain_root` is the one way a payer can give the property away (see [One chain per lane](#one-chain-per-lane)). The node's credit exposure is exactly the window — unbilled egress already on the wire — bounded there and nowhere else. This is the same bounded-credit shape a node already fronts on the upstream leg of a cache-miss pull (`pull_ahead_bytes`, [ADR 037](037-regional-proxy-warming.md#adr-037-latency-driven-proxy-warming-for-regional-locality)), and the downstream credit is strictly the cheaper of the two: egress it has already served, versus speculative USDC it pays an upstream provider. On the fused cache-miss serve path a single window bounds both quantities at once, since every pulled chunk is forwarded downstream immediately.

**Node-local policy, floored at one chunk.** `credit_window` is node configuration, not a wire or governance parameter — like `chunk_bytes` it has no on-chain counterpart and is never negotiated. It is floored at one chunk so a stream can always make progress (deliver a full chunk, then recoup it), and a window at one chunk reproduces the stop-and-wait cadence exactly. The self-enforcing threshold from [Chunk Cadence](#chunk-cadence) generalizes from "pause after one unproved chunk" to "pause once the unpaid balance reaches the credit window"; the sovereignty guarantee is unchanged, since a node can always choose a smaller window (down to one chunk) and pause sooner.

**Takedown latency.** The per-boundary in-flight takedown check ([ADR 011 § On Blacklist Event](011-content-takedown.md#on-blacklist-event)) runs after each collected proof — a voucher or a preimage, whichever advanced the lane — so widening delivery to a credit window widens the window in which a takedown that lands mid-stream is first observed: the first check falls up to one credit window into the stream (steady state, roughly one interval, as later intervals recoup one at a time). This is bounded by the window and floored at one interval, and even a full window of further egress is negligible against the takedown compliance window, so it does not weaken the takedown guarantee — but it is why a node with a strict compliance target configures a smaller window rather than a larger one.

**Defaults.** The window defaults to 8 MiB against the 1 MiB chunk of [Chunk Cadence](#chunk-cadence) — eight chunks of headroom. The window and the chunk move independently because they bound different things: the window bounds credit exposure and how far delivery may run ahead of payment, while the chunk bounds the amount a payer can withhold and the size of one meter tick. A preimage costs one hash and no durable commit, so the per-lane processing rate is no longer a reason to coarsen the chunk. At these values a single stream stays link-bound at any realistic round trip.
...
**The mechanism — a refundable minimum remaining deposit `M`.** A node stops serving a pool once its on-chain **remaining balance reaches `M`**. The reserved `M` then covers every outstanding in-flight claim, so **no node is ever stiffed — the tail is structurally zero** — and whatever `M` is not needed **refunds to the owner** at close. `M` is *locked but returned*, not spent, so it can be sized generously at only a temporary-lock cost, never a real loss. That is strictly better than sizing the deposit large to *dilute* the tail (where the tail is a genuine loss you merely make a small fraction of): with `M` the tail is zero and the deposit stays withdrawable — the owner always recovers `≥ M − in-flight`.
...
**Sizing.** `M` covers the worst-case total in-flight, `M ≥ N · t` (fan-out `N` × per-lane threshold `t`). In-flight value is measured on the best proof a node can present — `claimed = amount + chain_index × chunk_price` — not on the last signed `amount`. An unredeemed preimage is in-flight value exactly as an unredeemed voucher is. Equivalently `M = k · ρ · B · Δ`, where `ρ` is the price per bandwidth-time (`$0.00125` per Gbps·s at the `$0.01/GB` market rate), `B` is the maximum aggregate bandwidth the pool is defended against (fan-out × per-node throughput), `Δ` is the **detection delay** (how fast nodes notice the balance crossing `M` and stop — chain-dependent; see [Appendix: L2 Deployment](appendix-l2-deployment.md#appendix-production-l2-deployment-target)), and `k ≈ 2` is a safety factor. The per-lane threshold is `t = ρ · V · Δ` for a lane serving at bandwidth `V`.
...
**No per-node rate ramp — a fresh wallet serves at full speed.** Because the security bound is the reserved floor `M`, not a per-relationship earned window, a node does **not** throttle a brand-new pool: it serves at full link speed from the first byte, for as much volume as the deposit funds above `M`. Trust affects only redemption *cadence* (`t`, a gas concern), never delivery rate. The only caveat is fan-out *width*, not per-stream rate: a fresh pool spread across very many nodes at once can reopen the bounded tail below — a residual we accept, never a reason to slow any single stream. This is the cold-start improvement over a dynamic per-relationship window, which throttled every fresh `(node, pool)` pair until it earned history.
...
2. **One chain per lane, shared by every stream on it.** The lane's `chain_root` and chain index are lane-wide state, not per-stream. A stream that joins an active lane joins the running chain and continues its index; it does not open a second root. Opening a second root on a live lane is a protocol error (see [One chain per lane](#one-chain-per-lane)).
3. **Proof routing.** Vouchers and preimages are sent on any active stream in the lane — the node credits them against the lane-wide counter regardless of which stream carries the message.

**Example:** A signer has stream A (blob X) and stream B (blob Y) on the same lane. After receiving 1 MiB in total (for example 0.7 MiB from A and 0.3 MiB from B), the signer releases the next preimage on either stream. There is no per-stream cadence to reconcile, no lane-wide minimum to compute, and nothing to flush when a stream joins or ends: the chunk size is a protocol constant, so every stream on the lane already agrees on it.
...
**Redemption is the settlement primitive.** A node redeems its lane on-chain whenever it chooses, while the pool is `Open` or in the grace window. Redemption is provider-only (`msg.sender == voucher.provider`) and final — it needs no dispute window, because a claim is a signed, cumulative, monotone commitment by a capped signer, extended only by preimages that signer itself released, and the node redeems only its own lane. The node submits the voucher and the best proof it holds: `redeem(poolId, signer, provider, amount, bytesDelivered, chainRoot, chunkPrice, voucherSig, capability, chainIndex, preimage)`:
...
3. **Check payee, then resolve the claim.** Require `provider == msg.sender`. Then bound and walk the chain: require `chainIndex <= MAX_CHAIN_LENGTH`, apply `keccak256` to `preimage` exactly `chainIndex` times, and require the result to equal `chainRoot`; revert `BadPreimage` otherwise. The claim is `claimed = amount + chainIndex × chunkPrice` over `claimedBytes = bytesDelivered + chainIndex × CHUNK_BYTES`. There is no branch here and no optional field. A sealed voucher (`chainRoot = 0`) redeemed at `chainIndex = 0` with a zero preimage satisfies the same check with zeros and resolves to exactly `amount`; at any higher index it would need a preimage that hashes to zero, which is infeasible. A voucher carrying a real root and redeemed at index 0 likewise settles exactly `amount`, with the root passed as its own preimage. Redemption is cumulative — the voucher carries no nonce.
4. **Compute the payable amount.** With `w = watermark[poolId][signer][provider]`: `desired = claimed − w.amount` (the still-unpaid portion this voucher and preimage authorize), `capRoom = cap − spent`, and `remaining = deposit − totalRedeemed`. Then `paid = min(desired, capRoom, remaining)`. The floor check ([Rate-floor enforcement](#rate-floor-enforcement)) applies to `claimed` / `claimedBytes` here — the chain-extended values, so bytes proved by preimage carry the same per-byte price obligation as bytes proved by signature.
5. **Pay, or revert if nothing is payable.** If `paid == 0`, revert `NothingToRedeem` — the call writes **no** state and moves no funds, so the node simply retries later (after a top-up, with a higher voucher, or with a deeper preimage). Otherwise: `bytesDelta = claimedBytes > w.bytesDelivered ? claimedBytes − w.bytesDelivered : 0` (clamped at zero); `bytesPaid = mulDiv(bytesDelta, paid, desired)` (the **paid-proportional** byte count — `== bytesDelta` when `paid == desired`); advance `w.amount += paid`, `w.bytesDelivered += bytesPaid`, `spent += paid`, `totalRedeemed += paid`; `safeTransfer` `paid` USDC to the `FeeRouter`; then call `FeeRouter.routeSettlement(provider, bytesPaid, paid)`. The `bytesDelta` clamp settles the money owed even when a claim's `claimedBytes` has not advanced past the lane's paid bytes: the signer already committed the higher cumulative value, so the money is owed and settles, while the byte credit for that redemption is zero and the byte watermark holds — it recovers on a later claim whose `claimedBytes` advances again. This keeps the money axis (payment) independent of the served-bytes axis (vote weight) and denies any DoS from a bytes-regressed claim.

**The chain walk is bounded by construction.** The contract always hashes forward from the submitted preimage to `chainRoot`. It never hashes from a stored intermediate, so it keeps no chain state and needs no root-matching branch. The cost is `chainIndex` hashes, capped at `MAX_CHAIN_LENGTH` (255) — see [Chain walk and redemption cost](#chain-walk-and-redemption-cost).

**Rollover is safe because payment is cumulative.** A signer ends a chain by issuing the next voucher with `amount` set to the frontier the old chain reached, plus a fresh `chainRoot`. The node accepts it only if that base is at least what the lane has already earned. No sequencing rule is needed on-chain: if the node redeems the old voucher and then the new one, the new one's `desired = claimed − w.amount` is already net of the old payment, and a node that redeems only the new voucher collects the same total. Redeeming a superseded chain after a newer one computes `paid == 0` and reverts `NothingToRedeem`.

**Partial redemption is retry-safe.** The lane watermark tracks the cumulative amount **actually paid**, not the claim the node presented. When a drained pool covers only part of `desired`, `w.amount` advances by just `paid`, so re-presenting the *same* voucher and preimage after the owner tops up collects the remainder — `desired = claimed − w.amount` is still positive. A node never loses an over-committed claim to a transient drain; it loses value only if the owner closes and reclaims without ever topping up, bounded by the reserve `M` the node keeps against the pool ([Pool solvency and the refundable floor `M`](#pool-solvency-and-the-refundable-floor-m)). Because payment is `claimed − paid`, replay is automatic: an already-paid or stale claim — a lower `chainIndex`, or a superseded chain — computes `paid == 0` and reverts, moving nothing. The claim is resolved fresh on every call, so the chain needs no separate anti-replay state and none is stored. `min(desired, capRoom, remaining)` is the solvency backstop — the pool never goes negative, and no signer can cause more than `cap` USDC to leave the pool, since `spent` is the cumulative USDC actually paid on its behalf and `paid` is capped by `cap − spent`.
...
**Batch redemption.** `redeemMany(capabilities[], vouchers[])` registers signers and redeems lanes across many pools in one transaction. It takes two independent arrays: `capabilities` (each a `{poolId, signer, spendingCap, expiry, ownerSig}` registration) and `vouchers` (each a `{poolId, signer, provider, amount, bytesDelivered, chainRoot, chunkPrice, voucherSig, chainIndex, preimage}` claim, applied with the single-`redeem` arithmetic above). The call registers every capability first — verifying the owner signature and storing `authorized[poolId][signer]`, idempotent for an already-registered signer — then redeems every voucher. Separating registration from the voucher decouples the two: a signer is registered by its own capability entry regardless of whether any voucher for it pays, so a skipped voucher never loses a registration. A voucher that would pay `0` — drained pool, already-paid or stale claim, expired capability, cap reached, or a signer neither registered nor covered by this call's `capabilities` — is **skipped, not reverted**, so one empty lane never sinks the batch; the call emits one `PoolRedeemed` per paid voucher and returns the total paid. Only a structurally-invalid entry — a bad voucher signature, `provider != msg.sender`, a bad capability owner-signature, a preimage that does not hash to `chainRoot`, or a `chainIndex` above `MAX_CHAIN_LENGTH` — reverts the whole call, since that is caller error, not transient pool state. Each entry carries its own chain walk, so a batch's walk cost is the sum of its entries' `chainIndex` values.

**Close and reclaim.** Redemption pays nodes; close returns the owner's unspent remainder. There is no adversarial close: the owner submits no vouchers on any node's behalf, so a close can never understate a node's earnings, and no third-party dispute path is needed. `closePool(poolId)` (owner only) sets status `Closing` and starts the grace window (`disputeDeadline = block.timestamp + disputeWindow`, default 48 hours). Nodes may still `redeem` during `Closing`. After `disputeDeadline`, `reclaim(poolId)` transfers `deposit - totalRedeemed` to the owner and sets status `Closed`. A node that has not redeemed by the deadline forfeits its outstanding claim. A pool does not expire, so a node's only deadlines are the grace window (after an owner close) and each signer's capability `expiry`; a diligent node redeems within its capability-bounded serving window, so this is the node's own cash-flow choice, not a theft surface.
...
| `AmountRegression` | the proof's `claimed` (`amount + chain_index × chunk_price`) ≤ the node's accepted claimed cumulative | none on-chain (`redeem` pays `claimed − paid`, so a stale proof pays `0`) — the sole off-chain ordering/replay guard now that vouchers carry no nonce; keeps the node's own ledger consistent |
| `BytesRegression` | `voucher.bytes_delivered <` the node's accepted cumulative bytes | as for `AmountRegression`, on the byte axis |
| `BadPreimage` | a released preimage does not satisfy `keccak^k(preimage) == chain_root` at the claimed index `k` | `redeem` reverts `BadPreimage` at the chain walk; rejecting early saves the node the gas of a doomed walk |
| `ChainExhausted` | the claimed index would exceed `MAX_CHAIN_LENGTH` (255) | `redeem` reverts `ChainIndexTooLarge`; the signer MUST roll a fresh root (see [Chain length and rollover](#chain-length-and-rollover)) |
...
- The proof is the payment receipt; the BLAKE3 hash is the delivery receipt. Together they provide mutual protection: the signer doesn't release a preimage for bytes that fail hash verification; the node stops delivering if proofs stop arriving
- **Signatures are O(1) per transfer, never per chunk.** One signature covers a small transfer, two cover a large cooperative one, and rollover adds one per 255 MiB. Between signatures the meter advances one chunk at a time for the cost of a single hash — no signature, no acknowledgement, no round trip, nothing to persist per tick. A 10 GB transfer that once needed thousands of signed vouchers now needs tens (see [Two payment resolutions](#two-payment-resolutions))
- **PayWord adds no contract storage and no second settlement object.** The lane watermark is still `{amount, bytesDelivered}`, redemption is still one `claimed − paid` subtraction, and a chain-free voucher redeems along exactly the same code path as before
- Maximum risk is bounded by the credit window and floored at one chunk — 1 MiB at market rate is about $0.00001, and it no longer scales with a negotiated interval. The old wire-ceiling worst case ($1.024 for a 1024 MB interval) is unreachable, because the meter ticks every MiB however far apart the signatures fall (see [Chunk sizing and the payability floor](#chunk-sizing-and-the-payability-floor))
...
- The tail is structurally zero for bounded fan-out: the reserved `M` covers all in-flight claims. It reopens only past fan-out `N = M/t` — a bounded residual we accept (each node risks only its threshold `t`, and enlarging the tail costs the attacker real bandwidth); self-dealing is separately bounded by `M` and taxed by the FeeRouter cut.
...
- **The meter cannot go finer than about 1 MiB.** USDC's 6 decimals make a sub-MiB chunk price to zero base units at the governance floor rate, so the payability floor — not the cost of a hash — caps the granularity (see [Chunk sizing and the payability floor](#chunk-sizing-and-the-payability-floor)). A finer meter would need a higher floor price or a token with more decimals, and neither is on the table.
- **A preimage is a bearer proof within its lane.** It carries no payee field of its own; its binding to a payee comes entirely from the voucher whose `chain_root` it satisfies. That is why a root MUST NOT be shared across lanes ([One chain per lane](#one-chain-per-lane)) — a shared root would let preimages released to one node be spent against another's voucher. The contract cannot enforce this, because it never sees two vouchers together.
- **The unilateral-fallback redemption pays for the walk.** Up to `MAX_CHAIN_LENGTH` (255) hash rounds, on the order of 16k gas at full depth, and zero on the cooperative path where the closing voucher redeems at index 0 (see [Chain walk and redemption cost](#chain-walk-and-redemption-cost)). The hash chain is also a construction in the money path: well studied (Rivest–Shamir PayWord, 1996) and only a bounded loop on-chain, but it is more than a signature check and it carries its own audit surface.
- The chunk price is fixed for a chain. `chunk_price` is signed into the voucher, so a rate change takes effect at the next chain rather than the next chunk. This costs the signer one signature and matches the existing rule that the `StreamResponse` rate binds for the stream.
- The node absorbs no sub-chunk remainder, but it needs the payer's cooperation to collect one. A blob that ends mid-chunk yields no preimage for the final partial chunk; the payer settles it with a closing amount-voucher. If the payer never sends one, the node loses under one `chunk_bytes` — the same bounded per-lane loss it already accepts under [Voucher withholding](#voucher-withholding).
...
Client receives bytes but stops paying — it stops releasing preimages, or it runs a chain to exhaustion and never anchors the next voucher — getting content for free up to the last proof it sent.

The self-enforcing stop is sufficient, and the chain tightens it. Maximum loss is bounded by the node's credit window and floored at one chunk: 1 MiB at the market rate is about $0.00001. It no longer scales with a negotiated interval, because the node measures the unpaid balance against the best proof it holds rather than against the last signature — a payer cannot buy a larger free window by negotiating a coarser cadence, since there is no cadence to negotiate. Nodes serving high-value content can set a smaller window still.

Running a chain to exhaustion and stopping is the same bound in a different shape. The node stops serving the lane and returns `ChainExhausted` ([Off-chain Voucher Rejections](#off-chain-voucher-rejections-wire-encoding)). Nothing is lost: every chunk of the finished chain is already covered by a preimage the node holds and can redeem. The attacker gains no free bytes, only the end of its own service, and resuming costs it one signature.

The stop is per-lane and mechanical. The node pauses the moment the unpaid balance on a lane reaches that lane's [credit window](#credit-window) and waits for the next higher-cumulative proof on the same lane; it never fronts more than one window against one lane. The pause keys on the lane, not the counterparty. A client that stalls or crashes mid-stream and reconnects is treated no differently from a first request: it resumes on the same lane under the same window and pays only for the chunks it proved, never for the interrupted window. The node's own cost of the interruption — re-reading a held blob, or dropping and later re-pulling an unfinished cache-miss fill — is bounded to that one window either way, and no black mark follows the client into its next request. No history accumulates against a client across connections, and none needs to: a fresh signer from the same party gets its own window and is bounded the same way. The bound is a property of each lane, so a new key buys an attacker nothing it did not already have.
...
- **The node bounds its own exposure.** A node stops serving a pool once its remaining balance reaches the reserved floor `M`, which covers all outstanding in-flight claims, so the tail is zero (see [Pool solvency and the refundable floor `M`](#pool-solvency-and-the-refundable-floor-m)).
...
Node accepts a stream request, receives the opening voucher, then stops delivering bytes.

**The signed voucher is worth nothing on its own.** Its `amount` is the chain's *base* — what the lane has already earned — and every unit above it needs a preimage, which the signer releases only for a chunk it has received and hash-verified. A node that takes a voucher and delivers nothing can redeem only at `chain_index = 0`, which claims exactly that base: on a fresh lane the base is `0`, so the call pays nothing and reverts `NothingToRedeem`; on an established lane it is value the node could already have collected with the previous chain's preimages. Opening a chain therefore moves no money and grants no new claim — it fixes the price, the root, and the ceiling, and nothing more.
...
A signer reuses one `chain_root` across two providers. Every preimage it releases to node A is then also a valid extension of node B's voucher, so B claims chunks it never delivered.

**This is a payer-side footgun, and it costs the payer.** A preimage is a bearer proof: it carries no payee field, and its binding to a payee comes entirely from the voucher whose root it satisfies. Under a shared root, A hands preimage `k` to B, and B redeems at index `k` having delivered a fraction of those chunks. The signer pays for chunks it never received, bounded only by `MAX_CHAIN_LENGTH × chunk_price` per extra lane and ultimately by its own `cap`. This is precisely the zero-exposure property of [Credit Window](#credit-window) being given away.

**No node can detect it, and the contract cannot either.** A node sees only its own lane and cannot know another voucher shares its root. The contract never sees two vouchers together, so both claims are legitimate under their own signatures. The defence is therefore structural and payer-side: **a signer MUST derive an independent seed and root for every `(pool_id, signer, provider)` lane**, so the bearer scope of a preimage is exactly the one node already entitled to it (see [One chain per lane](#one-chain-per-lane)).

**The node-side backstop is root-reuse rejection.** A node MUST reject a `chain_root` it has already seen on a different lane, or already retired on this lane. That catches the case the node can see and keeps its own ledger one-shot per root. It does not need a wire reason of its own: on-chain a replayed root is already neutralized by `claimed − paid`, so the guard exists to keep the node's off-chain ledger consistent, exactly as `AmountRegression` does.

**The opposite direction is not an attack.** Presenting one node's voucher at another node achieves nothing: the root is a signed field of a voucher that names `provider`, and `redeem` requires `provider == msg.sender`. A node holding a voucher that names someone else cannot redeem it, and it rejects the stream with `WrongProvider` rather than serving.

#### Corrupted delivery

Node serves bytes that don't match the advertised BLAKE3 hash.

Absorbed at the wire by progressive BLAKE3 verification at the client (mandatory in `cdn/client/v1` per [ADR 002](002-content-addressing.md#adr-002-content-addressing) and [ADR 005](005-protocol.md#adr-005-wire-protocol)). Preimages are released only after the corresponding chunks have been verified — a corrupt chunk therefore yields no preimage. The client drops the connection, requests the blob from a different node, and recovers any unspent channel funds via channel-close. **Client monetary loss in the corruption case is zero**; the only cost is downstream bandwidth (sunk regardless of outcome).

No on-chain slash machinery is needed for content corruption. The threat is bounded in framing parallel to [§ Voucher withholding](#voucher-withholding) above: per-encounter wasted bandwidth is capped at one credit window on each side, and at one chunk in steady state (the client's downstream cost for a corrupt chunk; the node's upstream cost when a correctly-withheld preimage leaves that chunk unpaid). Both bounds are per-lane and mechanical — the same self-enforcing credit-window pause applies, keyed to the lane rather than the counterparty. Clients prefer nodes whose probe and delivery history they trust; the protocol coordinates neither side.
...
- **No regression or double-spend.** The lane watermark tracks cumulative **paid** amount and only increases, as do `spent` / `totalRedeemed`. Redemption pays `claimed − paid`, and `claimed` folds the chain in before the subtraction, so re-submitting a proof pays only what is still owed while an already-paid voucher, a lower `chainIndex`, or the same index twice all pay `0`; `FeeRouter.routeSettlement` is purely additive, so each paid byte and USDC unit is counted exactly once across a lane's redemptions.
...
EIP-712 typed data over `{poolId, signer, provider, amount, bytesDelivered, chainRoot, chunkPrice}` binds the voucher to a specific pool, signer, and payee. `provider` is what stops one node from redeeming a voucher meant for another, and it is also what stops a released preimage from paying at any other node. The EIP-712 domain separator (see [EIP-712 Voucher Signature](#eip-712-voucher-signature)) further binds each voucher to a specific chain and contract deployment, preventing replay across different chains, contract upgrades, or test vs production environments. The contract settles in a single token fixed at deployment, so a voucher can only ever pay in that token — cross-token replay does not arise, and the voucher carries no token field. Resubmission after redemption is neutralized by the cumulative accounting: `redeem` pays `claimed − paid`, so an already-redeemed claim pays `0` and moves no funds.

Preimage replay is neutralized the same way. A preimage is not independently redeemable — it settles only through a voucher that carries its `chain_root` and names the redeeming node — so a replayed preimage resolves to a claim the lane has already been paid and computes `paid == 0`. Across lanes the defence is the one-root-per-lane rule ([One chain per lane](#one-chain-per-lane)): a root that appears in exactly one lane's vouchers makes every preimage under it spendable only by the provider that lane names.
...
**The register carries no chain state.** A chain's root and released index live entirely off-chain. The contract folds the chain into the claim at redeem time and stores only the paid result, so PayWord adds no storage slot, no mapping, and no migration. The register keeps the same two mappings it would keep without it.
...
> **Terminology:** `poolNonce` is the pool creation counter and the only nonce in the payment path. Vouchers carry none: ordering and replay are handled entirely by the monotone cumulative claim (see [Voucher ordering](#voucher-ordering)).
...
| Lifecycle | `redeem(poolId, signer, provider, amount, bytesDelivered, chainRoot, chunkPrice, voucherSig, capability, chainIndex, preimage)` | Provider-only (`provider == msg.sender`): register the signer on first use (verify the owner `capability`), verify `preimage` against `chainRoot` at `chainIndex`, resolve `claimed = amount + chainIndex × chunkPrice`, then pay `min(desired, capRoom, remaining)` — the still-unpaid portion, capped by the signer's remaining cap and the pool balance; routes the paid amount through `FeeRouter` same-tx. A `chainIndex` of `0` with a zero `chainRoot` and a zero `preimage` settles exactly `amount` with no walk. Reverts `BadPreimage` on a chain-walk mismatch, `ChainIndexTooLarge` above `MAX_CHAIN_LENGTH`, and `NothingToRedeem` if nothing is payable (writes no state, so it is retried). No dispute window. See [Redemption and Close](#redemption-and-close). |
| Lifecycle | `redeemMany(capabilities[], vouchers[])` | Provider-only: register each capability in `capabilities` (idempotent — an already-registered signer is a no-op), then redeem each voucher in `vouchers`. The two arrays are independent: a node registers the signers it needs and redeems all its lanes in one transaction, and a skipped voucher never affects a registration. **Skip** (do not revert) any voucher that would pay `0` — drained pool, stale/already-paid claim, expired capability, cap reached, or a signer neither registered nor present in `capabilities`. Reverts only on a bad voucher signature, `provider != msg.sender`, a bad capability owner-signature, a bad preimage, or a `chainIndex` above `MAX_CHAIN_LENGTH`. Returns the total paid. Walk cost scales with the sum of the entries' `chainIndex` values. |
...
| View | `MAX_CHAIN_LENGTH() → uint256` | Ceiling on `chainIndex` (255), which bounds the redemption chain walk. See [Chain length and rollover](#chain-length-and-rollover). |
...
`redeem` is written with flat arguments above for continuity with the pre-chain signature. An implementation MAY take the signed fields as a single `Voucher` calldata struct instead, which is ABI-equivalent for a fixed-size type.
...
**`redeem` behavior:** `redeem(poolId, signer, provider, amount, bytesDelivered, chainRoot, chunkPrice, voucherSig, capability, chainIndex, preimage)` pays a node against a monotone claim **while the pool is `Open` or in the grace window**. It is safe without a dispute window because a voucher is a signed, cumulative claim by a capped signer and a node redeems only its own lane. The residual is the signing key itself — a compromised key can authorize up to its `spending_cap`, retired by the capability's `expiry` (see [Redemption while the pool is Open](#redemption-while-the-pool-is-open-no-dispute-window)).
...
- **Voucher validation.** Verify `voucherSig` against `signer` over the seven signed fields (see [EIP-712 Voucher Signature](#eip-712-voucher-signature)); require `block.timestamp < authorized[poolId][signer].expiry`. Then resolve the chain: require `chainIndex <= MAX_CHAIN_LENGTH`, walk `chainIndex` `keccak256` rounds forward from `preimage` and require the result to equal `chainRoot`, and set `claimed = amount + chainIndex * chunkPrice` over `claimedBytes = bytesDelivered + chainIndex * CHUNK_BYTES`. Apply the [rate floor](#rate-floor-enforcement) to `claimed` / `claimedBytes`. Redemption is cumulative; the voucher carries no nonce and the chain adds no stored state.
- **Effects (checks-effects-interactions):** let `w = watermark[poolId][signer][provider]`; compute `desired = claimed − w.amount`, `capRoom = cap − spent`, and `paid = min(desired, capRoom, deposit − totalRedeemed)`; if `paid == 0`, revert `NothingToRedeem` (no state written). Otherwise compute `bytesDelta = claimedBytes − w.bytesDelivered` (clamped at zero) and `bytesPaid = mulDiv(bytesDelta, paid, desired)`; set `w.amount += paid`, `w.bytesDelivered += bytesPaid`, `spent += paid`, `totalRedeemed += paid`; then `safeTransfer(feeRouter, paid)` and call `FeeRouter.routeSettlement(provider, bytesPaid, paid)` in the same transaction. `routeSettlement` is purely additive (`+=`); `bytesPaid` counts only paid bytes (`== bytesDelta` when the pool is solvent), so served bytes never outrun paid USDC and each paid byte and USDC unit is counted exactly once. Advancing `w` by `paid` (not to `claimed`) is what makes a partially-paid draw retriable — re-presenting the same voucher and preimage after a top-up collects the rest.
- **Modifiers:** `nonReentrant`.
- **Emits:** `PoolRedeemed(poolId, signer, provider, paid, bytesPaid, newPaidCumulative)`.

`redeem` changes nothing about off-chain payment exchange. Signers keep anchoring cumulative vouchers up to their cap and metering the bytes between them with preimages, and the node keeps accepting both; the node reads the authoritative on-chain lane watermark (`w.amount`, the cumulative paid) and redeems the best proof it holds against it — a closing amount-voucher if the payer cooperated, otherwise the opening voucher plus its deepest preimage. Because settlement is `claimed − paid`, presenting the weaker proof costs the node nothing but a retry. Deciding *when* to redeem is node operational policy, bounded only by the capability `expiry` and the grace window. `redeemMany` registers each capability, then applies this per voucher and skips any that would pay `0`.

#### Chain walk and redemption cost

```solidity
uint256 constant CHUNK_BYTES      = 1_048_576;  // payment quantum; equals BYTES_PER_MB
uint256 constant MAX_CHAIN_LENGTH = 255;        // 255 chunks = 255 MiB per chain; releasable indices
                                                // 1..=255 encode directly in one wire byte, and the
                                                // walk is bounded at <= 255 keccak rounds

// One check covers both resolutions. A sealed voucher (chainRoot == 0) redeemed at
// chainIndex == 0 with preimage == 0 satisfies it with zeros; at any higher index it
// would need a preimage hashing to zero, which keccak preimage resistance forbids.
// No branch on chainRoot exists.
require(chainIndex <= MAX_CHAIN_LENGTH, ChainIndexTooLarge());

bytes32 h = preimage;
for (uint256 i = 0; i < chainIndex; ++i) {
    h = keccak256(abi.encodePacked(h));
}
require(h == chainRoot, BadPreimage());

uint256 claimed      = amount         + chainIndex * chunkPrice;
uint256 claimedBytes = bytesDelivered + chainIndex * CHUNK_BYTES;
```

**The walk costs gas, and the implementation decides how much.** One 32-byte `keccak256` plus loop overhead costs roughly 60 gas per step when the implementation hashes from scratch space in assembly, so a full-depth walk of 255 adds on the order of 16k gas — about a tenth of a warm redemption. An implementation that round-trips through `abi.encodePacked` allocates memory per iteration and costs close to twice that for the same result, so implementations SHOULD hash from scratch space.

**The cost is paid only on the unilateral-fallback path.** The cooperative close redeems a zero-root voucher at index 0 and walks nothing. Redemption is also rare and amortized: one call settles every chunk and every prior chain, because the current voucher's `amount` already folds in the earlier frontier.

Neither `chainIndex` nor `preimage` is a voucher field. Both are redeem parameters the node supplies, so the payer never signs them and the calldata cost is two words. `chainIndex` is derivable from `(claimed, amount, chunkPrice)`, but passing it is gas-neutral and keeps the walk bound explicit, so it is passed. Hoisting `chain_length` and `chunk_bytes` out of the signed voucher into protocol constants is about 20% less redeem calldata, which is what pays for the walk on calldata-dominated chains. Overflow is not a concern: `chainIndex <= 255` and `chunkPrice <= MAX_RATE_PER_MB` (10^12) bound the product below 2^48.

**Tracking owed vs. paid.** A node tracks the *paid* side by **consuming events, not by polling**. It subscribes to `PoolRedeemed` filtered on its own `provider` address (indexed) and, as each event arrives — including the ones its own `redeem` / `redeemMany` transactions emit — sets that lane's paid cumulative to the event's `newPaidCumulative`. That event stream is the single write path for the paid side, so a drained-pool partial pay is recorded exactly; the node never assumes its voucher cleared. It follows `PoolToppedUp` (indexed by `poolId`) the same way, to re-drive a lane that a dry pool left `owed > paid`. On startup or after a gap it reconciles like every other chain watcher — enumerate `PoolRedeemed` for its address from a pinned block, then tail live, resyncing on a missed range — so the watermark is never reconstructed by guesswork. *Owed* is the highest **claim** the node can prove per lane — `amount + chain_index × chunk_price` over the best voucher-and-preimage pair it holds — persisted for replay-safety ([Off-chain voucher state persistence](#off-chain-voucher-state-persistence)). `unredeemed = owed − paid`, summed across a pool's lanes, is the in-flight value the reserved floor `M` must cover, and what keeps a lane pending until it is fully collected. A read-call (`getPool` → `deposit − totalRedeemed`; `getAuthorization` → `cap − spent`) is used only as an optional pre-flight to skip submitting a `redeem` that would revert `NothingToRedeem` against a dry pool — never to track paid state. All of this survives a restart: paid is rebuilt from the event log, owed from the persisted per-lane store.

#### Initial deployment values

The constructor takes `(usdc, capacityBond, feeRouter, disputeWindow, deliveryFloor, admin)` per [ADR 016 § Contract Inventory](016-contract-interactions.md#contract-inventory). Every governable parameter it exposes is a constructor argument; there are none it defaults. `disputeWindow` is a constructor argument validated against the hardcoded safety bounds (deployment default 48h — see the bounds table below and [ADR 009](009-governance.md#adr-009-governance-model) for governance ranges). A pool does not expire, so it configures no duration parameter. The constructor MUST reject any zero address among `(usdc, capacityBond, feeRouter, admin)` and a `feeRouter` whose code size is zero (EOA / undeployed address). `CHUNK_BYTES` and `MAX_CHAIN_LENGTH` are hardcoded constants, not constructor arguments and not governable: `CHUNK_BYTES` equals `BYTES_PER_MB` by construction ([Chunk sizing and the payability floor](#chunk-sizing-and-the-payability-floor)), and changing either value invalidates every voucher signed against a live chain.
...
| Chain walk per redeem (`chainIndex`) | 0 | `MAX_CHAIN_LENGTH` (255) |
| Chunk size (`CHUNK_BYTES`) | 1,048,576 bytes (fixed constant, not governable) | 1,048,576 bytes |
...
| --- | --- | --- | --- |
| `deliveryFloor` | $0.000001/MB | 1 | Anti-abuse minimum; 10× below expected market rate. **Enforced at redemption** — the contract rejects any claim whose cumulative `claimed / claimedBytes` falls below this floor, so claiming served bytes always costs proportional USDC. Because one chunk is one MiB, this is also the **payability floor** on `chunk_price`: a chunk priced below 1 base unit rounds to zero and the meter cannot tick ([Chunk sizing and the payability floor](#chunk-sizing-and-the-payability-floor)). Prevents zero-rate free-riding and the served-byte vote-weight inflation of [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight), while imposing no practical constraint on legitimate pricing (nodes set rates well above it; the floor is an anti-zero safeguard, not a recommended price). |

The expected market rate is $0.00001/MB (10 USDC base units per MB, or $0.01/GB). At that rate one chunk prices at 10 base units, so the chain meters in ~10% steps of a base unit — the finest the token's 6 decimals allow at that price. This positions deCDN ~4–9× cheaper than major traditional CDNs (CloudFront at $0.085/GB, KeyCDN at $0.04/GB) and at parity with budget providers (Bunny.net at $0.01/GB). The floor is governance-tunable from day one within the hardcoded safety constraints above — admin-key-gated in the PoC, DecdnGovernor in production (see [ADR 009](009-governance.md#adr-009-governance-model)). Node pricing is otherwise a market outcome: nodes compete on the rate they advertise, and a node that overprices loses selection ([ADR 001](001-network.md#adr-001-network-topology-and-peer-mesh)).
...
```
require:  claimed * BYTES_PER_MB >= claimedBytes * deliveryFloor   (BYTES_PER_MB = CHUNK_BYTES = 1_048_576, ADR 005)

where     claimed      = amount         + chainIndex * chunkPrice
          claimedBytes = bytesDelivered + chainIndex * CHUNK_BYTES
```

evaluated overflow-safely as `claimedBytes <= Math.mulDiv(claimed, BYTES_PER_MB, deliveryFloor)` so a claim carrying `claimedBytes` near `type(uint256).max` reverts with `RateFloorViolation` rather than an arithmetic panic. Because `deliveryFloor >= 1` the divisor is non-zero. The check runs on the **chain-extended** cumulative values in `redeem`, so bytes proved by preimage carry the same per-byte price obligation as bytes proved by signature. It uses **zero tolerance** — the floor sits 10× below the expected market rate, so honest traffic clears it by ≥10× and needs no rounding headroom (the off-chain 1% tolerance applies to the *advertised* `rate_per_mb`, not this floor).

**There is no separate on-chain check of `chunkPrice`.** Because `CHUNK_BYTES` equals `BYTES_PER_MB`, a below-floor `chunkPrice` passes the aggregate check only when the voucher's own `amount` over-pays enough to cover the shortfall — and in that case the claim as a whole is above the floor, which is the only property the contract needs. A standalone `require(chunkPrice >= deliveryFloor)` would reject legitimate claims where a large anchor subsidizes a cheap tail.

**Off-chain, the node MUST check the per-chunk price at accept time, and that check is stricter.** A node MUST verify `chunk_price >= deliveryFloor` against its cached floor, with zero tolerance, and MUST refuse a `chunk_price` below its own advertised `rate_per_mb`. The reason is the unilateral-fallback path: there the claim is dominated by `chain_index × chunk_price` with a small or zero `amount`, so an under-priced chunk leaves the aggregate below the floor and the whole chain unredeemable — a loss the node cannot recover once the payer has stopped responding. Nodes therefore mirror the floor at two granularities: per-chunk at accept, aggregate at redeem. An honest node never accepts a proof the chain would reject.

This binds served bytes to real USDC: stamping `B` bytes requires cumulatively claiming `>= B / 1_048_576` base units, restoring the proportional-cost assumption [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight) relies on. The floor is the only rate bound the protocol carries, and it is binding.
...
If the event subscription is lost and RPC polling fails, the node SHOULD continue operating with its last-known floor and log a warning. No service interruption is required. The worst-case consequence of a stale floor is that the node quotes below a raised floor and its claims are unredeemable — a revenue impact, not a safety violation.

A floor raise that lands mid-chain strands the whole chain, because `chunk_price` is signed for the chain's life and `redeem` reads the live `deliveryFloor` with no per-lane snapshot. The node abandons the chain and emits `RateFloorRaised` ([ADR 005 § `VoucherRejected` semantics](005-protocol.md#voucherrejected-semantics)) rather than accepting preimages it cannot redeem; the payer re-quotes and opens a fresh chain. The exposure is therefore one chain rather than one voucher, which is a reason to keep the 1-hour poll rather than to lengthen it. A raise never voids what the node already redeemed.
...
`signer` and `provider` are both in the signed payload: `signer` binds the voucher to the authorized key `redeem` validates against, and `provider` binds it to a single payee, so one node cannot redeem another node's voucher. `provider` is also what stops a released preimage from paying anywhere else: the preimage carries no payee, and its only route to settlement is a voucher that names one. `amount` and `bytesDelivered` are the lane's cumulatives.

`chainRoot` and `chunkPrice` are signed because both are repudiation surfaces otherwise. An unsigned root would let a payer disown a chain it authorized. An unsigned price would settle the chain at whatever governance rate happened to hold at redeem time, which can move between signing and redemption. `chainIndex` and `preimage` are deliberately **not** signed: they are the node's proof of what it delivered, supplied at redeem time, and signing them would reintroduce the per-chunk signature the chain exists to remove. `chain_length` is not signed either — it is always `MAX_CHAIN_LENGTH` ([Chain length and rollover](#chain-length-and-rollover)) — and `chunk_bytes` is not signed because it is a protocol constant ([Chunk Cadence](#chunk-cadence)). Keeping both out of the signed payload is about 20% less redeem calldata. The voucher carries no epoch: per-operator-epoch attribution is derived by `FeeRouter.routeSettlement` at redemption time as `epoch = uint64(block.timestamp / EPOCH_LENGTH)`, so a redemption's bytes credit the epoch its transaction lands in. A node redeems whenever it chooses within the capability `expiry` (the per-signer redemption deadline) and the grace window, so the practical bound on epoch-shifting is that `expiry` — a second-order effect on emission share that shrinks as the active-operator set grows.
...
Vouchers carry **no nonce**. A claim is ordered and replay-checked entirely by its cumulative value `claimed = amount + chain_index × chunk_price`. A node tracks the highest `claimed` it has accepted per lane and expects each new proof to advance it: a released preimage advances the index by one and `claimed` by exactly one `chunk_price`, and a fresh voucher advances `amount` and MUST NOT reduce `claimed` below the position the chain already proved. A proof whose `claimed` does not exceed the accepted cumulative is rejected (`AmountRegression`, [ADR 005](005-protocol.md#adr-005-wire-protocol)); that is the sole off-chain ordering/replay guard. Signers compute an anchor as `amount = ⌈bytes_delivered × chunk_price / CHUNK_BYTES⌉`, which prices a partial trailing chunk exactly instead of rounding a whole chunk up (see [Voucher Bytes-Delivered Field](#voucher-bytes-delivered-field)). On-chain redemption is likewise cumulative (`redeem` pays `claimed − paid`), so an already-redeemed claim, a stale voucher, a replayed preimage, or a re-presented index pays `0`, and the same proof can be re-presented after a top-up to collect a drained-pool shortfall — only the unpaid `claimed − paid` remains.
...
- **Committed voucher rate.** The advertised `rate_per_mb` in `ProbeResponse` / `StreamResponse` is a **floor**, not equality — nodes verify `amount_delta / bytes_delta >= rate_per_mb` on a voucher, and `chunk_price >= rate_per_mb` on the chain it opens ([Rate-floor enforcement](#rate-floor-enforcement)). Clients MAY commit at higher rates; nodes MAY use the committed rate as a per-stream priority key for **ordering admitted, in-flight streams** — it is a mid-stream signal (the first `Voucher` arrives only after `StreamResponse{ok: true}`), not an admission key — with the premium paid directly via [`FeeRouter.routeSettlement`](#feerouter-integration).
...
During delivery over `cdn/client/v1`, `{signature, amount, chain_root, chunk_price}` are transmitted on the wire; the remaining fields are derived from stream context — `poolId` from the `StreamRequest`, `signer` the bound client key, `provider` the delivering node, and `bytesDelivered` the node's per-lane cumulative byte counter. `chain_root` and `chunk_price` cannot be derived: the root is the payer's own secret-derived value, and the price is signed precisely so the node cannot substitute its own. Preimages travel separately from vouchers, as a distinct unsigned message. See [ADR 005](005-protocol.md#adr-005-wire-protocol) for wire protocol details.
...
- **Priced by the chunk, exact to the byte.** One chunk is `CHUNK_BYTES` = 1,048,576 bytes and prices at `chunk_price`, so a chain of `k` released chunks contributes exactly `k × chunk_price` and `k × CHUNK_BYTES`. A voucher's own `amount` is exact to one USDC base unit: `amount = ⌈bytesDelivered × chunk_price / CHUNK_BYTES⌉`. The round-up is on the price, not on the byte count, which is what lets a partial trailing chunk — or a whole transfer smaller than one chunk — settle at its true price instead of being rounded up to a whole chunk by the chain (see [Two payment resolutions](#two-payment-resolutions)). The voucher carries the byte count directly so the contract does not re-derive it.
- **The redeemed byte count folds the chain in.** `redeem` computes the byte delta against `claimedBytes = bytesDelivered + chainIndex × CHUNK_BYTES`, so bytes proved by preimage reach `bytesPerEpoch` on exactly the same footing as bytes proved by signature, and the rate floor binds both together.
