...
**The chain walk is bounded by construction.** The contract always hashes forward from the submitted preimage to `chainRoot`, never from a stored intermediate, so it stores no chain state and needs no root-matching branch. The cost is `chainIndex` hashes, capped at `MAX_CHAIN_LENGTH` (1024) — roughly 60k gas in the worst case, against a redemption that settles a whole epoch. Making the chain longer to amortize signatures further would raise this walk linearly, which is what fixes the ceiling at 1024 rather than at an arbitrary value.
