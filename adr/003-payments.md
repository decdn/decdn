...
A partial trailing chunk, or a transfer smaller than one chunk, settles through a fresh **amount-voucher**, redeemed at index 0. It does not settle through a preimage, because a preimage always advances the claim by a whole chunk. On a lane that meters no chain, the amount-voucher is sealed: its root is zero ([The sealed voucher (zero-hash root)](#the-sealed-voucher-zero-hash-root)). On a lane whose chain holds reveals, the amount-voucher folds that chain and rolls to a fresh root ([Chain length and rollover](#chain-length-and-rollover)). It still redeems at index 0. A 1.5 MiB transfer therefore pays one preimage for the first chunk plus one closing amount-voucher for the exact 1.5 MiB, and its redemption walks nothing.

| Transfer | Signatures | Walk at redemption |
| --- | --- | --- |
| Fits the credit window | 1 — deliver, then one closing amount-voucher | none |
| Large, payer closes cooperatively | 2, plus one per rollover — an opening voucher that carries `chain_root`, then a closing voucher that folds the chain and rolls to a fresh root | none |
...
**A floor refusal names itself to the proven pool owner.** `M` is node policy, not a protocol constant, so a client cannot size its reserve to every node's exact `M`. A client that under-estimates one node's `M` re-opens with a deposit the node still refuses, and a plain miss code dead-ends the fetch. So a node that refuses on the floor tells the proven pool owner the true reason. The floor gate is reached only past the lane-ownership proof: the request carries a client binding whose signer holds an owner-signed capability for the pool, or a registered on-chain signer. That party already reads the pool's on-chain `remaining`, so the true reason leaks it no balance it cannot compute. The node signs an `InsufficientDeposit` refusal in the open response, and the owner tops up the deposit and re-opens. Its reactive top-up loop funds toward the client's own working-deposit ceiling, which is the sole bound on how much a lying node makes the client escrow, so the signal is money-safe. An unauthenticated prober never reaches the floor gate; it is refused earlier as a plain miss (`NotFound`), which keeps a pool's balance unmappable off the wire (see [Probe fishing](#probe-fishing)).

**The floor refusal reveals an owner-only bound on `M`, not `M` itself.** An `InsufficientDeposit` refusal proves `remaining − M < window` is true. The owner knows `remaining`, so it learns an inequality bound on the node's private `M`. By topping up and re-probing it could binary-search toward `M` over several rounds. It never learns `M` outright. This probe costs the owner real deposits and is open only to a paying pool owner, not a free enumerator, so the residual is a slim, owner-only, self-funded leak. We accept it: naming the reason to the one party that can act on it — and only that party — is worth an inequality that party could already approximate.
...
1. **One preimage per whole chunk, per stream.** The payer releases one preimage on a stream for each whole chunk that it receives on that stream. Each preimage is the next index of the lane's one chain.
...
3. **Proof routing.** Vouchers and preimages advance one lane-wide claim, whichever stream carries them. The node pays each stream's outstanding chunks from that claim (see **A metering voucher pays no whole chunk** below).

**Example:** A signer has stream A (blob X) and stream B (blob Y) on the same lane. When A delivers a whole chunk, the signer releases the next preimage of the lane's chain on A. When B delivers a whole chunk, the signer releases the next preimage on B. A partial chunk at the end of a stream settles through that stream's closing voucher. The chunk size is a protocol constant, so the node and the signer agree on every boundary.
...
The payer sends a stream's root voucher immediately before that stream's first reveal, not at stream open. After a rollover, the payer sends the new-root voucher on **each** stream immediately before that stream's next reveal — not on one stream in the hope that the node propagates it. A stream that makes no reveal sends no anchor voucher: its closing voucher is its only voucher. QUIC orders messages within a stream, so each stream's preimages for the old chain are read against that stream's own old root before that stream's own roll voucher arrives. A fast stream that has already adopted `R₂` cannot invalidate a slower sibling still finishing `R₁`. Nothing needs a rendezvous, and the payer never has to know how far the node has processed any stream.
...
**A metering voucher pays no whole chunk.** The node pays each outstanding chunk of a stream from lane headroom. Lane headroom is the proved bytes that no stream has claimed. A reveal, a sealed voucher, and a voucher that settles a partial closing chunk take bytes from headroom. A voucher that carries a live `chain_root` does not take a whole chunk. It advances the watermark and anchors its stream, and the stream waits for its reveal. The reason is the open, re-anchor, and rollover vouchers. Such a voucher can fold a reveal that a sibling stream carries and that the node has not read. If the voucher took the chunk, the sibling's reveal would find no headroom. The sibling would then wait for a proof that the payer never sends.

**A chunk stays owed until proofs pay all of it.** A proof takes at most the headroom it finds, so it can pay part of a chunk. The rest of the chunk stays owed, and it still counts against the credit window. The stream reads more proofs for it. The node reads a limited number of proofs for one chunk. A payer that uses them up without paying the chunk ends the stream. The chunk's delivered size decides whether a metering voucher may pay it, not the part that is still unpaid. A whole chunk that a proof paid in part is still a whole chunk, so a metering voucher pays none of it.

An honest payer never gets a partial credit. Each of its proofs claims every byte that the payer's earlier proofs on the lane claim, plus the proof's own chunk. So when the node reads the proof, the lane headroom covers that chunk. A partial credit comes only from a payer that claims less than the bytes it received. The stream then waits for the rest, and it stops if the proof budget runs out first. After a stream's last reveal, the payer sends only that stream's closing voucher on it. It sends no open, re-anchor, or roll voucher there. This keeps a closing partial chunk from taking the headroom of a sibling's proof that the node has not read.
...
| `Underpaid` | the span the voucher adds over the node's accepted watermark pays below the quoted `rate_per_mb` | none on-chain — the contract settles whatever cumulative the signer signed, so the node refuses to credit the span before it delivers more (see [Voucher withholding](#voucher-withholding)) |

Surfacing these reasons off-chain saves both parties the gas of a doomed on-chain submission and gives the payer enough detail to recover (refresh state and re-sign for `AmountRegression`, raise the cap for `SpendingCapExhausted`, issue a fresh capability for `CapabilityExpired`, top up the deposit for `PoolExhausted`, resend the epoch's root voucher on that stream for `UnanchoredPreimage`, rebase once to the returned watermark for `Underpaid`) instead of an opaque connection drop. A delegated signer — one issued a capped capability by a pool owner — holds no funds to `topUp` (owner-only) and cannot read its lane watermark from chain until a redemption records it, so the node attaches an authenticated watermark bundle to the regression rejections and to `Underpaid` for self-heal, and defers a genuine top-up or cap raise to the owner (see [ADR 005 § `VoucherRejected` semantics](005-protocol.md#voucherrejected-semantics)). Riding in-band rather than via a QUIC stream reset preserves the reason for client retry logic without burning [ADR 013](013-schema-evolution.md#adr-013-schema-evolution) application-error-code numbers for the structured-response case.
...
The node checks that each voucher pays the quoted rate for the span it adds over the lane's accepted watermark. A voucher that pays less gets `Underpaid`. The node credits nothing for the span and ends the stream. The rejection carries the lane's accepted watermark. An honest payer gets `Underpaid` only when its own watermark has run ahead of the node's: it signed vouchers that the node never accepted, and it recorded them as paid. The payer verifies the watermark against its own signature, rebases its lane to it once, and resumes. The rebase costs the payer nothing, because redemption pays the highest cumulative the node holds. Without the watermark, the lane stays blocked: every later voucher measures its span from a watermark the payer does not have.

For a funding or authorization fault, a node rejects a voucher for one of three distinct reasons, and reports which one. `PoolExhausted` means the pool's on-chain remaining balance cannot cover the stream mid-flight; the owner must top up the deposit, and the node signals this only mid-stream, after it has already accepted the stream (see [Off-chain Voucher Rejections](#off-chain-voucher-rejections-wire-encoding)). `SpendingCapExhausted` means the signer's delegated cap is used up; the owner must raise the cap. `CapabilityExpired` means the signer's capability has passed its `expiry`; the owner must re-mint a fresh capability. An open-time floor refusal splits by audience. To an unauthenticated prober it stays the wire-level `NotFound` used for any unservable request, so the prober cannot distinguish "no such content" from "this pool cannot pay," which is the anti-enumeration property this ADR relies on elsewhere. To the proven pool owner — a request that passed the lane-ownership proof — the node instead signs an `InsufficientDeposit` refusal, so the owner tops up and re-opens rather than dead-ending on an ambiguous miss (see [A floor refusal names itself to the proven pool owner](#pool-solvency-and-the-refundable-floor-m)). The owner learns only an inequality bound on `M`, which it already reads `remaining` to approximate, so the carve-out leaks nothing a prober could use. `PoolExhausted` is the one reason that necessarily surfaces mid-stream and post-auth, because only a live stream can drain a pool below its floor while service is already underway.
...
An owner reconciling its pools after a restart reads its own `ownerPoolNonce` and recomputes each `poolId` (ids are derived per nonce, so there is no separate counter to drift), then re-hydrates each via `getPool`. A node does not enumerate pools from chain state — a pool names no provider — so it reconstructs its lanes from its own persisted `PoolStateStore` ([Off-chain voucher state persistence](#off-chain-voucher-state-persistence)) and confirms each on-chain via `watermark[poolId][signer][provider]`. It reads `getWatermark` for a single lane. It reads `getWatermarks` for a batch of lanes in one call. A signer learns the `poolId` and its cap from the capability the owner issued it.
...
| `setEpochLiquidityCapFraction(bps)` | Governance: per-epoch USDC liquidity cap as a fraction of epoch-start pool depth (bounded `[1%, 30%]`, [ADR 018](018-liquidity-strategy.md#adr-018-liquidity-strategy-uniswap-v3-5050-pol)). | `GuardedBuybackBurner` |
...
| `setSwapRouter(addr)` / `setPool(addr)` | Governance: rotate the swap router or the pool. | `BuybackBurnerUniswapV3` |

The members above are split across the three-layer `BuybackBurner` hierarchy — the abstract base, the `GuardedBuybackBurner` guard layer, and the concrete `BuybackBurnerUniswapV3` subclass — as the *Declared on* column records; there is no single flat contract. [ADR 026 § FeeRouter split](026-tokenomics.md#feerouter-split) defines the economic parameters and the 30% router-fed inflow source. [ADR 018](018-liquidity-strategy.md#adr-018-liquidity-strategy-uniswap-v3-5050-pol) specifies the venue (Uniswap V3 `SwapRouter02` + 50/50 pool) and how `setSwapRouter` / `setPool` are configured at deployment. **Integration note:** `setSwapRouter` holds the `SwapRouter02` address. The Router pulls input tokens through a plain ERC20 allowance, so the burner approves the Router itself, scoped per swap and reset to `0` after each swap, so no standing allowance survives. [ADR 018 § Buyback execution via Uniswap V3](018-liquidity-strategy.md#buyback-execution-via-uniswap-v3) is authoritative for the mechanism.
...
Clients without on-chain registration MAY include a signed binding in their `StreamRequest` to attest a NodeId↔Ethereum-address mapping for the connection's lifetime. The node verifies the EIP-712 signature over `BindNodeId(nodeId, nonce=0)` by recovering the signer from the fixed 65-byte form and comparing it against the claimed address. A smart-account binding is rejected fail-closed, so a client binds and signs with an EOA; a Safe funder is served through a capability-delegated EOA `signer` instead ([ADR 024 § Off-Chain Signature Verification — EOA Recovery Only](024-account-abstraction.md#off-chain-signature-verification--eoa-recovery-only)). The verified address is cached for the connection's lifetime and acts as a gate, not as an attribution source: the node refuses a request whose voucher `signer` is not the bound address, before any bytes are delivered, since that signer's vouchers would fail verification against the address it authenticated as. The address a voucher must recover to is the capability-authorized `signer` for the pool ([§ PaymentPool](#paymentpool)). This ephemeral binding is not stored on-chain and is valid only for the session. Wire-format details are in [ADR 005](005-protocol.md#client-identity-binding).
