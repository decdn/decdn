...
**Key rotation is the recovery path.** `dead_charge` is permanent and clears only when the pool is reclaimed on-chain, and it counts against the signer's cap. Under a bare share, an owner top-up raises the headroom and so raises the cap, which lets a flaky-but-honest signer recover. Under the window ceiling it does not: a key that accrues `k` windows of abandoned-stream loss over the pool's lifetime stays refused. The protocol already supplies the remedy — capabilities carry an expiry and are issued per session (see [Revocation](#revocation)), so the publisher issues the client a fresh key and the spent lane retires with the old one. The node keys `dead_charge` by signer, so a new key starts with a clean cap. This is deliberate: it needs no decay schedule, no split live and dead budgets, and no per-signer history for the node to keep.
...
| `PoolExhausted` | the pool's remaining deposit, minus the refundable floor `M` and already-committed concurrent floor credit, can no longer fund further credit — either pool-wide or for this signer's share | none on-chain — a node-side mid-stream solvency stop, not a voucher defect |
