...
| `0x10` | `RATE_LIMITED` | Connection rejected by the per-source or global rate limiter (issue #235). Delivered via `CONNECTION_CLOSE` immediately on accept. Peers that receive this code SHOULD back off before reconnecting; they MUST NOT treat it as a protocol error. |
