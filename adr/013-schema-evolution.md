...
- Varint length prefix adds 1–5 bytes per message. Negligible against a `ChunkData` payload, which a sender sizes at up to `MAX_MESSAGE_SIZE`; ~5% on a `ProbeRequest` (~40 bytes), which is small in absolute terms
