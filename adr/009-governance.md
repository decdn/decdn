...
| Publisher vetting timelock | TimelockedGovernanceVettingPolicy | 24 hours | 14 days |
...
- **Publisher vetting timelock 24 hours–14 days:** Applies when the `TimelockedGovernanceVettingPolicy` is the installed vetting policy (see [ADR 011 § Vetting policies](011-content-takedown.md#vetting-policies)); the genesis `ManualVettingPolicy` has no timelock. Lower bound gives governance and the broader community at least one full business day to surface concerns about a publisher before it can seat origins. Upper bound prevents governance from making vetting effectively unusable through delay.
