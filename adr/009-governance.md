...
**Transition.** The bootstrap-multisig phase ends when the bootstrap multisig executes a one-shot `transitionToGovernor` setter — a manual decision it takes when it judges the operator set broad and diverse enough for operator-weighted DAO voting to be safe. The setter revokes the bootstrap-multisig's `GOVERNANCE_ROLE` across the role-gated contracts and transfers it to the `TimelockController` controlled by `DecdnGovernor` proposals. The Governor's vote-weight source (`FeeRouter.bytesInWindow` + `CapacityBond.firstBondedAt` + `CapacityBond.slashedAtEpoch` per [ADR 036 § Formula](036-served-bytes-voting-weight.md#formula)) is wired at deployment, not at transition. The setter cannot be reversed; the bootstrap-multisig cannot be reinstated.
