...
| Rate floor/ceiling (per stablecoin) | StablePaymentChannel | Floor ≥ 1 base unit | Ceiling > floor |
...
- **Rate floor ≥ 1 base unit:** A zero floor allows free-riding nodes that advertise zero rates to attract traffic without generating protocol fees. The minimum of 1 base unit of the payment token (e.g., $0.000001/MB for 6-decimal USDC) is negligibly small but prevents true zero-rate abuse. At launch this is 1 USDC base unit; `addToken` enforces a per-token floor ≥ 1 base unit at stablecoin registration time ([ADR 003](003-payments.md)).
