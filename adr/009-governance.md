...
- **MMR retention buffer 1 day–30 days:** Per [ADR 027 §4](027-distinct-client-receipts.md#per-epoch-summary), `FeeRouter` retains the per-(`operator`, `epochId`) MMR `peaks` and `leafCount` entries for the §Challenge window duration (7 days) plus this buffer; after the combined window, entries are zeroed for refund eligibility and on-chain leaf-level inclusion proofs are no longer reconstructable. Lower bound of 1 day gives a late-arriving challenge submitted at the end of the §Challenge window a same-day resolution path on-chain. Upper bound of 30 days bounds long-term L2 storage growth: combined retention (7-day challenge window + 30-day buffer ≈ 37 days) maps to ~5 in-flight epochs per operator at the 7-day epoch length pinned by [ADR 026 §2 Epoch mechanics](026-gauge-boost-tokenomics.md#2-feerouter-split-40407553), which is the practical ceiling under the chosen L2's storage-cost schedule. Challengers needing post-retention resolution MUST cache `peaks` off-chain before submission; this is the documented workflow.
