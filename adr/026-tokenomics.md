...
Every operator must bond TOKEN proportional to the bandwidth capacity it declares. The bond is the only TOKEN-side requirement on operators. There is no optional lock for additional yield. The global `minBond` entry floor also applies: entry needs a bond of at least `max(minBond, bond_required(declaredMbps))` (see [§ Minimum bond is non-retroactive](#minimum-bond-is-non-retroactive)).
...
- `CapacityBond.bond(amount)` deposits the bond (emitting `Bonded`) and `CapacityBond.declareMbps(declaredMbps)` self-attests capacity (emitting `MbpsDeclared`). Declared capacity is operator-self-attested; it is not verified at registration. Vote weight, per [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight), is sourced from `FeeRouter.bytesInWindow` (proven delivered bytes), capped per epoch at what the operator's declared capacity could physically deliver over that epoch. Over-declaration still buys no weight — the cap only ever narrows counted bytes, never widens them — so the bond cost remains the primary structural disincentive against tier inflation. Under-declaration does cap weight: an operator that declares less capacity than it delivers has its vote weight clamped to the declared tier for that epoch.
...
**Distribution mechanism.** 20% unlocked at TGE, then 48-month linear (monthly) into a dedicated Timelock-controlled multisig (separate wallet from operational Treasury for accounting cleanliness). No on-chain contract; both sub-programs are Treasury-multisig-administered.
...
**Auto-ejection.** At 50% of the operator's grandfathered activation floor (`bondFloorAtActivation`). This slash-driven auto-ejection is **recoverable** — re-bonding back to the live `minBond` and re-registering clears the `ejected` flag. It does **not** override a concurrent governance blacklist, which sets a separate `blacklistEjected` latch that re-bonding cannot clear (see [ADR 011 § Hash Evasion and Origin Blacklisting](011-content-takedown.md#hash-evasion-and-origin-blacklisting)).

### Minimum bond is non-retroactive

`minBond` is the global entry floor. It gates only the entry transitions — `bond` reinstatement and `registerNode` — never ongoing active status. Each time an operator's active bond crosses up through `minBond` in `bond`, the contract records the live `minBond` as its `bondFloorAtActivation`. Ongoing active status (`isActive`) and slash auto-ejection read that recorded floor, not the live `minBond`.

Governance may tighten `minBond` within the fixed `[10K, 1M TOKEN]` rail as the network matures. A raise applies to new entrants only. It never de-activates an already-active operator whose bond falls below the new floor, and never eases an active operator's slash-ejection threshold — the electorate is not purged by a parameter change. Only the operator's own bond reduction below its floor (a partial unbond) or a real slash below `bondFloorAtActivation / 2` de-activates it. An operator that re-enters after ejection or deregistration records the then-current `minBond` afresh.
...
|---|---|---|
| Voting source | `FeeRouter.bytesInWindow` capped per epoch by `CapacityBond.declaredMbpsAtEpoch`, + `FeeRouter.totalBytesInWindow` × `age_ramp(CapacityBond.firstBondedAt)`; zeroed for `windowEpochs` after slash via `CapacityBond.slashedAtEpoch` | [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight) |
...
**Bond is the only TOKEN-side requirement.** There is no optional lock for additional yield. At entry, the bond must meet the larger of the curve and the global `minBond` floor (see [§ Minimum bond is non-retroactive](#minimum-bond-is-non-retroactive)).
...
- **Smaller contract surface.** The contract set is `CapacityBond` (operator registry, capacity-curve bond, escrow-on-slash), `FeeRouter` (three-bucket same-tx split), `SlashAppeal` (slash-appeal state machine), `BuybackBurner`, `DecdnGovernor`, `TimelockController`, `PaymentPool`, and `TOKEN`. No standing insurance reserve, no on-chain operator-credit grant/vest machinery, no epoch / claim / snapshot machinery, no separate emissions contract.
...
- **Wash-trading is structurally defeated.** Operator revenue is per-byte at the `FeeRouter` (the operator base is paid by the client, not pooled), so faking traffic does not raise revenue. Governance vote weight is sourced from `FeeRouter.bytesInWindow` per [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight) — proven delivered bytes, capped per epoch by declared capacity — so over-declaring a tier does not translate into governance influence either; the cap only narrows counted bytes, it never grants weight for capacity that was declared but not delivered. The super-linear bond curve makes over-declared capacity a dead-capital drag with no governance or revenue upside.
