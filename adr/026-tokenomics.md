...
- Declared capacity is clamped to `[MIN_CAPACITY_PER_OPERATOR, MAX_CAPACITY_PER_OPERATOR]`: `register(declaredMbps)` reverts on a declaration below the floor or above the ceiling, so every registered slot carries at least `bond_required(MIN_CAPACITY_PER_OPERATOR)` of slashable bond.
