...
The 10pp POL position is "in the pool" but not floating in the sense of being available to circulate — it sits as a treasury-owned LP position, removable only by governance. The DAO Treasury (30% / 45M) and App Incentives (20% / 38M) TGE unlocks are likewise unlocked-but-treasury-custodied — released into Timelock-controlled wallets and spent into the ecosystem, not sold — so they do not add to sell-side float either.
...
- **Full exit runs through deregistration.** `requestUnbond`'s floor is `bond_required(declaredMbps)`, and `declareMbps` cannot return a declared tier to 0 (the band floor `MIN_CAPACITY_PER_OPERATOR` bars `declareMbps(0)`), so the declared tier is what pins the residual bond. `deregisterNode` therefore clears `declaredMbps` — emitting `MbpsDeclared(operator, old, 0)` so tier indexers stay in sync — which is what makes the full-exit path [ADR 003 § Node Registry](003-payments.md#node-registry) describes actually reachable. The complete sequence is `deregisterNode()` → `requestUnbond(activeBond)` → 14-day window → `unbond()` (with a request already in flight, drain it with `unbond()` first — `requestUnbond` reverts `UnbondingInProgress` while one is pending). Deregistration does not move the bond, so an operator who re-registers instead of exiting needs no new funds — but must re-declare their tier.
  **`deregisterNode` is the only writer that clears the tier, and it requires an active node** (it reverts `NodeNotActive` otherwise). `declareMbps` carries no such gate, so a tier can be declared in states from which it can never be cleared, and in each the operator stays pinned at `bond_required(declaredMbps)`:
  - **Bonded and declared but never registered** — the state `decdn node bond --mbps N` leaves behind on its own. Releasing the floor means completing a `registerNode` purely in order to deregister.
  - **Auto-ejected on slashing, blacklist-ejected, or displaced by `reclaimNodeId`** — all deactivate without clearing the tier. Where a slash has already taken the active bond below the floor, *none* of the remainder is withdrawable until the operator re-bonds over the curve and re-registers, purely in order to deregister; for a blacklist-ejected operator `registerNode` reverts until governance calls `unEjectNode`.

  This asymmetry is a known gap rather than design intent. It is not a new one — before `deregisterNode` cleared the tier the floor was permanent for *every* operator — but whether the involuntary paths should also release it is a slashing-economics question, not a mechanical fix: it would let a just-slashed operator begin withdrawing their whole remaining bond immediately. Tracked in #1361.
...
There is **no dedicated on-chain credit and no earmarked allocation** for pre-launch testnet operators. The earlier "Genesis Bond Credit" mechanism (a Treasury-funded, on-chain `CapacityBond` grant with a 24-month vest) is removed; its 5pp folds back into the operational DAO Treasury (group 2, now a flat 15% — 30% at TGE, then 48-month linear). Any retroactive recognition of verified testnet contribution is a **discretionary off-chain Treasury TGE-unlock distribution** — liquid TOKEN released to recipients on a cliff/linear schedule the foundation/Treasury sets — with **no protocol contract surface**. Removing the on-chain mechanism keeps the contract set smaller and sidesteps the on-chain grant/vest/slash interactions entirely; early-operator unit economics are instead carried by the off-chain USDC infrastructure subsidies in [§ Bootstrap mechanism — pre-seed USDC](#bootstrap-mechanism--pre-seed-usdc).
