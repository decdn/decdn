...
**Slashing.** The full pending credit (`pendingCredit.total - pendingCredit.vested`) is slashable on the same terms as the operator's voluntary bond. Slashed credit joins the stake-slash total in escrow-on-slash and is distributed / refunded at finality per [§ Slashing and burn](#slashing-and-burn). Vested-but-unclaimed credit is no longer slashable — it has functionally become voluntarily-bonded TOKEN; this is reflected in [ADR 028](028-slashing-appeals.md#adr-028-slashing-appeals-and-dispute-escalation).
