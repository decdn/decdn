...
- **Bond management:** The Safe holds TOKEN and executes `CapacityBond.bond(...)` + `declareMbps(...)` (bond deposit + capacity attestation), `registerNode()` (NodeId binding), `deregisterNode()` (deactivate), and `unbond()` (start the 14-day unbonding window).
