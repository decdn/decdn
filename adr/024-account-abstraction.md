...
1. **High-frequency signing needs a hot key.** At the default 1 MB voucher cadence, a 100 MB download requires 100 EIP-712 signatures; nodes sign a `slash_sig` on every `ProbeResponse` / `StreamResponse`. Hardware-wallet-only operation is infeasible at that cadence regardless of wallet model. Prior work explored a derived hot key (HKDF from hardware wallet signature, [ADR 012](012-client.md)) and a payment-channel-level `setDelegate`/`clearDelegate` (PR 196); both are narrow workarounds.
...
- **PoC:** solve problems 2 and 3 now — adopt `SignatureChecker` across every contract (problem 3) and make Safe the recommended wallet for node operators and clients (problem 2). Leave problem 1 alone: the PoC accepts a software-held signing key on the signing host — either a Safe owner key or a plain EOA — matching today's `eth_keystore` trust posture. Multisig still protects stake/withdraw/channel-open paths; only the high-frequency signing path uses a single hot key.
- **Production:** solve problem 1 cleanly by migrating to ERC-7579 smart accounts (Safe via the Safe-7579 adapter) and installing [`erc7579/smartsessions`](https://github.com/erc7579/smartsessions) — a standardized session-key module with native ERC-1271 validation, policy enforcement (time windows, allowed selectors/domains, spending caps), and first-class revocation. This supersedes both the derived-hot-key scheme and PR 196's `setDelegate`/`clearDelegate`.

Deferring session keys to Production lets the PoC PR stay small, avoids committing to bespoke security-critical contract code, and takes the standardized ERC-7579 path for the long-term design.
...
**Recommended PoC configuration: 1-of-1 Safe** (single hot owner). **Production configuration: 2-of-3 Safe + session keys** (see §3).

- **Owners (PoC):** 1-of-1. The single owner is a software-held key on the signing host — the same trust posture as today's `eth_keystore`. 2-of-3 is not recommended for the PoC because no multisig threshold can be reached at wire speed without session keys: `SignatureChecker` → Safe's `checkSignatures` rejects a single-owner signature when threshold > 1, and pre-approving every `slash_sig` digest via `signMessage` is infeasible.
- **Stake management:** The Safe holds TOKEN and executes `StakingRegistry.stake()`, `registerNode()`, `deregister()`.
- **Channel operations:** The Safe executes `openChannel()`, `closeChannel()`, `topUp()` for outbound node-to-node payment channels (cache-miss pulls). Infrequent — not a hot-signing path.
- **Hot signing (slash_sig) — PoC:** The 1-of-1 Safe owner signs the EIP-712 digest directly; `SignatureChecker.isValidSignatureNow(safeAddress, digest, sig)` routes through the Safe's stock `CompatibilityFallbackHandler` → `checkSignatures` (which passes because threshold is 1 and the one owner signed).
- **Hot signing (slash_sig) — Production:** 2-of-3 Safe with a session key authorized via `erc7579/smartsessions` (see §3). Multisig protects stake/withdraw; the session key signs `slash_sig` at wire speed without needing owner quorum per message.

#### Clients

**Recommended PoC configuration: 1-of-1 Safe or plain EOA.** High-value client accounts in Production migrate to 2-of-3 + session keys (see §3); the same threshold constraint that applies to node operators applies here.
...
- **Voucher signing — PoC:** The client process signs EIP-712 vouchers with its owner key directly; `SignatureChecker` on the channel contract validates the signature against `channel.client` (EOA → ECDSA; 1-of-1 Safe → `checkSignatures` via stock handler).
...
**Session keys are out of scope for the PoC.** The PoC relies on the direct owner-signature path described in §2 (1-of-1 Safe or EOA, software-held owner key on the signing host). This matches today's `eth_keystore` trust posture and keeps the PoC from shipping bespoke security-critical contract code.
...
- **Session-key module:** [`erc7579/smartsessions`](https://github.com/erc7579/smartsessions) — a standardized ERC-7579 session-key module that natively implements ERC-1271 `isValidSignature` for session-key-authorized digests. Because the module is an ERC-7579 *validator*, `isValidSignature` on the Safe-7579 account routes through it without any custom fallback handler on deCDN's side; `SignatureChecker.isValidSignatureNow(safe, digest, sessionKeySig)` returns the ERC-1271 magic value once a session is enabled.
- **Policies:** smartsessions' existing policy system covers what the PoC would otherwise have had to invent:
  - *Time windows* (`validAfter` / `validUntil`) for both node and client session keys.
  - *Action policies* scoped to specific selectors / EIP-712 domains (e.g., a node session limited to signing `slash_sig`-shaped digests; a client session limited to `StablePaymentChannel` voucher digests).
  - *Spending caps* per session (useful for clients to bound total voucher value per session key).
  - *First-class revocation* via `removeSession` — no on-chain payment-channel tx required.

**Production authorization flow (sketch, not specified in detail here — a follow-up ADR will pin down the exact policy encodings):**

```
Node/client Safe (2-of-3) owners →
...
#### Why smartsessions rather than a bespoke module

PR 196's `setDelegate`/`clearDelegate` was a contract-level workaround in `StablePaymentChannel` for one use case. A bespoke Safe module + fallback handler (explored in an earlier draft of this ADR) would work but ships new security-critical code for a problem that is already solved upstream. smartsessions wins on every axis that matters:

| Aspect | PR 196 delegation | Bespoke Safe module (rejected) | `erc7579/smartsessions` (Production) |
| --- | --- | --- | --- |
| Contract surface owned by deCDN | New functions in `StablePaymentChannel` | Custom module + fallback handler | None — standardized upstream |
| ERC-1271 wiring | N/A (not used) | Custom fallback handler required | Built into the ERC-7579 validator path |
| Scope / policy language | None | Bespoke | Rich, composable, audited |
| Revocation | On-chain `clearDelegate` | Custom call | `removeSession` |
| Multi-sig approval of the session | Not built in | Custom | Native (owners call `enableSession`) |
| Works with any ERC-7579 account | No | Safe-only | Yes (node/client wallet-agnostic) |
...
- The PoC ships no bespoke Safe modules or custom fallback handlers — the stock `CompatibilityFallbackHandler` + `SignatureChecker` are sufficient for 1-of-1 Safes and EOAs.
- The Production path adopts a standardized, audited session-key module (`erc7579/smartsessions`) rather than deCDN-owned security-critical contract code.
...
- In the PoC, hot-signing parties (nodes, clients) use 1-of-1 Safes (or EOAs). Multisig protection of the high-frequency signing path is deferred to Production with session keys; 2-of-3 is only recommended for parties whose operations stay on the infrequent stake/withdraw/channel-open paths.
...
- Production rollout requires coordinated migration to Safe-7579 and smartsessions on every participating wallet; operators running on the PoC path must rotate to the new account type before turning off the direct owner-signing fallback.

## Amendments to Existing ADRs

This ADR amends the following:

- **[ADR 003](003-payments.md):** `SignatureChecker` replaces `ECDSA.recover` in voucher verification and `bindNodeId`. "Gasless Channel Opens" section updated — ERC-1271 is PoC, paymaster deferred. PR 196's `setDelegate`/`clearDelegate` amendment is superseded (resolution deferred to Production via `erc7579/smartsessions`, not a contract-level delegate).
