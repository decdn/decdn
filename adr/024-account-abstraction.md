...
1. **High-frequency signing needs a hot key.** At the fixed 4 MiB voucher granularity, a 100 MB download needs 25 EIP-712 signatures. A hardware wallet requires a physical confirmation (2–5 seconds) per signature, and nodes sign a `slash_sig` on every `ProbeResponse` / `StreamResponse`. Hardware-wallet-only operation is infeasible at this signing frequency, whatever the wallet model. [ADR 012](012-client.md#adr-012-client-architecture-bootstrap-and-trust-model) identified this and explored a derived hot key (HKDF from a hardware-wallet signature) as a narrow workaround.
