...
1. **Unattended signing needs a hot key.** Nodes sign a `slash_sig` on every `ProbeResponse` and every `StreamResponse`, at request rate and with no human in the loop. A hardware wallet requires a physical confirmation (2–5 seconds) per signature, so hardware-wallet-only operation is infeasible for a node whatever the wallet model. [ADR 012](012-client.md#adr-012-client-architecture-bootstrap-and-trust-model) identified this and explored a derived hot key (HKDF from a hardware-wallet signature) as a narrow workaround.

   On the **payer** side the raw rate is no longer the binding constraint. Payment signatures are per chain epoch, not per delivery interval: at the default 1024 × 64 KiB an epoch covers 64 MiB, so a 100 MB download costs two signatures rather than a hundred ([ADR 003 § Chunk Cadence](003-payments.md#chunk-cadence)). What remains is that the signatures are still *unattended* and still scale with **lanes** rather than bytes — a client fanning out across many nodes opens an epoch per lane — and a background or headless fetch has nobody to confirm them. So the case for a session key holds on the unattended-operation argument alone, and no longer rests on a per-megabyte signing rate.
...
**Signature validation and the accounting layer are separate concerns.** Account abstraction and session keys operate on signature *validation* — *who* counts as a valid signer, under what policy (ERC-1271, session keys, caps, expiry). They do not, on their own, give one pool multiple concurrent independent signers: that is a property of the payment *accounting* layer, and it is provided by the two-dimensional sharded register in [ADR 003](003-payments.md#redemption-and-close) (`authorized[poolId][signer]` plus `watermark[poolId][signer][provider]`), not by any signature scheme. AA improves the *key*; the sharded register is what lets many keys draw from one pool with independent ordering and per-signer caps. The two compose: a session key is one valid `signer`, and the register is what runs many such signers in parallel on a single deposit.
...
- **Voucher signing:** EIP-712 vouchers are signed by a capability-authorized `signer`, which the owner delegates (and which defaults to the owner itself). On-chain, `SignatureChecker` validates against it (EOA → ECDSA; 1-of-1 Safe → `checkSignatures` via the stock handler). **Off-chain, nodes verify vouchers and `BindNodeId` ephemeral bindings by recovery only, against that signer** — so a signer that is a smart account cannot be served, and its binding is rejected fail-closed ([§ Off-Chain ERC-1271 Verification](#off-chain-erc-1271-verification)).
- **A smart-account funder is servable today.** Because the owner and the signer are separate roles, a Safe can fund a pool and issue a capability to a plain EOA `signer`. The Safe custodies the deposit and receives the reclaim; the vouchers recover by `ecrecover` and clear the off-chain path unchanged. The residual gap is narrower than a wallet choice: it is a capability that names a smart account as the *signer*. The session-key path — signing at delivery speed via `erc7579/smartsessions` without exposing the Safe owner key — lands with the Production plan below, and is what a smart account signing for itself needs.
...
- Hot-signing parties (nodes, clients) hold a single software-held key. Multisig protection of the high-frequency signing path is deferred to Production; a multi-owner Safe is viable only on the infrequent stake/withdraw/pool-open paths, for parties who choose that split themselves.
