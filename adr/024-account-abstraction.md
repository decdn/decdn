...
- **Bond management:** The Safe holds TOKEN and executes `CapacityBond.bond(...)` + `declareMbps(...)` (bond deposit + capacity attestation), `registerNode()` (NodeId binding), `deregisterNode()` (deactivate), `requestUnbond(...)` (start the 14-day unbonding window), and `unbond()` (reclaim the tokens after the window).
