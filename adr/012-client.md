...
- Discovers nodes from the on-chain registry and probes candidates over `cdn/probe/v1`; speaks no `cdn/dht/v1` — content discovery is node-side, on the serving node's cache-miss pull leg, which bounds the DHT requester population by node count (see [ADR 022](022-content-discovery.md#adr-022--content-discovery-at-scale))
