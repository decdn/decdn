...
- **PoC:** An encrypted keystore file at `~/.decdn/eth_keystore` (Web3 Secret Storage format, EOA) — the documented default. A Safe or other smart-account address is accepted by every contract, since all contracts use `SignatureChecker`, but it cannot be served: the node verifies bindings and vouchers off-chain by recovery only ([ADR 024 § Off-Chain ERC-1271 Verification](024-account-abstraction.md#off-chain-erc-1271-verification)).
