...
**Mobile clients (iOS, Android) and web clients (browser) are out of scope.** They require fundamentally different choices for key custody (platform secure enclave / WalletConnect rather than filesystem keystore), transport (WebTransport rather than raw QUIC; no UDP on browsers), and storage (platform sandbox rather than `~/.decdn/`). The voucher-signing UX (a hot EOA signing at delivery speed, per [ADR 003 § PaymentPool](003-payments.md#paymentpool)) and the bootstrap procedure (registry RPC) both assume desktop-class capabilities.
...
Node-side registry interaction and bootstrap is in [ADR 019 § Step 3.2](019-node-onboarding.md#step-32--build-initial-node-view-from-on-chain-registry).
...
- **Storage:** An encrypted keystore file at `~/.decdn/eth_keystore` (Web3 Secret Storage format, EOA) — the documented default. A Safe or other smart-account address is accepted by every contract, since all contracts use `SignatureChecker`. A smart account cannot be *served* as a voucher `signer` — the node verifies bindings and vouchers off-chain by recovery only ([ADR 024 § Off-Chain Signature Verification — EOA Recovery Only](024-account-abstraction.md#off-chain-signature-verification--eoa-recovery-only)) — but it can own a pool and issue a capability to an EOA signer, in which case only the EOA is ever recovered and its vouchers are servable.

**High-frequency voucher signing needs a hot key.** A hash chain meters delivery between signatures ([ADR 003 § Hash-chain metering (PayWord)](003-payments.md#hash-chain-metering-payword)), so a 100 MB download needs two EIP-712 voucher signatures rather than one per interval. The frequency argument rests on the residual signatures — one per 255 MiB rollover, one per stream open, and one per close — and on holding a key hot at all. Hardware wallets require physical confirmation per signature (2–5 seconds each), which is infeasible.

The capability-authorized signer of [ADR 003 § PaymentPool](003-payments.md#paymentpool) fully answers this on the client side, without any account-abstraction machinery. A fresh EOA issued a capability is scope-limited to that pool's vouchers, bounded by its `spending_cap`, retired by its `expiry`, and needs no owner confirmation per signature — scope-limiting, a spending bound, *and* time-bounded revocation, more than an immutable per-channel pin gives. A high-value client keeps its owner key (EOA or Safe) cold on the deposit and delegates a hot EOA signer for delivery-speed signing. The client side has no residual that a smart account signing for itself would need; that residual is operator-only ([ADR 024 § Unimplemented — Operator Custody While Serving](024-account-abstraction.md#unimplemented--operator-custody-while-serving)).

#### Key Summary

| Key | Algorithm | Storage | Custody split | Rotation |
| --- | --- | --- | --- | --- |
| iroh identity | Ed25519 | `~/.decdn/iroh_key` (0600) | — | New key + reconnect |
| Ethereum | secp256k1 | `~/.decdn/eth_keystore` (encrypted) | Optional: cold owner (EOA or Safe) delegates a hot EOA voucher signer ([ADR 003 § PaymentPool](003-payments.md#paymentpool)) | Wallet-level; signer retired by capability `expiry` |
...
1. **Startup:** Load iroh key (→ `NodeId`). Load the Ethereum key from the keystore (EOA). A high-value client loads a cold owner key and delegates a hot EOA voucher signer ([ADR 003 § PaymentPool](003-payments.md#paymentpool)); the signer is what signs at delivery speed.
...
4. **Session:** The node verifies the signature by recovering the signer from the fixed 65-byte form and comparing it against the claimed address; a smart-account binding is rejected fail-closed, so a client binds and signs with an EOA ([ADR 024 § Off-Chain Signature Verification — EOA Recovery Only](024-account-abstraction.md#off-chain-signature-verification--eoa-recovery-only)). It then caches the verified address for the connection's lifetime and refuses any request whose voucher `signer` is not that address; the address a voucher must recover to is the capability-authorized signer, not the binding ([ADR 003 § Off-Chain Ephemeral Binding](003-payments.md#off-chain-ephemeral-binding-for-clients)). Subsequent requests on the same connection omit these fields.
...
- Key management is simple (a file-based EOA keystore); a high-value client splits custody by delegating a hot EOA voucher signer from a cold owner, with no account-abstraction machinery ([ADR 003 § PaymentPool](003-payments.md#paymentpool), [ADR 024](024-account-abstraction.md#adr-024-account-abstraction-and-safe-smart-wallet-support))
...
- Hardware wallet voucher signing is confirmed infeasible — the hot signer is a software EOA, delegated from a cold owner for custody ([ADR 003 § PaymentPool](003-payments.md#paymentpool))
