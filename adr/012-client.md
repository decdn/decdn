...
reads and writes `buyer-pools.redb`. A `decdn pool` command whose data dir contains a
`buyer.redb` is pointed at a node: it reports the node's pools through the admin RPC
(`decdn node pools`) and refuses to escrow into a store the node never reads.
