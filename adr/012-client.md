...
1. **Startup:** Load iroh key (→ `NodeId`). Load Ethereum key from keystore (EOA) or configure a 1-of-1 Safe with its owner key loaded from keystore; in Production, migrate to a Safe-7579 account and authorize a session key via `erc7579/smartsessions` for high-frequency signing ([ADR 024](024-account-abstraction.md)).
