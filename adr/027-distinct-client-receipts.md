...
| `contentRoot` | `bytes32` | Merkle root over the per-stream content delivered in this voucher window — a keccak256 Merkle tree over 1024-byte chunks per the construction in [§4 Aggregator implementation — MMR accumulator](#aggregator-implementation--mmr-accumulator) below. For sub-MB streams a single leaf hash; for multi-MB streams the root binds every chunk. **Multi-stream channels** (per [ADR 003 Concurrent Streams](003-payments.md#concurrent-streams), where multiple streams share a single channel and a single cumulative voucher): `contentRoot` is the Merkle root of the per-stream chunk-tree roots, ordered by stream-id ascending. The receipt thus commits to all bytes delivered across the voucher's stream set, not just one stream. |
...
The `EpochReceiptSummary` is committed by the operator (or any third party) calling `FeeRouter.commitEpochSummary(operator, epochId, summary)` after the epoch closes and before the summary window expires (timing per § Timing windows below). `claimedBytes` and `claimedDistinctClients` are both operator-asserted and taken at face value pending the challenge window — neither is derivable on-chain from `commitEpochReceiptRoot` alone (which carries only roots), and both must be verified post-hoc against the committed receipt leaves under the §Challenge window mechanism below. Validation rules below pin exactly what the contract checks at submit time.

#### commitEpochSummary validation rules

Every `commitEpochSummary(operator, epochId, summary)` call MUST pass all six checks below; failure of any check reverts atomically:

1. **Argument-struct consistency.** `summary.operator == operator` and `summary.epochId == epochId`. Defensive duplication that catches misrouted submissions and surfaces caller-side bugs immediately. Costs two cheap comparisons on already-loaded calldata.
2. **Epoch validity.** `epochId < currentEpoch()`; an in-progress epoch's summary cannot be committed (the MMR is still mutable via `commitEpochReceiptRoot`).
3. **Summary window open.** `block.timestamp <= epochEnd(epochId) + summaryWindow`. After the deadline, the summary is frozen at whatever was last committed (or absent, in which case gauge eligibility for that epoch is zero).
4. **Cryptographic anti-griefing.** `summary.aggregateRoot` MUST equal the current bagged-peaks root of the per-(operator, epochId) MMR. This is the load-bearing check — a malicious third party cannot lie about the aggregate because the MMR root is on-chain. The third-party-submission path (per § Per-epoch summary above) is safe precisely because the caller's identity is irrelevant: the cryptographic constraint binds the submission to the operator's actual on-chain MMR state, not to the submitter.
5. **Last-write-wins during the summary window.** Any `commitEpochSummary` call within the window replaces any prior summary for the same `(operator, epochId)`. This is necessary because the MMR can keep growing between commits (more `commitEpochReceiptRoot` calls); a summary based on an earlier MMR snapshot would be stale once new leaves arrive. Each commit re-verifies `aggregateRoot` against the *current* MMR root, so the latest commit always reflects the latest MMR state. Emits `EpochSummaryCommitted` on every call (so indexers can track the evolution); the latest event reflects the canonical pre-finalisation state.
6. **Operator-asserted fields are NOT verified at submit time.** `claimedBytes` and `claimedDistinctClients` are accepted at face value — verifying them on-chain would require revealing all leaves in the MMR, which is gas-prohibitive for any real epoch. Fraud detection is the §Challenge window mechanism's job; the bonded-challenger pattern (per [ADR 014 Bond Handling](014-on-chain-verification.md#bond-handling)) creates economic accountability for both submitter and challenger.

After the summary window closes (`block.timestamp > epochEnd(epochId) + summaryWindow`), `commitEpochSummary` reverts unconditionally; the recorded `EpochReceiptSummary` (if any) is the canonical input to the gauge formula and the §Challenge window mechanism. If no summary was committed for an epoch, gauge eligibility for that operator-epoch pair is zero — the operator forfeited the gauge bucket but still receives the §2 40% direct base from settlements.
...
The over-claimed `claimedBytes` and/or `claimedDistinctClients` fields are zeroed for the epoch on successful challenge — the operator's gauge-bucket eligibility for that epoch collapses to zero in addition to the stake slash. The two penalties are independent and stack: zeroing the field denies the unwarranted gauge payout; the slash creates the economic deterrent for repeat fraud. No new on-chain role is introduced.
