...
7. **Keep the old EVM keystore reachable until every outbound pool whose vouchers it signs is fully redeemed by its upstreams, closed, and reclaimed — or has expired.** On the buying leg (cache-miss pulls upstream) the old key is the capability-authorized `signer` those pools' vouchers are signed by ([ADR 003 § PaymentChannel](003-payments.md#paymentchannel)), so it remains the key you need to sign a further voucher; if the old address is also the pool `owner`, it is the key that calls `closeChannel` and `reclaim`. The retention obligation is scoped to exactly those pools: a pool whose signer is a different key needs nothing from this keystore, and a lane where this node is the provider (payee) needs no signing at all — you only redeem. Pools live up to `maxChannelDuration` (default 90 days per [ADR 003](003-payments.md#adr-003-payment-model)). An operator rotating on a schedule shortens the tail by issuing capabilities to the new key on every pool it opens from the changeover onward, leaving only the pre-rotation pools holding the old keystore open.
