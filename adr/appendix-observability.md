...
| `decdn_peer_table_evicted_ttl_total` | Counter | R | — | Peer-table entries removed by the TTL sweeper ([ADR 029 §1](029-peer-table-eviction-policy.md#1-lifecycle-and-ttl)). |
| `decdn_peer_table_evicted_registry_total` | Counter | R | `reason={deregistered,ejected}` | Peer-table entries removed in response to a `NodeDeregistered` or `NodeAutoEjected` registry event ([ADR 029 §3](029-peer-table-eviction-policy.md#3-registry-cache-interaction-active-eviction)). |
