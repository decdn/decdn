...
| `decdn_serve_stream_proof_budget_exhausted_total` | Counter | R | live | — | Times a `cdn/client/v1` delivery ended because the payer sent `MAX_PROOFS_PER_CHUNK` proofs and did not settle one chunk. Both the cache-hit and the miss serve loop bump it. This is a client payment fault, not a node fault. Dispatch logs it only at `debug!`, so this counter is the operator's signal at the default log level. A payer that answers one chunk with sliver after sliver ends here. Each increment ends exactly one inbound stream as failed. |
