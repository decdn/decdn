...
| `decdn_cache_origin_probe_failures_total` | Counter | R | live | Origin size probes that faulted during an origin rescan — every configured origin returned a transport error, or the probe walk overran `cache.origin_probe_timeout_ms`. A rescan decides what this node advertises, so a faulted candidate keeps its previous index entry where there is one and is otherwise absent from the announce set until a later rescan resolves it. A nonzero rate means the origin-held index is running on stale sizes; correlate with the origin backend's own error rate. Note an HTTP origin reports a 5xx as a plain negative, so this covers S3/R2 and the filesystem. |
...
| `decdn_floor_loss_persist_failures_total` | Counter | M | live | Floor dead-charge writes that failed: a drop-time persist (`record_loss` on a reservation drop) or a reclaimed pool's `forget_loss`. Both are best-effort — delivery keeps working — so this counter is the alertable signal that dead charges have stopped reaching disk (a restart then hydrates stale totals and re-grants pools free-floor budget they already consumed) or that a closed pool's row went undeleted and untombstoned, open to permanent re-insertion by a late persist ([ADR 003 § Pool solvency](003-payments.md#pool-solvency-and-the-refundable-floor-m)). A sustained rate usually means the shared `lanes.redb` refuses writes (redb latches after a failed commit until reopen — restart the node) or is corrupt (logged at `error!`). |
...
| `decdn_dht_republish_lag_sweeps_coalesced_total` | Counter | R | live | — | Lag events folded into a sweep that was already running or already queued. `lag_sweeps_total` minus this gives the walks actually started; a coalesced rate that tracks the lag rate one-for-one means the sweep slot is never released. |
...
| `decdn_staker_set_watcher_resolve_failures_total` | Counter | M | live | — | Operator-indexed events (`Reinstated` / `UnbondingRequested`) dropped because the follow-up `nodeIdOf(operator)` RPC failed. The membership change is lost, leaving the cached set out of sync for that operator until the next `getRegisteredNodes` re-enumeration corrects it — normally the one forced on the tick the watcher recovers — silent drift that trips no restart/down-seconds metric, hence its own counter. Pairs with the per-failure `warn!` in `capacity_bond_registry`'s `RegistrySink::on_operator_change`. |
...
| `decdn_capacity_bond_registry_last_resync_timestamp_seconds` | Gauge | M | live | — | Unix time of the last successful re-enumeration. The only signal that catches a repair being *skipped*, which emits nothing at all — the reconcile does not run while the route is errored. Reads `0` until the first success, which is one resync interval after boot at the earliest, so alert on it only with a `> 0` guard and a threshold above that interval. |
...
| `decdn_capacity_bond_registry_resync_failures_total` (rate) | > 0 | > 0 sustained | The repair for a drifted active-staker set is itself failing, so a dropped membership change stays dropped. Check the RPC provider's `eth_call` path; correlate with `decdn_staker_set_watcher_resolve_failures_total`. |
