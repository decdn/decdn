...
| `decdn_cache_prewarm_blobs_total` | Counter | R | Blobs pulled into the store by `cache.prewarm` — the opt-in warm of `cache.pinned_hashes` from a **remote** (`http`/`s3`) origin at startup and on reload (#1130). Counts real fetches only; an already-resident hash is not counted, so a node with a stable pin set holds this flat after its first boot. A value that keeps rising across restarts means prewarmed content is not surviving between runs, which for an LRU-exempt pinned set should not happen. An **fs-only** origin chain ignores prewarm — that content is served from the origin-held index instead; a mixed fs+remote chain does warm, and pins the fs entry serves are imported as a second copy. |
| `decdn_cache_prewarm_bytes_total` | Counter | R | Size of the blobs prewarm materialized before any client requested them (#1130) — a proxy for the spend the opt-in flag exists to gate, not a measurement of it. Like `decdn_cache_pull_through_bytes_total` this counts **content** bytes, not wire bytes (the HTTP origin decodes before the meter), so neither reports what a compressed origin billed; nor is it a strict subset of that counter — a failed size lookup contributes `0` here, while the pull meter counts failed pulls and attributes a coalesced fill to whichever pass owned it. |
| `decdn_cache_prewarm_failures_total` | Counter | R | Prewarm attempts that failed (#1130). Never fatal: the hash is left to an on-demand pull. Merges three causes with different remedies — origin does not hold the hash, origin unreachable / breaker open, local store error — so a value equal to the pin-set size is the signal to read the per-hash `warn` lines, not a diagnosis on its own. |
| `decdn_cache_prewarm_refused_total` | Counter | R | Pinned hashes prewarm refused because they are blacklisted or operator-evicted (#1130). The one prewarm outcome meaning an operator instruction is being actively overridden: without it, a pin set fully covered by a blacklist is indistinguishable from a healthy no-op. |
