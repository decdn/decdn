...
| `decdn_streams_completed_total` | Counter | M | live | `direction={inbound,outbound}` | Streams that delivered the whole request. `inbound` counts streams this node served. `outbound` counts node-to-node pulls this node made from one upstream candidate. |
| `decdn_streams_failed_total` | Counter | M | live | `direction={inbound,outbound}` | Streams that ended without the whole request: refused, stopped mid-stream, reset, or ended on an error. Every stream counts once in this counter or in `decdn_streams_completed_total`, so `completed / (completed + failed)` is the success ratio. The reason split is in the sibling counters `decdn_serve_stream_rejected_*` and `decdn_node_pull_*`, per [§ Reason splits](#reason-splits-sibling-counters-not-labels). |
...
| `decdn_bytes_received_total` | Counter | M | live | — | Payload bytes this node admitted from upstream nodes in paid node-to-node pulls, counted per verified range. |
...
| `decdn_pool_redemptions_total` | Counter | M | live | Vouchers this node redeemed on-chain. A landed `redeemMany` adds its voucher count. |
| `decdn_onchain_tx_landed_total` | Counter | R | live | Node transactions that mined and succeeded. |
| `decdn_onchain_tx_reverted_total` | Counter | R | live | Node transactions that mined and reverted. |
| `decdn_onchain_tx_send_failed_total` | Counter | R | live | Node transactions the RPC refused at `send`. No transaction was issued. |
...
| `decdn_vouchers_received_total` | Counter | R | live | Vouchers this node accepted as the payee on an inbound stream. |
| `decdn_pool_grace_closes_total` | Counter | R | live | Pools that entered the owner-close grace window while this node held unredeemed vouchers on them. Each one is revenue that must redeem before the grace window ends. |
...
| `decdn_config_reload_failures_total` | Counter | R | live | SIGHUP or admin config reloads that failed and kept the previous values. |
