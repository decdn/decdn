...
| `decdn_node_pull_min_draw_waits_total` | Counter | R | live | Pull-leg pauses because open room is below the minimum draw of half the ramped window (decdn#2061) — healthy batching, kept apart from `decdn_node_pull_through_window_paused_total` so that counter still means the credit window ([ADR 037 § Node serving](037-regional-proxy-warming.md#node-serving-window-paced-pull-through)) binds. |
