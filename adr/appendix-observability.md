...
| `decdn_cache_pull_through_bytes_total` | Counter | R | Bytes fetched from origin during a miss (i.e., received as `OriginFetch::Found`), counted regardless of whether the bytes pass BLAKE3 verification or land in the store. Independent signal from `decdn_cache_bytes_returned_total`: when the two are equal the cache is acting as pure pass-through; a `bytes_returned_total >> pull_through_bytes_total` ratio indicates effective caching. |
