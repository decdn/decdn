...
`upstream_stream` records one `outcome`. On a buffered miss, the values are `filled`, `clean_miss`, `local_fault`, and `below_margin`. On a streaming miss, the values are `filled`, `reassigned`, `terminal`, and `cancelled`. `reassigned` means that the node drops this candidate and pulls the rest of the run from another candidate.

A buffered miss nests as `serve_stream` → `pull_through` → `origin_pull` → `node_pull` → `upstream_stream` → `open_progressive_pull`. A streaming miss from a peer nests as `serve_stream` → `serve_miss_pull` → `upstream_stream` → `open_progressive_pull`. The size handshake of a streaming miss opens its `open_progressive_pull` under `serve_stream`.
