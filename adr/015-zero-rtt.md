...
>
> **Suggested enforcement pattern.** Implementations SHOULD use Rust's ownership system to enforce 0-RTT safety statically: the probe request handler receives `&ProbeState` (shared/immutable reference) rather than `&mut ProbeState`, making it a compile-time error to mutate cache priority, demand counters, or any other state. Side-effect-producing operations (demand tracking, cache priority boosting) SHOULD be in a separate code path gated on the QUIC `is_0rtt()` flag, called only for confirmed (non-0-RTT) requests.
