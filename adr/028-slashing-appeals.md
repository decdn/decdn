...
| (b) Verifiable network telemetry | Probe-fan-out timeouts logged by ≥3 independent probers during the slash window |
...
The sworn declaration is an EIP-712-signed statement (secp256k1, signed by the operator's registered Ethereum address — distinct from the Ed25519 wire identity used in `cdn/probe/v1` and `cdn/client/v1`), attesting under penalty of appeal-bond forfeiture that the outage was genuine and no slashable offense was committed during the window. A false declaration carries no separate on-chain re-slash: `SlashJudge` adjudicates only the two protocol offenses — rate manipulation and blacklist violation — each proven by a signed `Probe`/`StreamResponse`, and a sworn declaration is neither, so there is no offense type or decode path that could enter it as evidence. Its deterrents are therefore the ones the appeal itself carries: a declaration the multisig finds contradicted fails the appeal, which burns the full `APPEAL_BOND` ([§ Appeal bond](#appeal-bond)) and leaves the original slash standing, and the 365-day frequency cap ([§ Hard caps and frequency limits](#hard-caps-and-frequency-limits)) bounds repeat attempts. If post-hoc evidence is hard to obtain (private RPC logs, ISP records aged off), a false declaration that already cleared review is not separately prosecutable — the bond forfeit and frequency cap are the whole of the deterrent.
...
**Conflict tiebreaker.** If counter-evidence surfaces during the multisig review window contradicting the operator's bundle (e.g. ≥3 reputable peers attest the operator was *up* during the slash window, contradicting (b) probe-timeout logs), the emergency multisig adjudicates (it can `rejectAppeal` rather than `fastTrackAppeal`). Category (a) signed third-party records are weighted above category (b) probe telemetry on conflict; category (c) co-signed peer attestations are advisory only when (a) and (b) disagree.
...
The bond is the primary economic deterrent against pro-forma appeals filed hoping for multisig sympathy. The 365-day frequency cap ([§ Hard caps and frequency limits](#hard-caps-and-frequency-limits)) is the secondary deterrent.
...
| Abuse path | Counter |
| --- | --- |
| Operator A is slashed legitimately, files appeal hoping multisig sympathy. | Full `APPEAL_BOND` burn on every failed path; a sworn declaration the multisig's review finds contradicted fails the appeal, so the bond burns and the slash stands ([§ Eligibility and evidence standard](#eligibility-and-evidence-standard)); reputation cost stands ([§ Reputation handling](#reputation-handling)). |
| Operator commits a real offense, fabricates an outage to escape consequences. | Each [§ Eligibility and evidence standard](#eligibility-and-evidence-standard) evidence type is cryptographically bound to a third party whose own signing identity is on the line: (a) requires a verifiable third-party signature (ISP, cloud provider, NTP source); (b) requires probe-timeout telemetry that other parties have signed; (c) requires an EIP-712 signature from a peer operator whose reputation and bond are themselves at risk. The ≥2-of-3 requirement therefore forces collusion across at least two independent signing parties, and fabrication of any source is exposed when the original signer disclaims the signature during the multisig's review — which rejects the appeal and burns the bond. |
...
- **`ADR 032` — SafetyReserve appeal-surface contract surface:** RETIRED. The `SlashAppeal` contract holds the appeal state machine; the canonical surface is [§ Contract surface](#contract-surface) above.
