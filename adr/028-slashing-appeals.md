...
All four `SlashJudge` offense types are appealable: corruption, phantom, rate manipulation, blacklist. The legitimate-outage rationale applies to each — v1 corruption appeals address operators who missed the 24h counter-evidence window per [ADR 014 §2](014-on-chain-verification.md#2-blake3-content-corruption--optimistic-challenge-response); v2 corruption appeals address operators slashed by the cryptographic single-shot Merkle proof in [ADR 030 §2](030-blake3-merkle-verification.md#2-single-shot-corruption-proof); phantom/rate/blacklist appeals address operators who had no counter-evidence opportunity at all because the slash executed immediately ([ADR 014 §3 Bond Handling](014-on-chain-verification.md#bond-handling)). Both v1 and v2 corruption paths feed the unified `OffenseType.Corruption` and are equally appealable under this ADR — there is no v1/v2 distinction at the appeal layer.
