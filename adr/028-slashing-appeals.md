...
| (c) Co-signed attestation from another registered node operator | EIP-712-signed statement from a peer operator in good reputation standing per [ADR 008](008-reputation.md) confirming observed downtime in the same datacenter / region |
...
| Sybil operator network co-signs each other's (c) attestations. | Co-signers must be in good reputation standing per [ADR 008](008-reputation.md); reputation is itself peer-aggregated, raising the cost of building a sybil ring high enough to qualify as a witness. |
...
| Sybil ring of operators each file appeals to drain the SafetyReserve below incident-response thresholds. | `MAX_APPEAL_RESTITUTION` cap + `SafetyReserve` per-rolling-window USDC ceiling ([ADR 009](009-governance.md#emergency-multisig)) bound the worst case; sybil sets large enough to exceed those caps must each pass the §3 ≥2-evidence-source bar including good-reputation-standing peer attestations, raising the cost of building the ring above the reserve damage it could cause. SafetyReserve insolvency triggers the deferred-claim path (§5), so a drain attack cannot deny the reserve to other incident categories indefinitely. |
