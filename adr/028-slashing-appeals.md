...
**SafetyReserve insolvency at ratification.** If `SafetyReserve` cannot fund the full restitution at ratification time (rolling-window cap reached, or insufficient general balance after concurrent incidents), the appeal succeeds in principle but the unfunded portion is recorded as a *pending claim* with a monotonic `claimId` at the current accrual epoch. The four [ADR 026 §5 Spending controls](026-gauge-boost-tokenomics.md#spending-controls) gates were checked at the original `ratifyAppeal` authorization; once the claim is queued no further multisig action is required. The bond is refunded regardless. Disbursement is permissionless head-of-queue when reserve solvency permits, per [ADR 026 §5 Cross-category payout ordering](026-gauge-boost-tokenomics.md#cross-category-payout-ordering) — the queue is epoch-FIFO across all payout categories with `claimId` as a within-epoch monotonic tiebreaker, and no payout category has priority over another.
