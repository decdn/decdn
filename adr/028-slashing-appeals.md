...
- **Cross-subsidy / depletion ratio.** A 50% slash on a min-stake operator nets `SafetyReserve` ~30% × stake from inflow ([ADR 026 § Slashing and burn](026-tokenomics.md#slashing-and-burn) distribution: 50% challenger / 30% reserve / 20% burn), while a successful appeal can pay out up to 1× minimum stake — a worst-case ~3.3× depletion ratio for that incident, rising further if the slash tier was 5% or 15% rather than 50%. The reserve thus funds a cross-subsidy from non-slash inflows (FeeRouter 3% safety bucket per [ADR 026 § FeeRouter split (40/40/7/5/5/3)](026-tokenomics.md#feerouter-split-40407553)) into slash restitution. This cross-subsidy is accepted explicitly for stakes at or near the minimum, where restitution can fully cover the slash; for operators staking well above the minimum, a 50% slash exceeds `MAX_APPEAL_RESTITUTION` and the operator absorbs the residual ([§ Hard caps and frequency limits](#hard-caps-and-frequency-limits)). The protocol prioritizes reserve solvency over full-restitution-for-whales; restitution caps at the slashable stake floor, not the actual slash amount. Reserve sizing in [ADR 026 § Safety and insurance reserve (3% bucket)](026-tokenomics.md#safety-and-insurance-reserve-3-bucket) must accommodate this depletion ratio when projecting solvency. The deferred-claim path ([§ Hard caps and frequency limits](#hard-caps-and-frequency-limits)) bounds the worst case at the cost of operator working-capital exposure during depletion windows.
