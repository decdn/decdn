...
- **Persisted outboard.** For a blob the node holds, the node reads the outboard its store materialized at import and exports the requested range. The export validates each emitted proof node and chunk group against the root `H`. When held bytes or the outboard change on disk after admission, the serve stops at the first group that does not match, and the node quarantines the hash ([ADR 040 § Pinning, durable operator-evict, and the probe-hold stay engine-enforced](040-cache-policy.md#pinning-durable-operator-evict-and-the-probe-hold-stay-engine-enforced)).
