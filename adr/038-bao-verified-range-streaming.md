...
- The pipelined origin-cold path costs one extra origin fetch per cold serve — the `{H}.obao4` object, `~0.4%` of blob size — and it hashes the origin's bytes on the serve path, because those bytes arrive unverified.
