...
- **Under-sized pool.** One pool backs all sources, so there is no deposit fragmentation; the residual is that the single deposit must cover the sum of per-source redemption credit windows the client fronts in its blind window. Mitigated by sizing the deposit to the admitted set and topping up rather than over-committing up front ([ADR 003 § Pool solvency and the redemption credit window](003-payments.md#pool-solvency-and-the-redemption-credit-window)).
