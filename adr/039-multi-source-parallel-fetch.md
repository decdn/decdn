...
**The whole fan-out has a throughput floor.** The unit deadline judges one source at a time. Every source can make some progress inside its deadline while the fetch as a whole crawls. The client therefore also counts the fetch's verified bytes across all of its sources. When that count stays below `--min-throughput-bps` for `--stall-timeout-ms`, the client stops the fetch. The clock stops while the client waits on its own top-up. The client flushes the present record after the stop. A stop is a retryable failure, so the fetch falls back to single-source failover.
