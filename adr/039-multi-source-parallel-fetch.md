...
- **Delivery is self-enforcing.** Payment and delivery are coupled per stream: the payer stops sending proofs → the node stops sending chunks, and the reverse ([ADR 005 § Payment quantum and credit window](005-protocol.md#payment-quantum-and-credit-window)). To obtain a bounded sub-range, request from its start offset and cease payment at its end. There is no end-offset field on the wire.
