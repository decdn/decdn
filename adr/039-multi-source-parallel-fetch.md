...
- **Delivery is self-enforcing.** Payment and delivery are coupled per stream: the payer stops releasing preimages → the node stops sending chunks, and the reverse ([ADR 005 § Chunk cadence](005-protocol.md#chunk-cadence)). To obtain a bounded sub-range, request from its start offset and cease payment at its end. There is no end-offset field on the wire.
