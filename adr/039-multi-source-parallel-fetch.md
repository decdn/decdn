...
- **Client.** The candidate set is the bonded set the client already knows. It reads its persisted peer store ([ADR 037 § Client RTT map](037-regional-proxy-warming.md#client-rtt-map-and-latency-discovery)) when the store holds enough fresh, unsuppressed records; otherwise it reads the `CapacityBond` registry ([ADR 001 § Node Discovery](001-network.md#node-discovery-registry)) and narrows the set by its configured region allowlist. The client issues no `cdn/dht/v1` lookup. It probes each candidate for live availability, RTT, and **fresh** coverage ([ADR 005 § `cdn/probe/v1`](005-protocol.md#cdnprobev1--latency-probe)), or skips the probe round on the store fast path and uses the stored latency. The rank key is measured RTT only ([ADR 037 § Client selection policy](037-regional-proxy-warming.md#client-selection-policy-latency-driven-proxy-preference)). Price is not a rank key: the client pays the signed `StreamResponse.rate_per_mb` it receives, and refuses a quote above its optional hard ceiling (`--max-rate-per-mb`, off by default). Reputation does not enter: the client keeps no reputation score ([§ Source diversity and per-peer memory](#source-diversity-and-per-peer-memory)).
...
- **Client.** The client keeps no reputation score and links no reputation code. Its only per-peer memory is the persisted peer store ([ADR 037 § Client RTT map](037-regional-proxy-warming.md#client-rtt-map-and-latency-discovery)): an EWMA of measured latency, the most recent quoted `rate_per_mb` as a ranking hint, and a failure stamp that suppresses the peer from selection for a fixed window. The probe round and the first admitted lane's stream open feed that store. The scheduler's per-lane outcomes — stalls, drops, verification failures on the other lanes — are reported in the fetch result, not persisted. A client-side reputation score stays out: verification and no-pay-on-failure already bound what a bad source costs a client to one unpaid range, and the failure stamp plus live RTT already carry every selection signal a persistent score would add.
...
- Verification failures cost the source its payment for that range, and the consumer remembers the failure — a client suppresses the source, a node lowers its local reputation score — so multi-source fetch hardens the network against bad sources rather than merely tolerating them.
...
2. Each segment is verified against the content-hash root on receipt; a segment failing verification receives no voucher for the corrupt range and has its unverified remainder reassigned to a different source. A client suppresses the failing source from selection for the failure window; a node lowers the failing source's local reputation score. No slashing evidence is produced.
