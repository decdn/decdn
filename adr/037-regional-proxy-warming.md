...
- **Pipelined look-ahead beyond one interval.** The window is honored as a byte bound, but downstream vouchers are collected per interval in lockstep with the buyer's voucher cadence; a `pull_ahead_bytes` set well above one voucher interval bounds exposure as specified but does not yet overlap multiple in-flight interval payments.
