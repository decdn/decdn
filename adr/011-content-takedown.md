...
interface IOriginAssignment {
    ...
    // "Default-open allow-list" below for the lifecycle, genesis seed, and
...
**Genesis seed.** The `OriginAssignment` constructor seeds the default-open allow-list with an explicit initial operator set, validated against the same `[defaultOpenMinRedundancy, defaultOpenMaxOrigins]` size bound and no-duplicates checks as `setDefaultOpenAllowlist`, and emits `DefaultOpenAllowlistUpdated` at construction. The seed is structurally validated only — no operator is staked at construction time, so the per-operator `StakingRegistry.isActive` check that gates post-deploy governance updates does not apply to the seed; operator liveness is filtered off-chain by clients at probe time, as it is for any assignment (see [§ Cross-contract integration](#cross-contract-integration)). `isAuthorizedOrigin(0, op)` is strict set-membership from block zero — there is no permissive bootstrap window and no activation latch. Rationale: a permissive window would force every off-chain probe and slashing consumer to branch on bootstrap state, leaking a protocol-wide dual mode into node behaviour and complicating slashing. Seeding the set via an explicit, auditable constructor parameter (recorded by the genesis `DefaultOpenAllowlistUpdated` event — not an implicit hardcoded genesis set) gives one uniform code path from block zero and keeps slashing single-mode; launch is not gated on a Governor vote because the deployer supplies the initial set as a deployment parameter.
...
- Cache-only role is permissionless per [ADR 001](001-network.md#adr-001-network-topology-and-peer-mesh); the origin role is governance-gated for all content — per-namespace `OriginAssignment` for registered namespaces and the default-open allow-list, seeded at deployment and strict from genesis (see [§ Default-open allow-list](#default-open-allow-list)), for unregistered content
