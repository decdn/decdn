...
Removing a wrongful entry going forward (this ADR) does not mechanically refund slashes already taken ([ADR 028](028-slashing-appeals.md)); the decoupling is the explicit boundary stated in [§ Blacklist Entry Appeals](#blacklist-entry-appeals).
...
- **Ejected from `StakingRegistry`** — sets `active = false`, emits `NodeAutoEjected` ([ADR 001](001-network.md)). This follows the same code path as stake-based auto-ejection ([ADR 026 §8](026-tokenomics.md#8-slashing-and-burn))
