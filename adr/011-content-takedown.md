...
- The strict-gating model amends the unconditional permissionless-origin claim from [ADR 001](001-network.md). Cache-only role is preserved as permissionless, but the origin role is governance-gated for all content — per-namespace `OriginAssignment` for registered namespaces and the default-open allow-list for unregistered content. Until governance executes the first default-open allow-list activation, the bootstrap window is open (see [§ Default-open allow-list](#default-open-allow-list)); the activation closes it
