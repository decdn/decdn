...
| Parameter | Default | Hard bounds | Rationale |
| --- | ---: | --- | --- |
| `BLACKLIST_APPEAL_BOND` | 1,000 TOKEN | `[100, 10,000]` | Matches [ADR 028 § Appeal bond](028-slashing-appeals.md#4-appeal-bond) for operational parity. The two ADRs use distinct contract storage; defaults align but the parameters may diverge under governance. |
| `BLACKLIST_MULTISIG_REVIEW_WINDOW` | 14 days | `[3d, 30d]` | Same shape and bounds as [ADR 028 § Hard caps and frequency limits](028-slashing-appeals.md#5-hard-caps-and-frequency-limits)'s `MULTISIG_REVIEW_WINDOW`. |
...
- **Rejection at intake:** 100% of bond burned; no counter-bundle filer to credit (mirrors [ADR 028 § Appeal bond](028-slashing-appeals.md#4-appeal-bond)).
...
Operational-failure evidence types from [ADR 028 § Eligibility and evidence standard](028-slashing-appeals.md#3-eligibility-and-evidence-standard) (gossip telemetry, peer-attested downtime) are **not** admissible here. The two domains use disjoint evidence sets; an operator whose grievance is "I was offline during the compliance window" must use the [ADR 028](028-slashing-appeals.md#adr-028-slashing-appeals-and-dispute-escalation) path. Filing such evidence on this path is grounds for rejection at intake, not perjury.
...
- **Ejected from `StakingRegistry`** — sets `active = false`, emits `NodeAutoEjected` ([ADR 001](001-network.md#adr-001-network-topology-and-peer-mesh)). This follows the same code path as stake-based auto-ejection ([ADR 026 § Slashing and burn](026-tokenomics.md#8-slashing-and-burn))
...
- **Remaining stake enters forced unbonding** — the standard unbonding period applies (7 days PoC / governable in production, minimum 3 days). Stake remains slashable during unbonding ([ADR 026 § Operator economics and minimum stake](026-tokenomics.md#7-operator-economics-and-minimum-stake))
...
interface IOriginAssignment {
    ...
    // Default-open allow-list (namespaceId == 0). GOVERNANCE_ROLE only;
    // initially seeded by the deployer during post-deploy init (see ADR
    // 016), governance-managed thereafter. See "Default-open allow-list"
    // below for the lifecycle, seeding, and validation invariants.
...
**Lifecycle.** Allow-list updates are GOVERNANCE_ROLE-only single-step proposals under the Governor's standard timelock — no separate `defaultOpenAssignmentTimelock` parameter. `setDefaultOpenAllowlist(operators)` replaces the active set atomically; `addDefaultOpenOperator` / `removeDefaultOpenOperator` are convenience deltas with the same authority and delay. Each transition appends a checkpoint per affected operator. The contract enforces `operators.length ∈ [defaultOpenMinRedundancy, defaultOpenMaxOrigins]` and rejects duplicate addresses. There is no on-chain staking check — default-open authorization is a pure address allow-list; operator liveness is filtered off-chain by clients at probe time, the same as for any assignment (see [§ Cross-contract integration](#cross-contract-integration)).

**Seeding.** The default-open set is empty at deployment — the `OriginAssignment` constructor takes no operator-set parameter. It is populated by the deployer during post-deployment initialization via an ordinary `setDefaultOpenAllowlist` call (alongside the other one-shot wiring steps in [ADR 016 § Post-Deployment Initialization](016-contract-interactions.md#post-deployment-initialization)), before the network accepts traffic — not a constructor argument, not a Governor proposal, and with no special genesis event. `isAuthorizedOrigin(0, op)` is strict set-membership from block zero: while the set is empty no operator is an authorized default-open origin (cache-only serving stays permissionless), so seeding is a mandatory bring-up step. Rationale: a permissive bootstrap window would force every off-chain probe and slashing consumer to branch on bootstrap state, leaking a protocol-wide dual mode into node behaviour and complicating slashing; a strict set populated by a normal post-deploy `setDefaultOpenAllowlist` keeps one uniform code path and a single slashing mode with no constructor argument or genesis special-casing.
...
Serving a blacklisted hash after the compliance window is a slashable offense, subject to the escalating schedule in [ADR 026 § Slashing and burn](026-tokenomics.md#8-slashing-and-burn). Repeated offenses trigger cumulative stake loss; nodes whose stake drops below 50% of the minimum are auto-ejected. Individual slash percentages are capped at 50% per offense ([ADR 009 § Safety bounds](009-governance.md#governable-parameters-with-safety-bounds)). The standard 100 TOKEN challenge bond from [ADR 026 § Slashing and burn](026-tokenomics.md#8-slashing-and-burn) applies.
...
- Cache-only role is permissionless per [ADR 001](001-network.md#adr-001-network-topology-and-peer-mesh); the origin role is governance-gated for all content — per-namespace `OriginAssignment` for registered namespaces and the default-open allow-list, seeded by the deployer during post-deploy initialization and strict from genesis (empty until seeded; see [§ Default-open allow-list](#default-open-allow-list)), for unregistered content
