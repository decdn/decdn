...
- **Effectively removed from every namespace's authorized origin set** at every runtime check (probes, peer-table validation, slashing evaluation), with lazy permissionless storage cleanup — see [§ Interaction with ContentBlacklist](#interaction-with-contentblacklist) for the full pattern
