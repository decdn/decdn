...
Naming follows [appendix-observability.md § Gossip Metrics](appendix-observability.md#gossip-metrics). The existing `decdn_gossip_peer_table_size` gauge and `decdn_gossip_announces_rejected_total` counter cover most of the surface; this appendix adds three eviction counters and one log-visible reason token:
...
- Without `max_peer_entries` set, a registry-validation regression admitting unstaked `node_id`s could allow unbounded growth. `decdn_gossip_peer_table_size` is the early-warning signal; operators tracking it can set the ceiling reactively.
