...
| BuybackBurner (abstract) + GuardedBuybackBurner (abstract) + BuybackBurnerBalancerV3 / BuybackBurnerUniswapV3 | [018](018-liquidity-strategy.md#adr-018-liquidity-strategy-balancer-8020-pol), [026](026-tokenomics.md#adr-026-tokenomics) | Yes | USDC, TOKEN (transient) | `AccessControl`, `ReentrancyGuard`, `Pausable` via the shared `SunsettingPausable` base (swap-and-burn path; receives 30% of every settlement. `BuybackBurner` is the venue-neutral abstract base and owns the shared surface, the abstract `GuardedBuybackBurner` adds the MEV-defense stack once for every venue, and the two concrete contracts bind a live venue — the deployment picks one per [ADR 018 § Venue-neutral burner selection](018-liquidity-strategy.md#venue-neutral-burner-selection)) |
...
| 7 | BuybackBurnerBalancerV3 / BuybackBurnerUniswapV3 (optional) | USDC address, TOKEN address, admin address, then a venue `Config` struct. The abstract `BuybackBurner` base is not deployable — the deployment binds one concrete venue per [ADR 018 § Venue-neutral burner selection](018-liquidity-strategy.md#venue-neutral-burner-selection). Position 7 records the constructor dependency on TOKEN and USDC; it is not the wall-clock position, because the burner ships in the optional genesis-activation bundle that runs after step 15. See [§ BuybackBurner construction notes](#buybackburner-construction-notes) and [ADR 003](003-payments.md#buybackburner) for the interface. |
...
- **Constructor `Config`.** USDC, TOKEN, and admin are zero-rejected by the abstract base. For `BuybackBurnerBalancerV3` the struct carries the Balancer V3 Router address (zero-rejected), the Permit2 address (zero-rejected), the Balancer V3 Vault address, the initial pool `address`, and the `GuardedBuybackBurner` band (`twapMinWindow`, `maxBuybackAmount`, `minBuybackAmount`, `slippageBps`, `epochLiquidityCapFraction`).
- **Deferred venue wiring.** Vault and pool may both be zero at deploy, and governance wires them later via `setVault(address)` / `setPool(address)`. `executeBuyback` reverts `PoolNotWired` until both are set. A `Config` that supplies both is validated at construction: the pool must be Vault-registered and hold the USDC and TOKEN legs.
- **Inflow source.** `FeeRouter`, 30% of every settlement per [ADR 026 § Slashing and burn](026-tokenomics.md#slashing-and-burn).
- **Approvals.** The approval target is **Permit2**, never the Vault. [ADR 018 § Buyback execution via Balancer V3](018-liquidity-strategy.md#buyback-execution-via-balancer-v3) is authoritative for the mechanism and for the Router address.
...
| BuybackBurner | Balancer V3 Router | `swapSingleTokenExactIn(pool, tokenIn, tokenOut, exactAmountIn, minAmountOut, deadline, wethIsEth, userData)` | The Router pulls `tokenIn` through **Permit2**, not through a direct ERC20 allowance to the Router or the Vault. This indirection is the V3 footgun; [ADR 018 § Buyback execution via Balancer V3](018-liquidity-strategy.md#buyback-execution-via-balancer-v3) is authoritative for the scoped per-swap Permit2 approval it requires | Yes |
