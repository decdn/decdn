...
| `redeemMany(batches[])` | `IERC20.safeTransfer()` (the batch's paid total to FeeRouter), `FeeRouter.routeSettlement(operator, totalBytes, totalPaid)` once per batch, where each voucher contributes `paid = min(desired, capRoom, remaining)` (`desired = cumulative − lane.paid`) and `bytesPaid = mulDiv(bytesDelta, paid, desired)` — the **paid-proportional** byte count. Every voucher is redeemed for `msg.sender`, which is what lets one call settle the batch, and each `{poolId, capabilities[], vouchers[]}` group gates its pool's status once and advances its `totalRedeemed` in one write. The lane watermark advances by `paid`, not to `cumulative`, so a partially-drained draw is retriable; a voucher paying `0` is skipped with no state written. FeeRouter performs the split internally | `nonReentrant`, checks-effects-interactions across the whole batch; FeeRouter is `nonReentrant`-guarded on `routeSettlement` to defend against re-entry through the operator-base `safeTransfer` |
