...
| `submitMerkleCorruptionChallenge()` (v2 / single-shot Merkle proof — `cdn/client/v2`; per [ADR 030](030-blake3-merkle-verification.md)) | `IERC20.safeTransferFrom()` (TOKEN bond deposit), `StakingRegistry.slash()` on corruption-proven branch, `IERC20.safeTransfer()` (slash reward + bond return on corruption-proven; bond forfeit per ADR 014 §3 split on frivolous) | `nonReentrant`, checks-effects-interactions |
