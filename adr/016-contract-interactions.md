...
- **Default-open allow-list seed.** The constructor seeds the `namespaceId == 0` allow-list with an explicit initial operator set, validated against the same `[defaultOpenMinRedundancy, defaultOpenMaxOrigins]` size bound, `StakingRegistry.isActive`, and no-duplicates checks as `setDefaultOpenAllowlist`, and emits `DefaultOpenAllowlistUpdated` (updateIndex 0). `isAuthorizedOrigin(0, op)` is strict set-membership from block zero — there is no permissive bootstrap window and no `defaultOpenAllowlistActive` latch.
