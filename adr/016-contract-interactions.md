...
> **Tokenomics v3 driver.** The contract surface in this ADR is materially expanded by [ADR 026 — Tokenomics v3](026-tokenomics-v3.md), which adds `FeeRouter`, `VotingEscrow`, and `SafetyReserve`, and rewires `PaymentChannel`, `StakingRegistry`, `BuybackBurner`, and `Governor`. Read ADR 026 first for the economic model; this ADR is the integration view.
...
| --- | --- | --- | --- | --- | --- |
| TOKEN (ERC-20) | [004](004-tokenomics.md), [026](026-tokenomics-v3.md) | No (fungible token) | — | `ERC20`, `ERC20Permit` (recommended; enables gasless approvals) | PoC + Production |
| StakingRegistry | [003](003-payments.md), [004](004-tokenomics.md), [026](026-tokenomics-v3.md) | Yes | TOKEN | `AccessControl`, `ReentrancyGuard`, `Pausable` | PoC + Production |
| StablePaymentChannel | [003](003-payments.md) | Yes | USDC | `Ownable`, `ReentrancyGuard`, `Pausable`, `EIP712` | PoC only |
| PaymentChannel | [010](010-multi-token.md), [026](026-tokenomics-v3.md) | Yes | Governance-approved ERC-20s | `AccessControl`, `ReentrancyGuard`, `Pausable`, `EIP712` | Production only |
| FeeRouter | [026](026-tokenomics-v3.md) | Yes | USDC (transient + epoch buckets), TOKEN (delegator-pool epoch buckets) | `AccessControl`, `ReentrancyGuard`, `Pausable` | Production (v3 launch) |
| VotingEscrow | [026](026-tokenomics-v3.md) | Yes | TOKEN (locked, non-transferable) | `ReentrancyGuard`, `Pausable` | Production (v3 launch) |
| SafetyReserve | [026](026-tokenomics-v3.md) | Yes | USDC (3% bucket + slashing redirect) | `AccessControl`, `ReentrancyGuard`, `Pausable` | Production (v3 launch) |
| BuybackBurner | [004](004-tokenomics.md), [018](018-liquidity-strategy.md), [026](026-tokenomics-v3.md) | Yes | USDC, TOKEN (transient) | `AccessControl`, `ReentrancyGuard`, `Pausable` | PoC (accumulate-only) + Production |
...
| Contract | ADR | Status |
| --- | --- | --- |
| DelegatorBuyer (or `BuybackBurner` multi-output extension) | [026](026-tokenomics-v3.md) §6 | Implementation choice deferred — either a parallel contract or a `BuybackBurner` mode performs the delegator-pool 7% USDC→TOKEN swap. Selected during v3 implementation. |
...
The diagram below shows the v3 contract surface and its primary call relationships. Reproduced from [ADR 026](026-tokenomics-v3.md)'s source design spec §9.5 with the existing-ADR-016 contracts (`StakingRegistry`, `Governor`, etc.) included for orientation. `StakingRegistry` is unconnected on the fee-router path because it is independent of settlement — it governs slashable stake and is read by gossip / peer-validation logic ([ADR 001](001-network.md), [ADR 003](003-payments.md)) rather than by `FeeRouter`.
...
The full FeeRouter six-bucket split (40/40/7/5/5/3), epoch-bucket mechanics, and gauge-boost formula are specified in [ADR 026](026-tokenomics-v3.md) §2–§3; this ADR does not duplicate the bucket table.
...
| --- | --- | --- |
| 1 | TOKEN | None. **PoC:** freely mintable testnet token with `onlyOwner` mint ([ADR 004](004-tokenomics.md)). **Production (v3):** fixed 1B supply, no mint function ([ADR 026](026-tokenomics-v3.md) §1). |
| 2 | USDC | External (testnet faucet or mainnet address) |
| 3 | StakingRegistry | TOKEN address, `minStake` (PoC 1,000 TOKEN; **v3 production: 50,000 TOKEN** per [ADR 026](026-tokenomics-v3.md) §7), `unbondingPeriod` (7 days). **v3:** discount-threshold parameters removed; the discounted-fee mechanic is replaced by the gauge-boost flow in `FeeRouter`. |
| 4 | VotingEscrow (v3 production) | TOKEN address, `minLockDuration` (1 week), `maxLockDuration` (4 years). No `create_lock_for` privileged path; auto-ve-lock removed in v3. Implements `balanceOfAt(user, ts)` and `totalSupplyAt(ts)` historical checkpointing ([ADR 026](026-tokenomics-v3.md) §4). |
| 5 | SafetyReserve (v3 production) | USDC address, Governor address (payout authorizer), emergency-multisig address (fast-track approver under hard caps), `appealWindow` (48h) ([ADR 026](026-tokenomics-v3.md) §5). |
| 6 | BuybackBurner | TOKEN address, USDC address, Balancer V3 Router address, initial pool contract `address` (may be zero-address at deploy and set later via `setPool(address)` — see [ADR 003](003-payments.md#buybackburner) for the interface and [ADR 018](018-liquidity-strategy.md) for the venue rationale). The pool address remains governance-mutable post-deploy via `setPool(address)`; the constructor value is an initial convenience, not a hard requirement. **v3 inflow source:** `FeeRouter` rather than manual treasury transfer ([ADR 026](026-tokenomics-v3.md) §8); the contract surface is otherwise unchanged. **Router naming note:** the canonical Router `0xEAedc32a51c510d35ebC11088fD5fF2b47aACF2E` is labelled `v3-router-v2` in the Balancer deployments registry — "v2" is the second iteration of the Balancer V3 Router artifact, NOT Balancer V2 protocol routing (see [ADR 018](018-liquidity-strategy.md#buyback-execution-via-balancer-v3)). **Approvals note:** `BuybackBurner` MUST self-approve the Balancer V3 **Vault** address (distinct from the Router) during its initialization — the Vault pulls input tokens from the `msg.sender` of the Router call, which is `BuybackBurner` itself. This is the V3 footgun; see [ADR 018](018-liquidity-strategy.md#buyback-execution-via-balancer-v3). |
| 7 | FeeRouter (v3 production) | USDC address, TOKEN address, VotingEscrow address, BuybackBurner address, SafetyReserve address, treasury wallet address, Balancer V3 Router + pool addresses (for the delegator-pool USDC→TOKEN swap; may share `BuybackBurner`'s configuration), `epochLength` (1 week), `claimWindow` (26 epochs), default split shares (40/40/7/5/5/3 per [ADR 026](026-tokenomics-v3.md) §2), and `boostFloor` (0.4) per [ADR 026](026-tokenomics-v3.md) §3. Sum-to-100% across the six router shares is enforced on every governance update. |
...
| 8b | PaymentChannel (v3 production) | StakingRegistry address, Governor address, **FeeRouter address** ([ADR 026](026-tokenomics-v3.md) §2). `settleChannel` no longer skims a protocol fee; it transfers the full operator USDC balance to `FeeRouter.routeSettlement(operator, bytesDelivered, amount)` in the same transaction. The `feePercentage` / `discountedFeePercentage` constructor arguments from the PoC contract are removed. |
...
2. **Grant `SLASH_ROLE`** on StakingRegistry to SlashJudge:
   ...
   **v3:** also grant a slashing-redirect role so SlashJudge can route 30% of slashed stake to `SafetyReserve` per [ADR 026](026-tokenomics-v3.md) §8 (challenger 50% / SafetyReserve 30% / burn 20%); the address `SafetyReserve` becomes a recipient of slashed USDC (or TOKEN swapped via the existing Balancer V3 path — implementation choice deferred).
...
| PaymentChannel (v3) | FeeRouter | `routeSettlement(operator, bytesDelivered, amount)` | `ROUTER_CALLER_ROLE` on FeeRouter ([ADR 026](026-tokenomics-v3.md) §2) | Yes |
...
| Governor (v3) | SafetyReserve | `payout(bundle, recipient, amount)` | `PAYOUT_AUTHORIZER_ROLE` (Governor + emergency-multisig within hard caps; [ADR 026](026-tokenomics-v3.md) §5) | Yes |
...
| StakingRegistry (v3) | SafetyReserve | `safeTransfer()` (30% of slashed stake; the remaining 50% goes to the challenger and 20% burns per [ADR 026](026-tokenomics-v3.md) §8) | Caller holds balance | Yes |
...
**Fee allocation (v3 production, FeeRouter six-bucket split):** see [ADR 026](026-tokenomics-v3.md) §2 for the canonical 40/40/7/5/5/3 table — node base / gauge boost pool / delegator pool / buyback-and-burn / treasury / safety reserve. This ADR does not duplicate the bucket table. Treasury disbursement requires a governance proposal ([ADR 009](009-governance.md)).
...
| Destination | PoC ([ADR 004](004-tokenomics.md)) | v3 production ([ADR 026](026-tokenomics-v3.md) §8) |
| --- | ---: | ---: |
...
| VotingEscrow (v3) | TOKEN (locked, non-transferable) | User `createLock` deposits | `withdraw()` after lock expiry only; no early exit, no `create_lock_for` privileged path ([ADR 026](026-tokenomics-v3.md) §4) |
| SafetyReserve (v3) | USDC (3% router bucket + 30% slashing redirect) | `FeeRouter`, `StakingRegistry` slashing path | `payout(bundle, recipient, amount)` after evidence bundle, Governor (or emergency-multisig within hard caps), and 48h appeal window ([ADR 026](026-tokenomics-v3.md) §5) |
...
| BuybackBurner | USDC (accumulated), TOKEN (transient) | PoC: treasury transfers. v3: 5% USDC same-tx from `FeeRouter` ([ADR 026](026-tokenomics-v3.md) §8) | `executeBuyback()` (production; accumulate-only in PoC) |
...
| `PAYOUT_AUTHORIZER_ROLE` (v3) | SafetyReserve | `payout(bundle, recipient, amount)` | n/a | Governor via timelock; emergency multisig within hard caps ([ADR 026](026-tokenomics-v3.md) §5) |
...
| **Minimum stake (v3 production)** | **50,000 TOKEN (default)** | **per [ADR 026](026-tokenomics-v3.md) §7** | **StakingRegistry — discount-threshold logic removed** |
...
Safety bounds are `immutable` — hardcoded in constructors, not overridable by governance or admin. v3 FeeRouter shares are governable within the bounds above; sum-to-100% across the six router shares is enforced on every governance update ([ADR 026](026-tokenomics-v3.md) §11).
...
| `slash()` (v3) | `IERC20.safeTransfer()` (TOKEN: 50% challenger / 30% SafetyReserve / 20% burn per [ADR 026](026-tokenomics-v3.md) §8) | `nonReentrant`, checks-effects-interactions, `SLASH_ROLE` |
...
> **v3 inflow source.** Under [ADR 026](026-tokenomics-v3.md) §8, `BuybackBurner` no longer receives USDC via manual treasury transfer — it receives 5% of every settlement same-tx from `FeeRouter`. Inflow rate is ~8× higher per unit network revenue (5% of 100% vs ADR 004's 20% × 3% = 0.6% effective). The `executeBuyback` mechanics, `KEEPER_ROLE`-gating, and Vault-scoped self-approval pattern are unchanged.
...
| `executeDelegatorSwap(epoch, minOut)` | `BalancerV3Router.swapSingleTokenExactIn()` (Vault-scoped self-approval, same V3 footgun pattern as `BuybackBurner`) | `nonReentrant`, checks-effects-interactions, `KEEPER_ROLE`; per-epoch liquidity caps and TWAP-window guards REQUIRED ([ADR 026](026-tokenomics-v3.md) §6) |
| `setShares(...)`, `setBoostFloor(...)` | None (state change only) | `GOVERNANCE_ROLE` (Governor via timelock); sum-to-100% across the six router shares enforced; per-share bounds enforced ([ADR 026](026-tokenomics-v3.md) §11) |
...
> **Cashflow invariant.** The 20% lower bound on the node-base share is `immutable` and guarantees operators always receive enough liquid USDC to cover at least a meaningful fraction of infrastructure costs even under extreme governance proposals. See [ADR 026](026-tokenomics-v3.md) §11.
...
| `GovernorVotes` | Production governance | **PoC/ADR 004:** TOKEN as voting token. **v3:** replaced by a `VotingEscrow`-backed vote source per [ADR 026](026-tokenomics-v3.md) §9 |
...
| Settlement-fee mechanic | Skim at settlement contract (3% / 1.5% discounted) | `FeeRouter` six-bucket split (40/40/7/5/5/3) per [ADR 026](026-tokenomics-v3.md) §2 |
...
| SafetyReserve | Not deployed | Deployed; receives 3% router bucket + 30% slashing redirect; payouts gated per [ADR 026](026-tokenomics-v3.md) §5 |
...
| Minimum stake | 1,000 TOKEN | 50,000 TOKEN ([ADR 026](026-tokenomics-v3.md) §7); discount-threshold logic removed |
| Slashing distribution | 50% challenger / 50% burn | 50% challenger / 30% SafetyReserve / 20% burn ([ADR 026](026-tokenomics-v3.md) §8) |
...
- [ADR 026 — Tokenomics v3 (Gauge-Boost Design)](026-tokenomics-v3.md): FeeRouter six-bucket split, VotingEscrow, SafetyReserve, gauge-boost formula, v3 slashing distribution
