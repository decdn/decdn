...
| BuybackBurner (abstract) + GuardedBuybackBurner (abstract) + BuybackBurnerBalancerV3 / BuybackBurnerUniswapV3 | [018](018-liquidity-strategy.md#adr-018-liquidity-strategy-balancer-8020-pol), [026](026-tokenomics.md#adr-026-tokenomics) | Yes | USDC, TOKEN (transient) | `AccessControl`, `ReentrancyGuard`, `Pausable` (via the shared `SunsettingPausable` base) (swap-and-burn path; receives 30% of every settlement. Three layers: `BuybackBurner` is the venue-neutral abstract base and owns what every burner shares — the `KEEPER_ROLE` entry point `executeBuyback(amountIn, minOut)`, the actual-TOKEN-balance-delta check, the burn, USDC rescue, and pausing. `GuardedBuybackBurner` is an abstract layer that adds the MEV-defense stack once for every venue — a TWAP-derived `minOut` floor, the governed `slippageBps` / `minBuybackAmount` / `maxBuybackAmount` band, and a per-epoch USDC liquidity cap. `BuybackBurnerBalancerV3` and `BuybackBurnerUniswapV3` are the concrete contracts that bind a live venue; the deployment picks one per [ADR 018 § Venue-neutral burner selection](018-liquidity-strategy.md#venue-neutral-burner-selection)) |
...
| Ed25519Verifier | [003](003-payments.md#nodeid-ownership-verification), [019](019-node-onboarding.md#adr-019-node-onboarding-and-bootstrapping-flow) | No | — | None — a stateless `view` verifier behind the frozen `IEd25519Verifier` interface. It holds no storage, no funds, and no roles, so no OZ base contract applies. It performs the RFC 8032 PureEdDSA check that `CapacityBond` runs on the NodeId ownership proof in `registerNode`, `bindNodeId`, and `reclaimNodeId`. The core group check is delegated to the audited Smoo.th Crypto Lib EIP-6565 routine; the contract adds the public-key decompression and the strict guards (reject small-order points, non-canonical `y`, off-curve keys) that hold it at parity with `ed25519-dalek::verify_strict`, the check deCDN nodes run off-chain per [ADR 003 § NodeId Ownership Verification](003-payments.md#nodeid-ownership-verification) |
...
The parameter carries no `[floor, ceiling]` safety rail, because a hash has no monotonic direction to bound. The rail is instead a governance norm: every proposal that sets `currentTermsHash` references the terms text and its review record. Enforcement is evaluated at registration only — a later bump binds new registrants, and operators already registered keep the acceptance their `TermsAccepted` record holds.
...
| 7 | BuybackBurner | USDC address, TOKEN address, admin address (all three zero-rejected by the abstract base), then a venue `Config` struct. For `BuybackBurnerBalancerV3` that struct carries the Balancer V3 Router address (zero-rejected), the Permit2 address (zero-rejected), the Balancer V3 Vault address, the initial pool contract `address`, the sub-swap cadence pair, and the `GuardedBuybackBurner` band (`twapMinWindow`, `maxBuybackAmount`, `minBuybackAmount`, `slippageBps`, `epochLiquidityCapFraction`). Vault and pool may both be zero-address at deploy and are wired later via `setVault(address)` / `setPool(address)`; `executeBuyback` reverts `PoolNotWired` until both are set. A `Config` that supplies both at deploy is validated immediately — the pool must be Vault-registered and hold at least the {USDC, TOKEN} legs. See [ADR 003](003-payments.md#buybackburner) for the interface and [ADR 018](018-liquidity-strategy.md#adr-018-liquidity-strategy-balancer-8020-pol) for the venue rationale. **Inflow source:** `FeeRouter` (30% of every settlement per [ADR 026 § Slashing and burn](026-tokenomics.md#slashing-and-burn)). **Router address and naming:** see [ADR 018 § Buyback execution via Balancer V3](018-liquidity-strategy.md#buyback-execution-via-balancer-v3). **Approvals note:** the approval target is **Permit2**, not the Vault. A Balancer V3 Router pulls `tokenIn` through `permit2.transferFrom`, so each swap authorizes the spend in two scoped legs — ERC20-approve Permit2 for exactly `amountIn`, then grant the Router a Permit2 allowance for exactly `amountIn` with a `block.timestamp` expiry — and resets both to zero after the swap. The Vault is never an approval target; it is read-only here (pool registration, static swap fee, scaled-18 balances). |
| 8 | FeeRouter | USDC address, **CapacityBond address**, **`TimelockController` address** (treasury bucket destination), `epochLength` (1 week; the canonical served-bytes voting-weight clock per [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight)), `windowEpochs` (initial trailing-window length, default 13, bounds-checked `[4, 26]` per [ADR 036 § Governable parameters](036-served-bytes-voting-weight.md#governable-parameters-with-safety-bounds)), admin address, launch split shares per [ADR 026 § Governable parameters with safety bounds](026-tokenomics.md#governable-parameters-with-safety-bounds) (cross-validated against dependency addresses). The USDC, CapacityBond, treasury, and admin addresses are rejected as zero, and `epochLength` is rejected as zero. **CapacityBond is a deploy-time dependency, not a stored one:** the constructor reads `CapacityBond.epochLength()` once and reverts `EpochLengthMismatch` unless it equals the `epochLength` the deployer passes, then drops the reference. This holds both contracts on one epoch clock, so `CapacityBond.slashedAtEpoch` and `FeeRouter.bytesPerEpoch` carry comparable epoch indices for the [ADR 036 § Slashing zero-out](036-served-bytes-voting-weight.md#slashing-zero-out). **Dependency address** (`buybackBurner`) may be `address(0)` at deploy and set later via the governance-mutable setter in [§ Tunable Economics](#tunable-economics); the cross-validation invariant ensures any non-zero share has a non-zero destination at construction time. Steady-state target shares are `6000 / 3000 / 1000` in basis points. |
...
| Function | External Calls | Guards |
| --- | --- | --- |
| `executeBuyback()` | `BalancerV3Router.swapSingleTokenExactIn()` (swaps contract-held USDC; the Router pulls the input tokens via a scoped Permit2 allowance and settles to the Vault), `IERC20.safeTransfer()` (TOKEN to burn) | `nonReentrant`, checks-effects-interactions, `KEEPER_ROLE` |
