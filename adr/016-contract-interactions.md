...
- **Constructor `Config`.** USDC, TOKEN, and admin are zero-rejected by the abstract base. For `BuybackBurnerBalancerV3` the struct carries the Balancer V3 Router address (zero-rejected), the Permit2 address (zero-rejected), the Balancer V3 Vault address, the initial pool `address`, the sub-swap cadence pair, and the `GuardedBuybackBurner` band (`twapMinWindow`, `maxBuybackAmount`, `minBuybackAmount`, `slippageBps`, `epochLiquidityCapFraction`).
