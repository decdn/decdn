...
| Off-chain owner | PaymentPool | `ownerPoolNonce(address owner) returns (uint256)`, `getPool(bytes32 poolId) returns (Pool)`, `getAuthorization(bytes32 poolId, address signer) returns (Authorization)` | An owner recovers its pools after a restart by recomputing each `poolId` from its own nonce sequence and reading `getPool`; a node reconstructs its lanes from its own persisted store and confirms each via `watermark[poolId][signer][provider]` — a pool names no provider, so it is not node-enumerable | [ADR 003](003-payments.md#adr-003-payment-model) |
