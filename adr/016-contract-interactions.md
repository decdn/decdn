...
| PaymentPool | [003](003-payments.md#adr-003-payment-model) | Yes | USDC | `AccessControl`, `ReentrancyGuard`, `Pausable`, `EIP712` (USDC-only; the USDC address is fixed at deployment; one funded pool backs many capped signers to many nodes; `redeem` forwards each paid amount to `FeeRouter.routeSettlement` rather than skimming inline) |
...
classDiagram
    class PaymentPool {
        +openPool(deposit)
        +redeem(voucher, sig, chainIndex, preimage, cap)
        +redeemMany(entries)
        +closePool(poolId)
        +reclaim(poolId)
    }
...
| 9 | PaymentPool | USDC address, CapacityBond address, FeeRouter address, `disputeWindow` (48h grace window), `deliveryFloor` ([ADR 003](003-payments.md#adr-003-payment-model)). A pool is one deposit backing many capped signers to many nodes; `redeem` does not skim a protocol fee inline — it transfers each paid amount to `FeeRouter.routeSettlement(operator, bytesDelivered, amount)` in the same transaction. `setFeeRouter(address)` is governance-mutable per [§ No proxy deployment patterns](#no-proxy-deployment-patterns) carve-out. `MAX_CHAIN_LENGTH` (1024) is a compile-time constant, not a constructor argument or a governable knob: it bounds the redemption chain walk, so raising it would make a redemption's gas unbounded. The register stores no chain state — a redemption carries its own voucher and preimage. |
...
| Off-chain owner | PaymentPool | `ownerPoolNonce(address owner) returns (uint256)`, `getPool(bytes32 poolId) returns (Pool)`, `getAuthorization(bytes32 poolId, address signer) returns (Authorization)` | An owner recovers its pools after a restart by recomputing each `poolId` from its own nonce sequence and reading `getPool` | [ADR 003](003-payments.md#adr-003-payment-model) |
| Off-chain node | PaymentPool | `watermark(bytes32 poolId, address signer, address provider) returns (Lane)` | A node seeds an unknown lane's floor from the paid `{amount, bytesDelivered}` on first contact, so its local store is an index and a fast path rather than the source of truth. The read is **lazy and per-lane**: a pool names no provider and is therefore not node-enumerable, and lazy seeding is what makes that irrelevant — the node only ever needs the lane a request names. See [ADR 003 § Off-chain voucher state persistence](003-payments.md#off-chain-voucher-state-persistence) | [ADR 003](003-payments.md#adr-003-payment-model) |
