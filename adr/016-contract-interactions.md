...
| 4 | StakingRegistry | TOKEN address, `minStake` (**50,000 TOKEN** per [ADR 026](026-gauge-boost-tokenomics.md) §7), `unbondingPeriod` (**14 days** — raised from 7 days per [ADR 030 §7](030-blake3-merkle-verification.md#7-interaction-with-stake-unbonding--unbondingperiod-raised-to-14-d) to cover the worst-case Merkle-bisection lifecycle; bounds `[3 d, 30 d]` per [ADR 009](009-governance.md)). No discount-threshold parameters; operator return is differentiated through the gauge-boost flow in `FeeRouter`. |
...
| `submitCorruptionChallenge()` (v1 / optimistic — `cdn/client/v1`) | `IERC20.safeTransferFrom()` (TOKEN bond deposit) | `nonReentrant`, checks-effects-interactions |
| `submitMerkleCorruptionChallenge()` (v2 / Merkle bisection — `cdn/client/v2`; per [ADR 030](030-blake3-merkle-verification.md)) | `IERC20.safeTransferFrom()` (initial TOKEN bond deposit; per-round bonds via subsequent `bisectMove` calls) | `nonReentrant`, checks-effects-interactions |
| `bisectMove()` (v2 bisection step) | `IERC20.safeTransferFrom()` (per-round bond deposit by the moving side) | `nonReentrant`, checks-effects-interactions |
| `terminalReveal()` (v2 round N+1) | None (state-only; on-chain BLAKE3 chunk hash + bao parent path verification) | `nonReentrant`, checks-effects-interactions |
| `resolveMerkleCorruption()` (v2 resolution) | `StakingRegistry.slash()` on defender-loss branch, `IERC20.safeTransfer()` (slash reward + bond return to winner) | `nonReentrant`, checks-effects-interactions |
| `forfeitDispute()` (v2 voluntary concede) | Defender-forfeit branch: `StakingRegistry.slash()` + `IERC20.safeTransfer()` (slash reward to challenger + bond return) — equivalent to a defender timeout. Challenger-forfeit branch: `IERC20.safeTransfer()` only (bond forfeit per ADR 014 §3 challenger-loss split: 50% burned, 50% to defender; no `StakingRegistry.slash()` call). | `nonReentrant`, checks-effects-interactions |
