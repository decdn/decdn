...
| 12 | OriginAssignment | StakingRegistry, PublisherRegistry, ContentBlacklist (latter may be zero at deploy; bound via `setContentBlacklist`), and the initial default-open operator set (seeds the `namespaceId == 0` allow-list at genesis; must satisfy the `[defaultOpenMinRedundancy, defaultOpenMaxOrigins]` size bound and no-duplicates checks, same as `setDefaultOpenAllowlist`; not staking-validated on-chain — no operator is staked at genesis, so liveness is an off-chain runtime concern). Min-redundancy, timelock, and default-open parameters are governance-controlled. See [ADR 011 § Origin Assignment Authority](011-content-takedown.md#origin-assignment-authority) and [§ OriginAssignment construction notes](#originassignment-construction-notes) below. |
...
- **Default-open allow-list seed.** The constructor seeds the `namespaceId == 0` allow-list with an explicit initial operator set, validated against the same `[defaultOpenMinRedundancy, defaultOpenMaxOrigins]` size bound and no-duplicates checks as `setDefaultOpenAllowlist`, and emits `DefaultOpenAllowlistUpdated` (updateIndex 0). The genesis seed is not staking-validated on-chain (no operator is staked at construction time); the per-operator `StakingRegistry.isActive` check applies to post-deploy governance updates only. `isAuthorizedOrigin(0, op)` is strict set-membership from block zero — there is no permissive bootstrap window and no `defaultOpenAllowlistActive` latch.
...
| `setDefaultOpenAllowlist(operators[])`, `addDefaultOpenOperator(operator)`, `removeDefaultOpenOperator(operator)` | `StakingRegistry.isActive(operator)` per operator (read) | `GOVERNANCE_ROLE`; emits `DefaultOpenAllowlistUpdated`; resulting set size must be within `[defaultOpenMinRedundancy, defaultOpenMaxOrigins]`; duplicates revert; runs under the Governor's standard 48h timelock |
...
| Default-open allow-list | Seeded at deploy with the initial operator set; `isAuthorizedOrigin(0, op)` strict membership from genesis | Same (governance-managed via `setDefaultOpenAllowlist` / `add` / `remove`) |
