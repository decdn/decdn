...
| **Payment-channel validity.** Tier 3 may change the channel-ID formula or voucher format, invalidating **open channels** on the old contract deployment (release notes state this). Separately, when governance de-allowlists a stablecoin, its open channels are force-closed via `forceCloseChannel` ([ADR 003 § Force-close channels in removed tokens](003-payments.md#force-close-channels-in-removed-tokens)). | Close open old-protocol channels before the cutover window or risk losing payments. |
...
For Tier 3 upgrades touching governance-controlled parameters (rate bounds in [ADR 003 §Rate Bounds Refresh](003-payments.md), stablecoin allowlist in [ADR 003 § Stablecoin allowlist](003-payments.md#stablecoin-allowlist-governance-managed), slashing schedule in [ADR 026 §8](026-gauge-boost-tokenomics.md)):
...
| `closeChannel` reverts with new contract | Voucher format changed; old vouchers invalid against the redeployed `StablePaymentChannel` | Close/settle old-deployment channels before cutover. `forceCloseChannel` ([ADR 003](003-payments.md#force-close-channels-in-removed-tokens)) applies only to channels whose stablecoin was de-allowlisted; the dispute window applies. |
...
| Mass `closeChannel` calls clog the L2 sequencer | Large fleets force-closing all old-format channels at once | Batch closes across operators; use `forceCloseChannel` only for stablecoins removed from the allowlist; let unforced channels settle naturally over their `maxChannelDuration` (default 90 days per [ADR 003](003-payments.md)). |
...
- [ADR 003 — Force-close channels in removed tokens](003-payments.md#force-close-channels-in-removed-tokens)
