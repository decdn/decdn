...
> **EWMA/clamp interaction:** When `reporter_weight` exceeds ~2.0, the EWMA delta is almost always overridden by the ±0.05 per-report clamp for score gaps > 0.5. The effective influence range is therefore 0–2× rather than 0–5×. This is an accepted property: the clamp prevents large single-report swings regardless of reporter credibility.
