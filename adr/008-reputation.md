...
> **Attack vector: reporter weight divergence targeting.** Because `max_effective_settled_value_observed` is local to each node, newer nodes with a lower observation history are disproportionately influenced by high-weight reporters. An attacker with high `effective_settled_value` could target newer nodes with false reports, knowing the reporter weight cap is relative to the target's local maximum. **Mitigation:** the 70/30 local/network weight split limits network report impact. Per-report clamping (±0.05 in production) bounds per-report damage regardless of reporter weight. The cold-start bootstrap bonus gives new nodes a grace period with elevated scores, reducing the impact of early negative reports. This attack is documented as a known limitation; production monitoring should track sudden score drops on recently registered nodes.
...
| Score clamping | Not implemented — PoC uses local-only scoring with no gossip propagation, so no cross-node report surface exists to clamp. The per-report cap is meaningful only when processing `ReputationReport` gossip messages (production-only). | Per-report ±0.05 cap |
