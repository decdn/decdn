...
[ADR 027 §6](027-distinct-client-receipts.md#6-implementation-sequencing-and-launch-prerequisite) establishes that the 40% gauge bucket MUST NOT pay out until distinct-client diversity gating is live and the per-operator gauge-share cap from §3 above is enforced — wash-trading at TOKEN price 3–5× genesis is net-profitable without these gates. This sub-section pins the contract-level mechanism for that pause and the cutover.
...
Per-operator pool share = `min(working_bytes_i / sum(working_bytes), MAX_GAUGE_SHARE_PER_OPERATOR)`, where `working_bytes_i = min(bytes_i, 0.4·bytes_i + 0.6·(ve_i/total_ve)·total_bytes)` over the epoch's verified bytes (full derivation in design spec §9.4). `bytes_i` is sourced from `FeeRouter.bytesPerEpoch[operator][epoch]` per [ADR 027 §1](027-distinct-client-receipts.md#1-feerouter-per-epoch-bookkeeping); the diversity gate `distinctClientCount[operator][epoch] >= MIN_DISTINCT_CLIENTS_PER_EPOCH` must also hold per [ADR 027 §3](027-distinct-client-receipts.md#3-identity-diversity-gating), or the operator's gauge share for the epoch is zero.
...
**Rationale.** Defense-in-depth against wash-trading. The distinct-client diversity gate ([ADR 027](027-distinct-client-receipts.md)) raises the per-Sybil capital floor; the per-operator cap bounds the upside even if a sophisticated attacker bypasses the diversity heuristics (e.g., funding Sybils through public mixers, CEX cycles, or DEX swaps to disguise common ancestry — see [ADR 027 §Risks](027-distinct-client-receipts.md#risks)). With the cap, the gauge-share gain from wash-trading is bounded at 5% of the gauge bucket regardless of inflated `bytes_i` claims, shifting attack ROI decisively toward unprofitable. A single honest operator with a dominant byte share is also subject to the cap, which is the intended posture — the gauge pool exists to incentivize a diverse operator set, not to reward concentration.
...
- **Wash-trading / self-routed traffic.** An operator could induce noise settlements to inflate gauge-pool share. Two layered mitigations close the surface: (1) **distinct-client diversity gating** at `FeeRouter.routeSettlement` time, requiring `MIN_DISTINCT_CLIENTS_PER_EPOCH` distinct funded channels per operator per epoch with a funding age and per-operator cooldown — see [ADR 027 §3](027-distinct-client-receipts.md#3-identity-diversity-gating); and (2) the **per-operator gauge-share cap** from §3 above, bounding the wash-trading payoff at `MAX_GAUGE_SHARE_PER_OPERATOR` (default 5%) of the gauge bucket regardless of how many Sybils clear the diversity gate. The launch prerequisite is contract-pinned in §2 Pre-launch gauge accumulation: `gaugeLaunched == false` escrows the 40% gauge bucket per epoch, and the one-shot `enableGauge()` setter is the only path to live gauge payouts. (See also [ADR 027 §6 — Implementation sequencing and launch prerequisite](027-distinct-client-receipts.md#6-implementation-sequencing-and-launch-prerequisite).)
