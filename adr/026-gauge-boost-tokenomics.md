...
The 20% floor on the node-base share guarantees operators always receive enough liquid USDC to cover at least a meaningful fraction of infrastructure costs even under extreme governance proposals — preserves the cashflow invariant. The `boostFloor` bounds prevent governance from collapsing the gauge pool to a winner-take-all distribution (lower-bound) or flattening it into uselessness (upper-bound). The `MAX_GAUGE_SHARE_PER_OPERATOR` bounds prevent governance from disabling the wash-trading defense (lower bound implicitly enforced by the cap being non-zero) or so over-tightening that legitimate large operators are starved (upper bound). The `epochLiquidityCapFraction` parameter is the combined per-epoch ceiling on USDC notional swapped through the Balancer V3 80/20 pool across both [`BuybackBurner` execution](018-liquidity-strategy.md#buyback-execution-via-balancer-v3) and the [delegator-pool swap path](018-liquidity-strategy.md#delegator-pool-swap-path-parallel-usdctoken-conversion-on-the-same-pool), expressed as a fraction of in-pool USDC depth captured at epoch start; the 1% floor prevents governance from starving the swap paths under sustained revenue, the 30% ceiling prevents a single epoch from draining pool depth, and the 10% default sizes one epoch's combined swap pressure conservatively against worst-case sustained execution per the [Liquidity-cap interaction](018-liquidity-strategy.md#liquidity-cap-interaction) requirement that the cap be a single pool-wide budget shared across both paths.
