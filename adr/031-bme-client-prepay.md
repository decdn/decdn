...
The protocol will, in v2, add an optional client-side TOKEN-prepay path implementing the Burn-and-Mint Equilibrium pattern. **This ADR is Deferred — no v1 implementation is committed.** The decision recorded here is forward-looking: it pins the design intent, the v1 prerequisites that must be preserved, and the design surface that v2 will fill in.
...
Clients may optionally prepay bandwidth in TOKEN at a discount to the equivalent USDC rate. The discount is governable, sized in v2 to motivate adoption without subsidizing the path past the point where the network burn flow exceeds the discount cost. The source-spec range is 5–8% discount; this ADR does not pin a specific value — v2 will calibrate based on then-observed TOKEN price stability, oracle-manipulation cost, and competing demand-side levers.

| Parameter | v2 sizing intent | Notes |
| --- | --- | --- |
| Discount vs equivalent USDC rate | Source-spec range 5–8%; final value deferred | Governable within bounds set in v2 |
| Discount lower bound | Non-zero (otherwise no client incentive) | Set at v2 launch |
| Discount upper bound | Capped to avoid net-negative flow vs operator USDC base share | Set at v2 launch |
...
**Opt-in for operators.** Operators may refuse BME-routed traffic if their cost-of-conversion exceeds the discount. This requires a per-operator advertised flag in probe / stream responses (forward-referenced — design fold-in deferred to v2). Operators that accept BME traffic are entitled to the same per-byte revenue (in USDC-equivalent) as they would have received via the USDC channel rail.
...
| Burn timing | At consumption (per-settlement or per-epoch; deferred to v2) | Granularity is a v2 parameter; finer granularity = more burn events; coarser = lower gas cost |
...
v1 (USDC channels) and v2 (BME prepay + USDC channels) must both be supported in the production protocol. The two paths must coexist at the operator-payment boundary without either being aware of the other's wire-level details.
...
| Oracle requirement | v2 sizing intent | Notes |
| --- | --- | --- |
...
| TWAP window | Long enough to make manipulation cost > expected attacker gain | Deferred to v2; depends on observed pool depth |
| Multi-source check | Required for v2 launch | Single-source TWAP is insufficient against a determined attacker with capital |
...
| `consume(client, operator, bytesDelivered, vouchers)` | Burns TOKEN, computes USDC-equivalent, disburses to operator (per §3 option) | Authorization model deferred to v2 |
...
- **Mechanism only matters at scale.** Low-usage networks see negligible BME burn; the v1 cost is concentrated and the v1 benefit is small. Deferring to v2 mitigates this but does not eliminate the structural constraint.
...
## v1 prerequisites (for future v2 implementation)
...
## Open questions (v2 will answer)

- Final discount value (within governable bounds; v2 calibration based on observed TOKEN-price stability and oracle-manipulation cost).
