...
- **[Appendix: Observability](appendix-observability.md) observability surface.** Launch should add metrics for "USDC channel rail revenue per byte" and "USDC channel rail bytes per operator per epoch" with a label dimension that can later be split by payment-rail (USDC vs BME). Avoiding a label-shape change later costs less than retrofitting it under load.
...
- Burn timing granularity (per-settlement vs per-epoch) and gas-cost tradeoff on the chosen L2 ([Appendix: L2 Deployment](appendix-l2-deployment.md)).
