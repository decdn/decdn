...
| P-19 | Offline lease blast radius | Up to 500 `K_blob` values extractable from a compromised device's sealed lease | T4 | [006](appendix-encrypted-content-publishing.md) §Offline Leases |
| P-20 | No forward secrecy for epoch keys | Compromising `server_secret` retroactively exposes all past and future epoch keys until rotation | T4 | [006](appendix-encrypted-content-publishing.md) §Consequences |
...
| 5 | Epoch key forward secrecy (§5.5) | Pre-mainnet | Medium | High — but already specified in the encrypted-content-publishing appendix; implementation priority |
