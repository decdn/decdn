...
> **[ADR 026](026-tokenomics.md) alignment.** Under [ADR 026](026-tokenomics.md), this ADR's `BuybackBurner` is router-driven (5% of routed USDC at settlement, not manual treasury transfers), and a parallel delegator-pool USDC→TOKEN swap path runs through the same Balancer V3 80/20 pool. The pool design is unchanged — Balancer V3, 80/20 TOKEN/USDC, 1% swap fee, MEV protection via TWAP + `minTokenOut`, POL custody by the Timelock, and the activation criteria. The genesis liquidity allocation is **100M TOKEN at genesis (10% of the 1B supply)**. Both swap paths use **Flashbots-style private-RPC routing and per-epoch liquidity caps** (mandatory, not optional).

Buyback execution is deferred to production because thin TOKEN/USDC pool liquidity at genesis cannot absorb buyback flow without unacceptable slippage. [ADR 016](016-contract-interactions.md) recommends TWAP execution as MEV mitigation but does not specify how pool depth is created.

Questions answered by this ADR:

1. How is the 10% genesis `Liquidity (DEX + CEX)` allocation ([ADR 026 §1](026-tokenomics.md#1-supply-and-distribution)) actually deployed into the pool?
...
The treasury holds the 100M TOKEN POL allocation per [ADR 026 §1](026-tokenomics.md#1-supply-and-distribution) and is USDC-poor at PoC scale — no meaningful USDC reserve to pair 50/50 against TOKEN. The small protocol team has no bandwidth to operate a concentrated-liquidity keeper stack. Both constraints favour a venue minimizing USDC requirements and operational burden.
...
| MEV protection for buybacks | TWAP + `minTokenOut` + **mandatory Flashbots-style private-RPC routing** + per-epoch liquidity cap (per [ADR 026](026-tokenomics.md) hardening); CoW Swap batch-auction routing as an independent conditional add-on, pending verification that CoW solvers route through Balancer V3 weighted pools (see Consequences) | Requires custom TWAP + private mempool (Flashbots Protect) |
...
- **Source:** The `Genesis liquidity (POL)` allocation (10% of supply = 100M TOKEN of the 1B supply per [ADR 026 §1](026-tokenomics.md#1-supply-and-distribution)) funds the TOKEN side. The USDC side is drawn from the treasury.
- **PoC seed:** A minimal position for integration testing. Exact amount is chosen by governance at deployment; the target range of $5K–$20K USDC equivalent per side reflects expected PoC scale (20–100 nodes). The range is wide because the point estimate depends on observed PoC traffic; deployment scripts should accept the seed size as a parameter, not hardcode it.
...
Under [ADR 026](026-tokenomics.md), `BuybackBurner` no longer relies on manual treasury transfers. The `FeeRouter` contract receives the full operator USDC balance from `PaymentChannel.settleChannel` at every settlement and atomically forwards **5% of routed USDC directly to `BuybackBurner` in the same transaction**, alongside the other fee buckets. The full router split lives in [ADR 026 §2](026-tokenomics.md#2-feerouter-split-40407553) (six buckets summing to 100%); this ADR cross-references rather than duplicating it.

**Implications for this ADR:**

- **Inflow source.** USDC arrives at the `BuybackBurner` address from the `FeeRouter` via a same-transaction `transfer` inside `routeSettlement`. Manual treasury transfers as the funding pattern are obsolete. Anywhere prior drafts of this ADR or [ADR 016](016-contract-interactions.md) imply a manual or batched-treasury inflow path, the operative pattern is now router-driven, per-settlement inflow.
- **Inflow rate.** [ADR 026](026-tokenomics.md) routes 5% of gross routed USDC (the full operator balance, post-settlement) to `BuybackBurner`. Production sizing of `maxBuybackAmount`, the keeper schedule, and the slippage-budget assumptions in the [Activation Criteria](#activation-criteria-production) below should be tuned against this rate.
...
- **Per-sub-swap guard** is derived from remaining required output and remaining sub-swaps: `remainingMinOut = minTokenOut − cumulativeReceived`; `perSubSwapLimit = ceilDiv(remainingMinOut, remainingSubSwaps)`. This keeps the aggregate bound tight even under uneven fills.
- **`maxBuybackAmount` is a per-transaction cap**, consistent with its definition at [ADR 016 — BuybackBurner reentrancy row](016-contract-interactions.md#buybackburner). Each sub-swap (its own transaction) MAY be as large as `maxBuybackAmount`; the *total* buyback is bounded by the caller-supplied `amount`, not by `maxBuybackAmount × subSwapCount`. Dividing the per-transaction cap by the split count would unnecessarily restrict processing of accumulated fees.
- **Spacing.** Sub-swaps are separated by at least `subSwapMinBlockGap` blocks.
- **Required: Flashbots-style private-RPC routing (per [ADR 026](026-tokenomics.md) and the market-dynamics analysis §1).** The keeper MUST submit every buyback sub-swap (and every delegator-pool sub-swap, see [Delegator-pool swap path](#delegator-pool-swap-path-parallel-usdctoken-conversion-on-the-same-pool) below) through a private-RPC bundle endpoint that bypasses the public mempool — Flashbots Protect, MEV-Share, or the equivalent on the production L2. Public-mempool routing is **not** an acceptable fallback: programmatic buybacks are a known MEV target (front-run-and-dump around `executeBuyback` calls), and TWAP + `minTokenOut` alone do not prevent a searcher from observing the pending transaction and skewing the pool against the protocol within the slippage budget. This is a hard requirement; the keeper configuration MUST refuse to publish to a public RPC.
- **Required: per-epoch liquidity cap.** Independent of `maxBuybackAmount` (a per-transaction cap), the contract MUST enforce a per-epoch aggregate cap on the USDC notional swapped through the Balancer V3 pool across all buyback executions in a single 1-week epoch. The cap is sized as a fixed fraction of in-pool USDC depth at epoch start, so even a worst-case sustained-execution profile cannot exhaust pool depth. This is a hard requirement, not a tunable nicety; the cap also bounds the delegator-pool swap path's pressure on the same pool (see below). The fraction is the `epochLiquidityCapFraction` parameter in [ADR 026 §11](026-tokenomics.md#11-governable-parameters-with-safety-bounds) — default 10%, bounded `[1%, 30%]`, governance-tunable through the 48-hour timelock per [ADR 009](009-governance.md). Operational tuning is informed by the `finance/notebooks/` analyses.
- **Conditional add-on: CoW Swap routing.** The keeper MAY route the buyback through CoW Swap instead of calling the Router directly, **if CoW solvers route through the deployed Balancer V3 weighted pool at best-execution time**. This is a structural MEV-protection layer (batch auctions, no public mempool exposure) complementing TWAP but NOT guaranteed available for Balancer V3 as of this ADR's date: Balancer's V2-only aggregator fee-discount program was explicitly not extended to V3, and CoW's solver routing coverage of V3 weighted pools is still maturing. **Before enabling CoW routing in production, the operator MUST verify via CoW's `/api/v1/quote` endpoint that the target pool is reachable and that quoted prices are within `slippageBps` of the Router-direct path.** If CoW routing is unavailable or regressed, fall back to direct Router + TWAP.
...
[ADR 035](035-delegator-pool.md#delegator-pool--usdc--token-conversion) introduces a 7% delegator-pool bucket also denominated in TOKEN, not USDC. The conversion path runs through **the same Balancer V3 80/20 pool** this ADR seeds for buyback execution — a second consumer of the same pool with the same MEV-defense requirements as `BuybackBurner`.

**Mechanics.**

1. `FeeRouter` accumulates 7% of routed USDC into the delegator-pool epoch bucket per epoch.
2. At epoch rollover (or via keeper trigger within the epoch), the bucket's USDC is swapped to TOKEN against the same Balancer V3 80/20 pool, under the same execution rules as `BuybackBurner`: TWAP splitting, `minTokenOut` aggregate guard, `subSwapMinBlockGap` spacing, mandatory private-RPC routing, and the per-epoch liquidity cap (spanning both swap paths combined).
3. The acquired TOKEN is held in the per-epoch delegator bucket until claimed pro-rata by ve-balance (mechanic per [ADR 035](035-delegator-pool.md#delegator-pool--usdc--token-conversion); not this ADR's concern).

**Implementation per [ADR 016 § Shared swap helper](016-contract-interactions.md#shared-swap-helper-buybackburner--delegatorbuyer).** A parallel `DelegatorBuyer` contract handles the delegator-pool path. It shares Balancer V3 swap execution — TWAP splitting, `minTokenOut` aggregate guard, `subSwapMinBlockGap` spacing, private-RPC routing, and combined per-epoch-cap accounting — with `BuybackBurner` through an internal `BalancerV3SwapHelper` abstract contract inherited by both. Separate contract addresses preserve independent governance setters (`FeeRouter.setBuybackBurner` / `setDelegatorBuyer`) and divergent downstream value flows (burn vs deposit-back).
...
| Per-epoch liquidity cap (combined buyback + delegator) | N/A (execution disabled) | **Required** — `epochLiquidityCapFraction` of in-pool USDC depth at epoch start (default 10%, bounded `[1%, 30%]` per [ADR 026 §11](026-tokenomics.md#11-governable-parameters-with-safety-bounds)) | Yes (the fraction; presence of the cap is structural) |
...
5. Either (a) a keeper with `KEEPER_ROLE` is operational and configured to call `executeBuyback()` (and the delegator-pool swap entry point per [Delegator-pool swap path](#delegator-pool-swap-path-parallel-usdctoken-conversion-on-the-same-pool)) on a schedule, or (b) governance is prepared to trigger executions manually.
...
- TWAP + `minTokenOut` guards provide deterministic on-chain MEV protection. Combined with the [ADR 026](026-tokenomics.md) hardening — mandatory Flashbots-style private-RPC routing and a per-epoch liquidity cap shared across the buyback and delegator-pool swap paths — programmatic-buyback front-running (the canonical MEV pattern flagged in the market-dynamics analysis) is closed off both by transaction-graph invisibility (private RPC) and execution shape (TWAP, slippage guard, hard cap on per-epoch pool pressure). CoW Swap batch-auction routing is an independent conditional add-on if CoW solvers route through the deployed pool (verified per the activation criteria), not a load-bearing assumption.
- Balancer V3's new Vault architecture (transient accounting via `unlock`/settle, unified scaling) specifically mitigates the bug class exploited in V2 (see Negative section). Choosing V3 over V2 materially reduces smart-contract risk.
...
- [ADR 026 — Gauge-Boost Tokenomics](026-tokenomics.md) — Source of the router-driven inflow, the 5%-of-routed-USDC buyback rate, the parallel delegator-pool swap path, and the hardened MEV-mitigation requirements applied to this ADR
