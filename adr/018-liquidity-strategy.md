...
> **[ADR 026](026-gauge-boost-tokenomics.md) alignment.** Under [ADR 026](026-gauge-boost-tokenomics.md), this ADR's `BuybackBurner` is router-driven (5% of routed USDC at settlement, not manual treasury transfers), and a parallel delegator-pool USDC→TOKEN swap path runs through the same Balancer V3 80/20 pool. The pool design is unchanged — Balancer V3, 80/20 TOKEN/USDC, 1% swap fee, MEV protection via TWAP + `minTokenOut`, POL custody by the Timelock, and the activation criteria. The genesis liquidity allocation is **100M TOKEN at genesis (10% of the 1B supply)**. Both swap paths use **Flashbots-style private-RPC routing and per-epoch liquidity caps** (mandatory, not optional).
