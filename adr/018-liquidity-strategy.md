...
- TWAP + `minTokenOut` guards provide deterministic on-chain MEV protection. Combined with the [ADR 026](026-gauge-boost-tokenomics.md) hardening — mandatory Flashbots-style private-RPC routing and a per-epoch liquidity cap shared across the buyback and delegator-pool swap paths — programmatic-buyback front-running (the canonical MEV pattern flagged in the market-dynamics analysis) is closed off both by transaction-graph invisibility (private RPC) and execution shape (TWAP, slippage guard, hard cap on per-epoch pool pressure). CoW Swap batch-auction routing is an independent conditional add-on if CoW solvers route through the deployed pool (verified per the activation criteria), not a load-bearing assumption.
...
- One new interface method (`setPool(address)`) must be added to `IBuybackBurner` — a minor extension, but must be reflected in the ADR 003 interface block. V3's approvals footgun (approve the Vault, call the Router) must also be documented in deployment runbooks to prevent a common integration mistake.
