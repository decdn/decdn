...
**Implementation choice — deferred to [ADR 016](016-contract-interactions.md).** Either extend `BuybackBurner` with a multi-output mode (lower-friction default; halves audit surface) or add a parallel `DelegatorBuyer` (cleaner audit boundary; duplicates Router/Vault plumbing). Both preserve the MEV-defense invariants below.
