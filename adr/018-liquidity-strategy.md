...
> **[ADR 026](026-gauge-boost-tokenomics.md#adr-026-gauge-boost-tokenomics) alignment.** Under [ADR 026](026-gauge-boost-tokenomics.md#adr-026-gauge-boost-tokenomics), this ADR's `BuybackBurner` is router-driven (5% of routed USDC at settlement, not manual treasury transfers), and a parallel delegator-pool USDC→TOKEN swap path runs through the same Balancer V3 80/20 pool. The pool design is unchanged — Balancer V3, 80/20 TOKEN/USDC, 1% swap fee, MEV protection via TWAP + `minTokenOut`, POL custody by the Timelock, and the activation criteria. The genesis liquidity allocation is **100M TOKEN at genesis (10% of the 1B supply)**. Both swap paths use **Flashbots-style private-RPC routing and per-epoch liquidity caps** (mandatory, not optional).
...
| MEV protection for buybacks | TWAP + `minTokenOut` + **mandatory Flashbots-style private-RPC routing** + per-epoch liquidity cap (per [ADR 026](026-gauge-boost-tokenomics.md#adr-026-gauge-boost-tokenomics) hardening); CoW Swap batch-auction routing as an independent conditional add-on, pending verification that CoW solvers route through Balancer V3 weighted pools (see Consequences) | Requires custom TWAP + private mempool (Flashbots Protect) |
...
Under [ADR 026](026-gauge-boost-tokenomics.md#adr-026-gauge-boost-tokenomics), `BuybackBurner` no longer relies on manual treasury transfers. The `FeeRouter` contract receives the full operator USDC balance from `PaymentChannel.settleChannel` at every settlement and atomically forwards **5% of routed USDC directly to `BuybackBurner` in the same transaction**, alongside the other fee buckets. The full router split lives in [ADR 026 §2](026-gauge-boost-tokenomics.md#2-feerouter-split-40407553) (six buckets summing to 100%); this ADR cross-references rather than duplicating it.
...
- **Inflow rate.** [ADR 026](026-gauge-boost-tokenomics.md#adr-026-gauge-boost-tokenomics) routes 5% of gross routed USDC (the full operator balance, post-settlement) to `BuybackBurner`. Production sizing of `maxBuybackAmount`, the keeper schedule, and the slippage-budget assumptions in the [Activation Criteria](#activation-criteria-production) below should be tuned against this rate.
...
[ADR 026 §6](026-gauge-boost-tokenomics.md#6-delegator-pool-usdc-token-conversion) introduces a 7% delegator-pool bucket also denominated in TOKEN, not USDC. The conversion path runs through **the same Balancer V3 80/20 pool** this ADR seeds for buyback execution — a second consumer of the same pool with the same MEV-defense requirements as `BuybackBurner`.
...
| Per-epoch liquidity cap (combined buyback + delegator) | N/A (execution disabled) | **Required** — `epochLiquidityCapFraction` of in-pool USDC depth at epoch start (default 10%, bounded `[1%, 30%]` per [ADR 026 §11](026-gauge-boost-tokenomics.md#11-governable-parameters-with-safety-bounds)) | Yes (the fraction; presence of the cap is structural) |
...
- TWAP + `minTokenOut` guards provide deterministic on-chain MEV protection. Combined with the [ADR 026](026-gauge-boost-tokenomics.md#adr-026-gauge-boost-tokenomics) hardening — mandatory Flashbots-style private-RPC routing and a per-epoch liquidity cap shared across the buyback and delegator-pool swap paths — programmatic-buyback front-running (the canonical MEV pattern flagged in the market-dynamics analysis) is closed off both by transaction-graph invisibility (private RPC) and execution shape (TWAP, slippage guard, hard cap on per-epoch pool pressure). CoW Swap batch-auction routing is an independent conditional add-on if CoW solvers route through the deployed pool (verified per the activation criteria), not a load-bearing assumption.
...
- [ADR 026 — Gauge-Boost Tokenomics](026-gauge-boost-tokenomics.md#adr-026-gauge-boost-tokenomics) — Source of the router-driven inflow, the 5%-of-routed-USDC buyback rate, the parallel delegator-pool swap path, and the hardened MEV-mitigation requirements applied to this ADR
