...
1. How is the 10% genesis `Liquidity (DEX + CEX)` allocation ([ADR 026 § Supply and distribution](026-tokenomics.md#1-supply-and-distribution)) actually deployed into the pool?
...
- **Source:** The `Genesis liquidity (POL)` allocation (10% of supply = 100M TOKEN of the 1B supply per [ADR 026 § Supply and distribution](026-tokenomics.md#1-supply-and-distribution)) funds the TOKEN side. The USDC side is drawn from the treasury.
...
| Per-epoch liquidity cap (combined buyback + delegator) | N/A (execution disabled) | **Required** — `epochLiquidityCapFraction` of in-pool USDC depth at epoch start (default 10%, bounded `[1%, 30%]` per [ADR 026 § Governable parameters with safety bounds](026-tokenomics.md#11-governable-parameters-with-safety-bounds)) | Yes (the fraction; presence of the cap is structural) |
...
- TWAP + `minTokenOut` guards provide deterministic on-chain MEV protection. Combined with the [ADR 026](026-tokenomics.md#adr-026-tokenomics) hardening — mandatory Flashbots-style private-RPC routing and a per-epoch liquidity cap shared across the buyback and delegator-pool swap paths — programmatic-buyback front-running (the canonical MEV pattern flagged in the market-dynamics analysis) is closed off both by transaction-graph invisibility (private RPC) and execution shape (TWAP, slippage guard, hard cap on per-epoch pool pressure). CoW Swap batch-auction routing is an independent conditional add-on if CoW solvers route through the deployed pool (verified per the activation criteria), not a load-bearing assumption.
...
- [ADR 026 — Gauge-Boost Tokenomics](026-tokenomics.md#adr-026-tokenomics) — Source of the router-driven inflow, the 5%-of-routed-USDC buyback rate, the parallel delegator-pool swap path, and the hardened MEV-mitigation requirements applied to this ADR
