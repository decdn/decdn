...
> POL holds a 10pp / 100M TOKEN allocation. The combined POL+MM Liquidity-Provision category is 15% (top of the 5–15% DeFi norm), with MM at 5pp. That 10pp allocation is *seeded from the DAO treasury, incrementally, at governance's discretion* — not at genesis, and self-funded from protocol revenue rather than team or investor capital. Mainnet launches dormant — no pool, `FeeRouter` split `[9000, 0, 1000]`. Governance may create the first 50/50 Uniswap V3 pool whenever the treasury holds enough USDC — for example ~$100K, which pairs ~10M TOKEN (a tenth of the POL envelope) at the $0.01 anchor — and may activate the buyback separately, flipping the split to 60/30/10 once the pool and the activation criteria are ready. The remaining ~90M TOKEN stays a Timelock reserve that governance deploys to deepen the pool as treasury revenue accrues. Per-epoch buyback liquidity-cap headroom scales with pool depth, and each buyback swap adds USDC to the pool, so seed depth is a floor that glides up — see [§ Buyback per-epoch-cap headroom](#buyback-per-epoch-cap-headroom) and [Activation Criterion 7](#activation-criteria-production).
...
- **Source:** The 10pp Protocol-Owned Liquidity allocation (100M TOKEN per [ADR 026 § Allocation](026-tokenomics.md#allocation)) funds the TOKEN side — a supply allocation, already protocol-owned. The USDC side is funded from the DAO treasury's own revenue — the 10% `FeeRouter` share ([ADR 026 § FeeRouter split](026-tokenomics.md#feerouter-split)). POL is therefore self-funded from protocol revenue and protocol supply; no team or investor capital seeds it, and the position stays DAO-custodied. The DAO may also raise USDC by selling TOKEN at arm's length before it seeds the pool, but the pool is created and held by the DAO either way. Governance creates the first pool when it chooses — mainnet launches dormant with no pool (§ Deploy-time genesis activation) — pairing, for example, ~10M TOKEN (1pp, a tenth of the POL envelope) against ~$100K USDC at the $0.01 anchor, and holds the remaining ~90M TOKEN as a Timelock POL reserve. The reserve deploys to POL depth only, so the 10pp keeps its meaning as protocol-owned liquidity.
- **First-pool seed (governance-timed, not at genesis):** an example ~$100K USDC / ~10M TOKEN. Pool creation and buyback activation are separate governance actions — the pool may exist before the buyback bucket is switched on. Mainnet seeds no pool at genesis; the launch is dormant until governance chooses to create the pool and, separately, activate the buyback (§ Deploy-time genesis activation, § Activation Criteria). `maxBuybackAmount` is sized so a single buyback swap causes less than `slippageBps` price impact against the pool's depth; governance raises it as the pool deepens.
- **Incremental deployment.** After activation, governance deploys the POL reserve over time, not at once. Each top-up adds reserve TOKEN and treasury USDC to the position through a standard Uniswap `NonfungiblePositionManager.increaseLiquidity` call — PoC: the admin key; production: the Timelock under a governance proposal. Adding liquidity is price-neutral, so a top-up deepens the pool without moving spot; it needs no swap and no bespoke contract. Between top-ups the buyback deepens the USDC side on its own (§ Buyback per-epoch-cap headroom), so both sides grow as network revenue and treasury USDC accrue.
...
- **Top-up:** depositing additional USDC or TOKEN to deepen the position — the mechanism the POL reserve deploys through (§ Protocol-Owned Liquidity mechanics § Incremental deployment). Permissionless (anyone may add liquidity to the public pool); the protocol executes it from the reserve via governance proposal.
...
| Initial pool seed size | Governance-timed, not at genesis; example ~$100K USDC / ~10M TOKEN at the $0.01 anchor. `maxBuybackAmount` sized so a single swap stays under `slippageBps` against the pool's depth. The ~90M TOKEN reserve deploys incrementally (§ Protocol-Owned Liquidity mechanics) |
...
**The glidepath.** Each buyback swap sends USDC into the pool and takes TOKEN out to burn, so the pool's USDC depth **grows** with every execution. The seed depth is therefore a floor, not a fixed ceiling: it glides up as buybacks run. Because the contract re-snapshots epoch-start depth at the first swap of each epoch, the per-epoch cap tracks *current* depth automatically — the admitted notional rises epoch over epoch as the pool deepens. The first-pool epoch-start depth is the governance-chosen seed — for example ~$100K USDC / ~10M TOKEN. Governance top-ups from the POL reserve are the second glidepath source: they deepen both sides directly, so the admitted per-epoch notional rises from buyback USDC inflow and from reserve deployment together.
