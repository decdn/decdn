...
> **v3 alignment (2026-04 update).** [ADR 026](026-tokenomics-v3.md) supersedes [ADR 004](004-tokenomics.md) and changes how this ADR's `BuybackBurner` is fed (router-driven, not manual) and adds a parallel delegator-pool USDC→TOKEN swap path through the same Balancer V3 80/20 pool. **The pool design itself is unchanged** — Balancer V3, 80/20 TOKEN/USDC, 1% swap fee, MEV protection via TWAP + `minTokenOut`, POL custody by the Timelock, and the activation criteria are all carried over verbatim. The genesis liquidity allocation is **100M TOKEN at genesis (10% of the 1B v3 supply)**; the supply-fraction (10%) is unchanged from ADR 004, only the absolute TOKEN count is restated against the v3 supply. What changed is the inflow plumbing into `BuybackBurner` (now `FeeRouter`-fed at 5% of routed USDC, not manual treasury transfers), the resulting per-revenue inflow rate (~8× higher; see the [Buyback inflow](#buyback-inflow-source-and-rate-router-driven-per-adr-026) section), the addition of a delegator-pool swap path on the same pool, and the hardening of MEV mitigations from "primary defense + optional add-on" to "**Flashbots-style private-RPC routing required, per-epoch liquidity caps required**" for both swap paths.
...
| MEV protection for buybacks | TWAP + `minTokenOut` + **mandatory Flashbots-style private-RPC routing** + per-epoch liquidity cap (per [ADR 026](026-tokenomics-v3.md) hardening); CoW Swap batch-auction routing as an independent conditional add-on, pending verification that CoW solvers route through Balancer V3 weighted pools (see Consequences) | Requires custom TWAP + private mempool (Flashbots Protect) |
...
- **Source:** The genesis `Genesis liquidity (POL)` allocation (10% of supply = 100M TOKEN of the 1B v3 supply per [ADR 026 §1](026-tokenomics-v3.md#1-supply-and-distribution); historically the `Liquidity (DEX + CEX)` allocation in [ADR 004 Token Distribution](004-tokenomics.md#token-distribution-production)) funds the TOKEN side. The USDC side is drawn from the treasury. The supply-fraction (10%) is unchanged across the v2→v3 transition; the absolute count (100M TOKEN of 1B) is the one stated against the canonical v3 supply.
...
Under [ADR 026](026-tokenomics-v3.md), `BuybackBurner` no longer relies on manual treasury transfers to accumulate USDC. The new `FeeRouter` contract receives the full operator USDC balance from `PaymentChannel.settleChannel` at every settlement and atomically forwards **5% of routed USDC directly to `BuybackBurner` in the same transaction**, alongside the other v3 fee buckets. The full v3 router split lives in [ADR 026 §2](026-tokenomics-v3.md#2-feerouter-split-40407553) (six buckets summing to 100%); this ADR cross-references rather than duplicating the table to avoid drift.
...
- **Inflow rate.** The effective USDC-flow per unit of network revenue is **~8× higher** than under ADR 004. ADR 004 routed ~20% of a 3% protocol fee to `BuybackBurner` — an effective `0.20 × 0.03 = 0.6%` of gross revenue. ADR 026 routes 5% of gross routed USDC (the full operator balance, post-settlement) to `BuybackBurner` — `0.05 / 0.006 ≈ 8.33×`. Production sizing of `maxBuybackAmount`, the keeper schedule, and the slippage-budget assumptions in the [Activation Criteria](#activation-criteria-production) below should be tuned against this higher inflow rate, not the ADR 004 rate.
...
3. The acquired TOKEN is held in the per-epoch delegator bucket until claimed pro-rata by ve-balance (mechanic per [ADR 026 §6](026-tokenomics-v3.md#6-delegator-pool--usdc--token-conversion); not this ADR's concern).

**Implementation choice — deferred.** Two viable shapes; this ADR documents both and defers selection to the implementing ADR / contract-interactions update. Trade-offs:

| Option | Shape | Advantage | Disadvantage |
| --- | --- | --- | --- |
| **A. `BuybackBurner` multi-output mode** | Extend the existing `BuybackBurner` with an additional swap entry point `executeDelegatorBuy(amount, minTokenOut)` that performs the same Router call as `executeBuyback` but transfers the resulting TOKEN to a configurable `delegatorBucket` address (the `FeeRouter`'s per-epoch sink) instead of burning. | One contract, one approval, one keeper, one set of MEV-defense parameters to tune. Reuses the V3 Router/Vault approval, the `setPool` configuration, and the TWAP machinery verbatim. Halves audit surface vs option B. | The `BuybackBurner` is no longer a "burner" by name — its responsibility broadens to "Balancer V3 swap helper for both buyback and delegator-pool flows." Conflates two policy domains (deflation and yield distribution) in one contract; care is needed to keep the per-epoch liquidity cap shared across both call paths. |
| **B. Parallel `DelegatorBuyer` contract** | A second contract with the same Balancer V3 integration boilerplate as `BuybackBurner` but with TOKEN delivery to the `delegatorBucket` instead of burn. | Single-responsibility contracts; clearer mental model and audit boundaries. Each contract's TWAP / liquidity-cap parameters can be tuned independently if buyback and delegator flows diverge in volume profile. | Duplicates Router/Vault approval logic, `setPool` plumbing, keeper integration, and TWAP code. The shared per-epoch liquidity cap must be enforced via a third contract or a shared storage slot, adding a coordination point. |

Both options preserve the MEV-defense invariants of this ADR. Option A is the lower-friction default unless contract-architecture review surfaces a specific reason to split. Final selection should be tracked in [ADR 016](016-contract-interactions.md) when the v3 contract surface is ratified.

**Liquidity-cap interaction.** The per-epoch liquidity cap defined above is a **single pool-wide budget** shared across both the buyback and delegator paths. Under sustained heavy network revenue, the two flows together MUST NOT exceed the cap; the implementing contract(s) MUST track combined epoch-to-date notional. Sizing rationale: Balancer V3's curve is smoother than V3 concentrated bands, but two independent keepers each calling near-`maxBuybackAmount` repeatedly within an epoch can still chew through pool depth.
...
7. **Required: per-epoch liquidity cap configured and shared.** The combined-path per-epoch liquidity cap MUST be set on-chain to a value calibrated against epoch-start in-pool USDC depth, and MUST be enforced across both the buyback and delegator-pool swap entry points (whether implemented as `BuybackBurner` multi-output mode or as a parallel `DelegatorBuyer` per [Delegator-pool swap path](#delegator-pool-swap-path-parallel-usdctoken-conversion-on-the-same-pool)).
...
- TWAP + `minTokenOut` guards provide deterministic on-chain MEV protection. Combined with the [ADR 026](026-tokenomics-v3.md) hardening — mandatory Flashbots-style private-RPC routing and a per-epoch liquidity cap shared across the buyback and delegator-pool swap paths — programmatic-buyback front-running (the canonical MEV pattern flagged in the v3 market-dynamics analysis) is closed off both by transaction-graph invisibility (private RPC) and by execution shape (TWAP, slippage guard, hard cap on per-epoch pool pressure). CoW Swap batch-auction routing is available as an independent conditional add-on if CoW solvers route through the deployed pool (verified per the activation criteria), but it is not a load-bearing assumption.
...
- **Mandatory private-RPC dependency.** [ADR 026](026-tokenomics-v3.md) makes Flashbots-style private-RPC routing a hard requirement (was: optional add-on). This introduces a third-party operational dependency on the chosen private-bundle provider for the production L2 — provider downtime or de-listing of the protocol's bundles is a new failure mode. Mitigated by selecting a provider with a strong uptime track record on the production L2 and by keeping the keeper code provider-agnostic (so a swap of providers is configuration, not a redeployment). The cost of this dependency is judged proportionate to the MEV exposure the v3 inflow rate (~8× higher) creates.
- **Two consumers on one pool.** The delegator-pool swap path (per [ADR 026 §6](026-tokenomics-v3.md#6-delegator-pool--usdc--token-conversion)) routes a parallel TWAP USDC→TOKEN flow through the same Balancer V3 pool as buybacks. The combined-path per-epoch liquidity cap defends pool depth, but the contract surface, keeper coordination, and audit scope grow. Whether implemented as `BuybackBurner` multi-output mode or as a parallel `DelegatorBuyer` (see [Delegator-pool swap path](#delegator-pool-swap-path-parallel-usdctoken-conversion-on-the-same-pool)), the combined-path notional tracking is a non-trivial new invariant.

## References

- [ADR 003 — Payment Channels (IBuybackBurner interface)](003-payments.md#buybackburner)
- [ADR 004 — Dual-Currency Token Model](004-tokenomics.md) (Superseded by ADR 026 for tokenomics; the ADR 004 references in this ADR remain valid for historical genesis-allocation framing)
- [ADR 009 — Governance Model](009-governance.md)
- [ADR 016 — Smart Contract Interaction Model](016-contract-interactions.md)
- [ADR 026 — Tokenomics v3 (Gauge-Boost Design)](026-tokenomics-v3.md) — Source of the router-driven inflow, the 5%-of-routed-USDC buyback rate, the parallel delegator-pool swap path, and the hardened MEV-mitigation requirements applied to this ADR
