...
1. How is the 10% genesis `Liquidity (DEX + CEX)` allocation ([ADR 026 § Supply and distribution](026-tokenomics.md#supply-and-distribution)) actually deployed into the pool?
...
- **Source:** The `Genesis liquidity (POL)` allocation (10% of supply = 100M TOKEN of the 1B supply per [ADR 026 § Supply and distribution](026-tokenomics.md#supply-and-distribution)) funds the TOKEN side. The USDC side is drawn from the treasury.
...
Under [ADR 026](026-tokenomics.md#adr-026-tokenomics), `BuybackBurner` no longer relies on manual treasury transfers. The `FeeRouter` contract receives the full operator USDC balance from `PaymentChannel.settleChannel` at every settlement and atomically forwards **5% of routed USDC directly to `BuybackBurner` in the same transaction**, alongside the other fee buckets. The full router split lives in [ADR 026 § FeeRouter split (40/40/7/5/5/3)](026-tokenomics.md#feerouter-split-40407553) (six buckets summing to 100%); this ADR cross-references rather than duplicating it.
