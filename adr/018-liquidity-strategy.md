...
>
> - **Router storage.** The base `BuybackBurner` stores only the Vault and pool; the subclass adds the Router (`swapRouter`), **constructor-set** and rotatable by governance via `setSwapRouter`. The Router is the swap call target; approvals still go to the **Vault** (§ Buyback execution).
...
| `maxBuybackAmount` | Set so a single swap causes < `slippageBps` impact; **required non-zero** — a zero ceiling reverts every buyback forever, wedging the burner with an unspendable balance |
| `slippageBps` | 200 bps (2%), bounded `[0, 1000 bps]` — the TWAP floor scales by `(10000 - slippageBps)`, so a tolerance near 100% collapses it to nothing and disables the MEV stack silently |
