...
Under [ADR 026](026-gauge-boost-tokenomics.md), `BuybackBurner` no longer relies on manual treasury transfers to accumulate USDC. The new `FeeRouter` contract receives the full operator USDC balance from `PaymentChannel.settleChannel` at every settlement and atomically forwards **5% of routed USDC directly to `BuybackBurner` in the same transaction**, alongside the other v3 fee buckets. The full v3 router split lives in [ADR 026 §2](026-gauge-boost-tokenomics.md#2-feerouter-split-40407553) (six buckets summing to 100%); this ADR cross-references rather than duplicating the table to avoid drift.
...
- TWAP + `minTokenOut` guards provide deterministic on-chain MEV protection. Combined with the [ADR 026](026-gauge-boost-tokenomics.md) hardening — mandatory Flashbots-style private-RPC routing and a per-epoch liquidity cap shared across the buyback and delegator-pool swap paths — programmatic-buyback front-running (the canonical MEV pattern flagged in the v3 market-dynamics analysis) is closed off both by transaction-graph invisibility (private RPC) and by execution shape (TWAP, slippage guard, hard cap on per-epoch pool pressure). CoW Swap batch-auction routing is available as an independent conditional add-on if CoW solvers route through the deployed pool (verified per the activation criteria), but it is not a load-bearing assumption.
...
- **Two consumers on one pool.** The delegator-pool swap path (per [ADR 026 §6](026-gauge-boost-tokenomics.md#6-delegator-pool--usdc--token-conversion)) routes a parallel TWAP USDC→TOKEN flow through the same Balancer V3 pool as buybacks. The combined-path per-epoch liquidity cap defends pool depth, but the contract surface, keeper coordination, and audit scope grow. Whether implemented as `BuybackBurner` multi-output mode or as a parallel `DelegatorBuyer` (see [Delegator-pool swap path](#delegator-pool-swap-path-parallel-usdctoken-conversion-on-the-same-pool)), the combined-path notional tracking is a non-trivial new invariant.
...
- [ADR 026 — Tokenomics v3 (Gauge-Boost Design)](026-gauge-boost-tokenomics.md) — Source of the router-driven inflow, the 5%-of-routed-USDC buyback rate, the parallel delegator-pool swap path, and the hardened MEV-mitigation requirements applied to this ADR
