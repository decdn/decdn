...
> **ADR 026 alignment.** [ADR 026](026-gauge-boost-tokenomics.md) supersedes [ADR 004](004-tokenomics.md) and changes how this ADR's `BuybackBurner` is fed (router-driven, not manual) and adds a parallel delegator-pool USDC→TOKEN swap path through the same Balancer V3 80/20 pool. **The pool design itself is unchanged** — Balancer V3, 80/20 TOKEN/USDC, 1% swap fee, MEV protection via TWAP + `minTokenOut`, POL custody by the Timelock, and the activation criteria carry over. The genesis liquidity allocation is **100M TOKEN at genesis (10% of the 1B supply)**. The inflow plumbing into `BuybackBurner` is `FeeRouter`-fed at 5% of routed USDC (not manual treasury transfers); the resulting per-revenue inflow rate is ~8× higher (see the [Buyback inflow](#buyback-inflow-source-and-rate-router-driven-per-adr-026) section), the delegator-pool swap path adds a second consumer of the same pool, and MEV mitigations are hardened to "**Flashbots-style private-RPC routing required, per-epoch liquidity caps required**" for both swap paths.
