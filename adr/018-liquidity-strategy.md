...
>
> - **Router storage.** The base `BuybackBurner` is venue-neutral and stores only USDC and TOKEN; the subclass owns the whole venue triple — the Vault, the pool, and the Router (`swapRouter`) — with the Router and Permit2 **constructor-set**, the Router rotatable by governance via `setSwapRouter`, and the Vault and pool rotatable via `setVault` / `setPool`. The Router is the swap call target; approvals go to **Permit2**, not the Vault (§ Buyback execution).
...
| Token approvals spender | Permit2 (canonical `0x000000000022D473030F116dDEE9F6B43aC78BA3`), which in turn is granted a scoped per-swap allowance to the Router. The Vault is never an approval target |
