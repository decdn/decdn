...
- **Trait seams at crate boundaries.** Each mode-varying capability — key storage, reputation scoring, the payment-channel client, the governance/parameter reader, and the on-chain contract clients (`FeeRouter`, `CapacityBond`, `OperatorEmissions`, `BuybackBurner`, `SafetyReserve`) — is a trait owned by its crate. PoC and production concrete implementations sit behind that trait; the wiring layer constructs the right one.
