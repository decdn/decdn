...
Crucially, none of these is a *contract-surface* difference. Per [ADR 016 § Contract Inventory](016-contract-interactions.md#1-contract-inventory), the full on-chain surface ships in a single audit pass with governance-tunable economics from day one; "PoC" is a network-scale milestone (tens of nodes on a testnet), not a reduced contract surface. Deliberate, separately-documented scope reductions are operational or governance-process choices (e.g. admin-key vs ve-Governor, simplified peer bootstrap, USDC-only at launch with governance allowlisting further stablecoins per [ADR 003](003-payments.md)) — explicit in the canonical inventory, never implicit contract-surface mode-branching.
