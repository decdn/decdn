...
This ADR does **not** re-litigate ADR 028 semantic decisions (bond size, filing windows, evidence rules, frequency caps, the 48-hour SafetyReserve counter-bundle window, restitution caps, reputation-preservation rule); restatements here are for self-containedness, with canonical authority remaining ADR 028 and ADR 026 §5. Parameter values (`APPEAL_BOND`, `MULTISIG_REVIEW_WINDOW`, `RATIFICATION_WINDOW`, `MAX_APPEAL_RESTITUTION`, `OPERATOR_APPEAL_FREQUENCY`) are pinned in [#451](https://github.com/decdn/decdn/issues/451) and remain ADR 028 §5's responsibility. Reserve sizing against the 3.3× depletion ratio is tracked at [decdn/finance#3](https://github.com/decdn/finance/issues/3). Audit-pinned constants and gas micro-tuning are out of scope.
...
- Pins the storage layout and event schema so [#452](https://github.com/decdn/decdn/issues/452) has a single source of truth, removing cross-derivation between ADR 028's narrative and the eventual Solidity.
