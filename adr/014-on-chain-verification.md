...
- Off-chain verifiers (clients, requesting nodes, third-party fraud detectors) must `ecrecover` and look up `StakingRegistry.nodeIdOf(recovered)` to attribute a message to a NodeId, rather than verifying directly against the iroh key. The binding cache is already maintained by these parties for `clientStakeOf` and voucher attribution, so the marginal cost is one extra map lookup per verification.
