...
The wash-trading defense under v2.1 is structural: per-byte settlement (60% operator base) is paid by real clients, so wash trades do not raise revenue. Capacity-shortfall slashing per [ADR 026 § Capacity-shortfall slashing](026-tokenomics.md#capacity-shortfall-slashing) is the on-chain mechanism — operators delivering less than `min_delivery_ratio × declared_capacity` are auto-downgraded with the bond delta forfeit to `SafetyReserve`. There is no operator-asserted summary, no fraud-challenge mechanism, no monitoring role for capacity-share; the deterministic threshold binds directly on probe data. Off-chain reputation observation ([ADR 008 § Capacity-Shortfall and Wash-Trading: Reputation as Off-Chain Signal](008-reputation.md#capacity-shortfall-and-wash-trading-reputation-as-off-chain-signal)) and operator-cluster detection are the soft layer informing governance threshold-tuning if persistent patterns surface.
...
- [ADR 026 § Capacity-shortfall slashing](026-tokenomics.md#capacity-shortfall-slashing) — wash-trading defense under v2.1; deterministic on probe data, no external attestation surface
