...
The wash-trading defense is structural: per-byte settlement (60% operator base) is paid by real clients, so wash trades do not raise revenue, and governance vote weight is sourced from `FeeRouter.bytesInWindow` per [ADR 036](036-served-bytes-voting-weight.md#adr-036-served-bytes-voting-weight) (proven delivered bytes), so over-declared capacity does not translate into governance influence. The super-linear capacity-bond curve makes over-declared capacity dead capital with no governance or revenue upside. There is no operator-asserted summary, no fraud-challenge mechanism, no monitoring role for capacity-share. Off-chain reputation observation ([ADR 008 § Wash-Trading: Reputation as Off-Chain Signal](008-reputation.md#wash-trading-reputation-as-off-chain-signal)) and operator-cluster detection are the soft layer informing governance threshold-tuning if persistent patterns surface.
