...
**Clock synchronization:** The ±60-second freshness check in gossip validation (3) means that nodes with a wall-clock offset exceeding 60 seconds will have their `NodeAnnounce` messages silently rejected by all peers — the node becomes invisible to the mesh with no error feedback. All node hosts MUST run NTP (or an equivalent time-synchronization service) to maintain wall-clock accuracy well within this 60-second window. At startup, the node SHOULD compare its local clock against a reference source (e.g., an NTP query or the latest on-chain `block.timestamp`) and log a warning if the offset exceeds 10 seconds, giving operators an early signal before silent gossip rejection occurs.
