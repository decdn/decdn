...
All staked nodes form a flat peer mesh with no fixed routing hierarchy. Node discovery is gossip-based; content discovery uses `cdn/dht/v1` (a lightweight Kademlia subset — see [ADR 022](022-content-discovery.md)), with broadcast probe fan-out as a bootstrap fallback:
...
Content discovery uses `cdn/dht/v1` as the primary mechanism (see [ADR 022](022-content-discovery.md)). `cdn/probe/v1` is used **after** the DHT lookup to confirm live availability and measure latency, and as a **bootstrap fallback** before the routing table is populated. When a node or client needs blob H:
...
2. **DHT FIND_VALUE.** Issue an iterative O(log N) `FindValueRequest` for H via `cdn/dht/v1` (see [ADR 022 §1.6](022-content-discovery.md)). Returns a `Vec<NodeId>` of known holders. If the routing table has fewer than k entries (bootstrap window), skip to step 2a.
   - **2a. Bootstrap fallback.** Send `ProbeRequest {hash, timestamp_us}` in parallel to all known peers (broadcast fan-out). Use these responses as the candidate set and skip step 3.
3. **Probe the DHT candidate set.** Send `ProbeRequest {hash, timestamp_us}` in parallel to the NodeIds returned by the DHT. ALPN: `cdn/probe/v1` — `ProbeResponse {has_blob, rate_per_mb, timestamp_us, signature, total_bytes?, slash_sig}`. If DHT returned no providers, fall back to broadcast fan-out (step 2a).
...
- **DHT-primary discovery (all scales).** `cdn/dht/v1` is the primary content discovery mechanism from PoC onward. At PoC scale (30 nodes), FIND_VALUE resolves in 1–2 hops. A DHT miss followed by broadcast fan-out fallback definitively means no node holds the blob. See [ADR 022](022-content-discovery.md) for the full discovery flow.
...
- All nodes participate in the same discovery and transport protocols. The cache-only role remains permissionless — any staked operator may pull cached blobs from authorized origins and re-serve them. The origin role is permissionless for default-open content (any staked operator may act as origin) and DAO-governed for content claimed in a registered namespace (only operators in the namespace's `OriginAssignment` set may respond `is_origin: true`); see [ADR 011 § Origin Assignment Authority](011-content-takedown.md#origin-assignment-authority) and [ADR 002 § Publisher Identity and Namespaces](002-content-addressing.md#publisher-identity-and-namespaces)
...
- Broadcast probe fan-out (bootstrap fallback) generates O(N) probe messages. Rate limits prevent bursty cache misses from becoming a self-DoS. DHT is the primary path; fan-out is only used during the bootstrap window or as an emergency fallback
...
- Origin-backed nodes become the last line of defense for content availability — if all authorized origins for a given blob go offline or are deregistered, the content becomes permanently unavailable (unless cached elsewhere). For content in registered namespaces, the `OriginAssignment` minimum-redundancy invariant ([ADR 009](009-governance.md), [ADR 011 § Origin Assignment Authority](011-content-takedown.md#origin-assignment-authority)) ensures activated assignments always include at least the configured floor (default 3) of authorized origins, raising the bar for total loss. For default-open content (`namespaceId == 0`) any staked operator may serve as origin, so the only failure mode is total loss of every operator that ever cached the blob — the same behaviour as before this ADR was extended.
