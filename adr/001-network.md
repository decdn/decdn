...
3. **Collect.** Wait for probe responses in two phases:
   - **Minimum wait** (`probe_min_wait`, default 50ms): Always wait at least this long to collect responses from nearby nodes, ensuring multiple candidates compete rather than always selecting the single fastest responder.
   - **Early exit** (evaluated continuously after `probe_min_wait`): Exit early when **both** conditions are met: (a) at least `min_probe_responses` (default: 3) `has_blob: true` responses have been received, and (b) the best selection score among collected responses is below `early_exit_score_threshold` (default: `1.5 × rolling_median_score`). The rolling median is computed from the node's last 100 successful pull scores (bounded circular buffer, seeded with `+Inf` so early exit is disabled until the node has enough history).
   - **Maximum wait** (`probe_max_wait`, default 500ms): If the early-exit condition is not met, wait up to 500ms as today. The 500ms ceiling accommodates inter-continental RTTs (e.g., London↔Sydney ~250-300ms) to avoid creating geographical bottlenecks where only nearby nodes are ever selected.

   Store all `has_blob: true` responses received (whether collection ended early or at 500ms) in the probe cache.
   ...
   **PoC defaults:** `probe_min_wait = 50ms`, `min_probe_responses = 3`, `probe_max_wait = 500ms`, score threshold multiplier = `1.5`. All four parameters are operator-configurable. Setting `probe_min_wait = probe_max_wait` disables early exit entirely (equivalent to the previous fixed-wait behavior).
