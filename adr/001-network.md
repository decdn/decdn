...
2. **Fan-out.** Send `ProbeRequest {hash, timestamp_us}` in parallel to all known nodes (regional + global). The ALPN remains `cdn/probe/v1` — `ProbeResponse {has_blob, rate_per_mb, timestamp_us, signature, slash_sig, total_bytes?}`.
