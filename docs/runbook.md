...
1. Raise `cache.cache_size_mb` (restart required), **or** move
   `cache.cache_dir` to a larger volume, **or** provision more disk. The
   eviction driver enforces the budget, so raising it is a real lever.
   Note the budget is also the admission ceiling on a single blob (#1678):
   a blob larger than `cache_size_mb` is refused with `BlobTooLarge`, so a
