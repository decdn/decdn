...
- Creates a poor UX: listeners see different prices every time they open the app
- Providers competing on rate become a proxy for providers competing on price predictions

The inverse problem affects listeners: a 10x token pump means their deposited channel funds become worth far more than intended, creating opportunity cost.
...
**Principle:** Use each currency for what it does best. Stablecoins for predictable value transfer. The native token (AUDIO) for network-specific economic functions that require aligned incentives.

| Function | Currency | Rationale |
|----------|----------|-----------|
| Provider staking | AUDIO | Stake must align provider incentives with network health. If the network thrives, stake appreciates. |
| Governance voting | AUDIO | Governance power should reflect network commitment, not purchasing power. |
| Challenge bonds | AUDIO | Security mechanism in the staking domain. |
| Slashing penalties | AUDIO | Already denominated in stake. No change. |
...
| Delivery payments | Stablecoin (USDC) | Per-MB streaming payments. Providers need predictable unit economics. |
| Protocol fees | Stablecoin (collected), partially converted to AUDIO (buyback) | Fees match payment currency to avoid forced conversion at collection time. |
...
The `incentive` crate must handle both 6-decimal (USDC/USDT) and 18-decimal (AUDIO, DAI) tokens:

```rust
enum Currency {
    Native { decimals: u8 },      // AUDIO, 18 decimals
...
**Why these numbers:** They roughly match current cloud storage/CDN economics with a markup for decentralization overhead. A provider storing 500GB and serving 2TB/month earns $50 + $20 = $70/month in stablecoin — enough to cover a budget VPS ($35-100) with a margin, regardless of AUDIO token price.
...
Client deposits 100 USDC into channel
  → Streams audio, signs vouchers totaling 80 USDC
...
| Token buyback & burn | 20% | USDC → AUDIO → burn | Via `BuybackBurner` contract |
...
2. `BuybackBurner.executeBuyback()` swaps USDC for AUDIO via Uniswap V3 on Arbitrum
3. Purchased AUDIO is sent to a burn address (`0x000...dead`)
...
| DEX | Uniswap V3 AUDIO/USDC pool | Yes (pool address) |
...
**Why buyback is better than direct burn:** The tokenomics spec burned 20% of fees collected in native token — those tokens were already held. The buyback mechanism creates *new buy pressure* from the open market, directly linking network usage to token demand. More streaming → more USDC fees → more AUDIO purchased and burned.

---

## 6. Token Utility Preservation

If payments move to stablecoin, why hold AUDIO? The token must have clear, non-substitutable utility.

### Six Demand Drivers

| Utility | Mechanism | Why USDC Can't Substitute |
|---------|-----------|--------------------------|
| **1. Provider staking** | Min 1,000 AUDIO to operate as provider. No AUDIO = no network access. | Stake must correlate with network health. USDC-staked providers have no skin in the game. |
...
| **3. Fee discount** | Providers staking ≥10x minimum (10,000 AUDIO) get 50% protocol fee reduction: 1.5% instead of 3%. | Rewards long-term network commitment. Direct financial incentive to hold more AUDIO. |
| **4. Client priority staking** | Listeners optionally stake AUDIO (no minimum, no slashing). Providers prioritize high-stake clients during congestion. | Creates demand-side utility. Listeners who hold AUDIO get better service. |
| **5. Buyback pressure** | 20% of all stablecoin fees used to buy and burn AUDIO on open market. | Direct link: more streaming = more USDC fees = more AUDIO bought and burned = supply reduction. |
| **6. Bootstrap rewards** | Provider bootstrap fund (200M AUDIO from distribution) paid as bonuses on top of USDC payments. | Early providers earn USDC for costs + AUDIO for upside. |
...
**Economic impact:** At 3% fee, a provider earning $70/month pays $2.10 in fees. At 1.5%, they pay $1.05 — saving $1.05/month. To qualify, they must stake 10,000 AUDIO. If AUDIO is $0.01, that's $100 locked for a $12.60/year savings — a 12.6% yield on staked capital just from fee reduction. This creates real demand for AUDIO.

### Client Priority Staking

A lightweight, optional mechanism:

- Clients call `StakingRegistry.clientStake(amount)` to deposit AUDIO
...
| Role | Why they hold AUDIO |
|------|-------------------|
| Provider | Must stake to operate. More stake = fee discount. |
| Listener | Optional stake for priority access during congestion. |
...
|----------|--------|---------|
| `Token` (ERC-20) | **No change** | AUDIO token unchanged |
...
| `BuybackBurner` | **New contract** | Fee accumulation, Uniswap swap, AUDIO burn |
...
3. Provider bootstrap fund begins distributing AUDIO bonuses on top of USDC payments
...
- **Provider revenue stability.** $70/month in USDC covers costs regardless of AUDIO price.
- **Listener cost predictability.** ~$0.0000375 per track (3.75MB * $0.00001/MB), period.
...
- **Cleaner token model.** AUDIO becomes a capital asset (staking, governance) not a payment token.
...
| **Decimal mismatch bugs** | Low-Medium | USDC (6 decimals) vs AUDIO (18 decimals). Off-by-12-orders-of-magnitude bugs possible. Mitigation: `Currency` enum in the `incentive` crate with explicit decimal handling. Extensive unit tests with both 6 and 18 decimal tokens. |
| **Liquidity fragmentation** | Low | Payment volume moves from AUDIO to USDC markets. The AUDIO/USDC pool may thin. Mitigation: buyback mechanism provides continuous buy-side demand. Protocol treasury can seed the AUDIO/USDC pool. |
| **User complexity** | Low | Users need USDC + AUDIO (for staking). Two tokens instead of one. Mitigation: client software can integrate a DEX swap (swap AUDIO↔USDC in one click). Listeners who only stream need only USDC — simpler than understanding a volatile token. |
...
3. **Client-side fee discounts?** Should listeners who stake AUDIO also get reduced fees? Creates demand-side utility but complicates fee calculation (who gets the discount — client or provider?). **Recommendation:** Defer to v2. Provider-only fee discounts are simpler and create sufficient token demand.
...
5. **Should the bootstrap fund pay in USDC, AUDIO, or both?** The tokenomics spec allocates 200M AUDIO for provider bootstrapping. With stablecoin payments, early providers already earn USDC. The bootstrap fund could pay AUDIO bonuses (upside exposure) or USDC (guaranteed value). **Recommendation:** AUDIO bonuses. Providers get USDC for costs, AUDIO for network-aligned upside. This also creates token distribution among active participants.
