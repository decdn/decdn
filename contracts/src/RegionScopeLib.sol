...
library RegionScopeLib {
    ...
    /// @param nowTs        The instant the ripening window is measured at. The
    ///                     read-path caller (`ContentBlacklist`) passes
    ///                     `block.timestamp` ("what should I serve now"); the
    ///                     slash-path caller (`SlashJudge`) passes the served
    ///                     `responseTs` ("was this past serve in scope") — ADR
    ///                     030 § Region-stability window item 2 (#801).
    ...
    ///      served `responseTs`, which CAN precede `effective` whenever the operator
