...
/// @title StakeMath
/// @notice Pure arithmetic for tier-based capacity-bond stake reduction,
...
///         `CapacityBond`). `CapacityBond._reduceStakeAtTier` reads the
...
library StakeMath {
    ...
    ///      tier above 100% of at-risk stake (`tierBps > BPS_DENOMINATOR`)
    ...
    ///      ordering. Single trusted caller today (`_reduceStakeAtTier`).
    /// @return slashAmount  the amount actually removed across both balances
    /// @return newActive    the operator's active stake after reduction
