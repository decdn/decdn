...
//! rate-limit a log line. The aggregate answers the triage question, and the
//! per-event detail lives in the counter or the `debug!` beside each gate.
//!
//! Each call site owns its OWN throttle. Sharing one window would let whichever
//! cause fires more often hold it open and silence the other entirely, and would
//! mix both causes into the `suppressed` count each line reports.
...
pub(crate) struct WarnThrottle {
    interval: Duration,
    /// Millisecond timestamp of the last admitted line. `0` means no line was
    /// ever admitted.
...
impl WarnThrottle {
    ...
    /// Force the window open so the next [`Self::admit`] warns. `1`, not `0`,
    /// because `0` is the never-warned sentinel.
    #[cfg(test)]
    pub(crate) fn force_open(&self) {
        self.last_warn_ms.store(1, Ordering::Relaxed);
    }
}
...
/// subtraction), suppressing rather than spamming. A clock that jumps FORWARD
/// parks `last_warn_ms` in the future, so the gate stays shut for the size of the
/// jump rather than for `interval`. That is tolerated: a log gate that errs
/// toward silence costs one late line, and the counter beside every gate still
/// records each event.
...
mod tests {
    ...
    #[test]
    fn warn_suppresses_rather_than_spams_on_a_backwards_clock() {
        ...
        // for a log gate: the alternative is every event warning until then.
        let interval = Duration::from_mins(5);
