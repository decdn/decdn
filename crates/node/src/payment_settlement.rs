...
impl<P: Provider + Clone + 'static> PaymentChannelService<P> {
    ...
    async fn close_all_unredeemed(&self) -> usize {
        ...
        for st in states {
            ...
            match self.contract.getChannel(st.channel_id).call().await {
                Ok(ch) => {
                    let unredeemed = st.last_amount.saturating_sub(ch.withdrawnAmount);
                    let still_open = matches!(ch.status, PaymentChannel::Status::Open);
                    if !still_open || unredeemed.is_zero() {
                        continue;
                    }
                    if self.submit_close(&st).await {
    ...
    /// Submit `closeChannel` for `st`'s latest voucher. Returns `true` on a
    /// landed transaction. Logs and returns `false` on any failure.
    async fn submit_close(&self, st: &ChannelState) -> bool {
        ...
        let landed = async {
            let pending = self
                ...
                .send()
                .await
                .context("closeChannel send")?;
            pending.get_receipt().await.context("closeChannel receipt")
        }
        .await;
        match landed {
            Ok(receipt) => {
            ...
            Err(err) => {
                warn!(%err, channel_id = %st.channel_id, "closeChannel failed");
                false
            }
        }
    }
