...
impl BuyerLedgers {
    ...
    pub fn get_or_seed(&self, key: LaneKey, master: B256, seed: Cumulative) -> Arc<PoolLedger> {
        ...
        Arc::clone(
            live.entry(key)
                .or_insert_with(|| Arc::new(PoolLedger::new(key, master, seed))),
        )
    }
...
mod tests {
    ...
    fn concurrent_pulls_on_one_lane_share_a_ledger() {
        ...
        let a = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(7));
        let b = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(7));
        assert!(
            Arc::ptr_eq(&a, &b),
    ...
    fn a_stale_seed_cannot_rewind_a_live_ledger() {
        let ledgers = BuyerLedgers::default();
        let live = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(7));
        let rejoined = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(7));
    ...
    fn a_rotated_pool_gets_a_fresh_ledger() {
        let ledgers = BuyerLedgers::default();
        let old = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(7));
        let new = ledgers.get_or_seed(lane(10, 1), B256::ZERO, seed_at(0));
    ...
    fn forget_drops_only_the_named_lane() {
        let ledgers = BuyerLedgers::default();
        let first = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(7));
        // A concurrent open already rotated us onto pool 10; a late retire of pool 9 must not
        // throw away the live ledger.
        let live = ledgers.get_or_seed(lane(10, 1), B256::ZERO, seed_at(0));
        ledgers.forget(lane(9, 1));
        let rejoined = ledgers.get_or_seed(lane(10, 1), B256::ZERO, seed_at(0));
        assert!(Arc::ptr_eq(&live, &rejoined), "pool 10's ledger survives");
        drop(first);

        ledgers.forget(lane(10, 1));
        let fresh = ledgers.get_or_seed(lane(10, 1), B256::ZERO, seed_at(3));
    ...
    fn a_stale_pool_does_not_evict_the_live_ledger() {
        ...
        let live = ledgers.get_or_seed(lane(10, 1), B256::ZERO, seed_at(5));
        // A late call with a STALE pool: keyed by `LaneKey`, it addresses pool 9's
        // own entry and never touches pool 10's live ledger.
        let _stale = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(0));
    ...
    fn distinct_providers_get_distinct_ledgers() {
        let ledgers = BuyerLedgers::default();
        let a = ledgers.get_or_seed(lane(9, 1), B256::ZERO, seed_at(0));
        let b = ledgers.get_or_seed(lane(9, 2), B256::ZERO, seed_at(0));
