...
impl ClientHandler {
    ...
    pub(super) async fn serve_leg(
        ...
        // successful completion, which is necessarily after the concurrent pull
        // leg filled the whole range and ran its fill-time admission read — so
        // the serve's `observe` follows the admission estimate read, preserving
        // the ordering invariant even though the two legs run concurrently. One
        // sighting per served blob, regardless of range or interval count.
