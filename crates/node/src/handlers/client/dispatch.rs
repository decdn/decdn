...
impl ClientHandler {
    ...
    pub(super) async fn serve_stream(
        ...
        if pool_status.is_none() {
            // One bump per request, HERE rather than at each gate: the skip is
            // decided once and skips four of them, so a per-gate counter would
            // multiply one cause. It makes the fail-open countable — without it a
            // stream abandoned in this window consumes floor budget that is charged
            // to no signer and appears in no series.
