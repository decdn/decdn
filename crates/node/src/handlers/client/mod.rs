...
//
// `floor` also owns the floor accumulator's types, so its counters are unreachable
// from here and can only be moved through the mutators that keep the pool total and
// the per-signer rows in step. See its module doc.
mod delivery;
mod dispatch;
mod fill;
mod floor;
...
use floor::{FloorAccumulator, FloorRefusal, FloorRefusalSite, FloorReservation};
...
mod tests {
    ...
    pub(super) async fn handler_for_tests(
        metrics: &Arc<Metrics>,
    ) -> (Arc<ClientHandler>, tempfile::TempDir) {
    ...
    pub(super) async fn handler_for_tests_with_floor(
        metrics: &Arc<Metrics>,
        pool_min_remaining_deposit: U256,
    ) -> (Arc<ClientHandler>, tempfile::TempDir) {
    ...
    pub(super) async fn handler_for_tests_with_signer_policy(
        metrics: &Arc<Metrics>,
        pool_min_remaining_deposit: U256,
        pool_floor_signer_live_windows: u64,
        pool_floor_signer_bucket_windows: u64,
    ...
    pub(super) async fn handler_for_tests_with_floor_store(
        metrics: &Arc<Metrics>,
        pool_floor_signer_bucket_windows: u64,
        pool_floor_signer_refill_secs: u64,
