...
/// also bounds the whole probe-collection phase. The budget covers connection
/// setup plus one unpaid probe request/response; warm or 0-RTT connections
/// (ADR 015) complete in a single round trip, so the 500ms ceiling accommodates
/// inter-continental RTTs (London↔Sydney ~250–300ms) while still dropping a slow
/// or unreachable candidate before it burns the caller's miss-latency budget.
