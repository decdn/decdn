...
pub struct DecdnMetrics {
    ...
    /// counts ONLY the local list — governance blacklist entries reach the serve
    /// path as cache evictions and land on
    /// `serve_stream_rejected_evicted_since_probe`. Splitting them here is safe
    /// where the wire code must not: this is the operator's own gauge of their
    /// own denylist, not something a client can probe. A rising value after a
    /// takedown is the confirmation the order is being discharged. Visible name:
