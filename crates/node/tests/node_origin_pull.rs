...
async fn prefetch_acquire_pulls_and_records_spend() -> Result<()> {
    ...
    let engine = Arc::new(decdn_node::prefetch::PrefetchEngine::new(prefetch_cfg));
...
async fn prefetch_acquired_blob_credits_served_through_serve_loop() -> Result<()> {
    ...
    let handler_b = build_handler_full_configured(
        ...
        0,
        16,
        |deps| deps.prefetch_engine = Some(Arc::clone(&engine)),
    )?;
...
async fn window_pull_through_tee_finalize_failure_serves_but_does_not_cache() -> Result<()> {
    ...
    let (handler_b, b_target, ep_b, _recorded, cache_b, b_metrics, _local_rep, _leech_gov) =
        build_node_b_with_leaves(
            a_id,
            a_addr,
            a_eth.address(),
