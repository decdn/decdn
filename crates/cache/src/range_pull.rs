...
//!
//! [`import_bao_bytes`]: iroh_blobs::api::blobs::Blobs::import_bao_bytes
...
/// `from_chunk_log(4)`). Re-exported from upstream rather than re-declared
/// locally so the outboard and interleaved encoding the cache produces always
/// match what `import_bao_bytes` frames — even across an iroh-blobs bump, which
/// ADR 038 treats as a protocol-contract change. A local copy could silently
/// diverge; this cannot.
pub use iroh_blobs::store::IROH_BLOCK_SIZE;
