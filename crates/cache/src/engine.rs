...
impl CacheEngine {
    ...
    /// origin read. The caller's live `origin_size` probe still confirms the data
    /// object, and a draw that the origin then declines fails the signed stream.
    ...
    pub async fn origin_range_wire(
        ...
        for ix in order {
            ...
            match opened {
                Ok(Some(cursor)) => {
                    return OriginRangeWire::spawn(
                        cursor,
                        aligned,
                        permit,
                        self.inner.outboards.clone(),
                    )
                    .map(Some);
                }
                Ok(None) => {}
...
mod tests {
    ...
    async fn observed_present_bytes_of_an_absent_hash_is_zero() -> anyhow::Result<()> {
        ...
        let engine = CacheEngine::open(tmp.path(), vec![], 16).await?;
        let bytes =
            observed_present_bytes(engine.inner.store.blobs(), Hash::new(b"absent")).await?;
