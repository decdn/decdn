...
pub fn write_validate_summary<W: std::io::Write>(
    ...
    writeln!(
        ...
        resolved.blockchain.pool_floor_signer_live_windows
    )?;
    writeln!(
        w,
        "  cache_dir:                {}",
...
# get_logs_max_block_span = 10000    # max blocks per chain-watcher eth_getLogs request; set to your RPC provider's range limit (the poller also shrinks the window when the provider rejects a range; set it to the span the shrink warning logs); default 10000, must be > 0
