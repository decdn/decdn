...
# origin_probe_fault_ttl_sec = 5           # TTL for a memoised fault (backend outage) live-origin probe answer; longer than the negative TTL so a failing origin is not re-HEADed on every request for its namespace, shorter than the positive TTL so a recovery is noticed soon (#1130 pt3, #1789 item 6)
