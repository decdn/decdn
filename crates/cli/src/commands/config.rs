...
# origin_assignment_address = ""     # OPTIONAL: 0x-prefixed hex; chain-backed origin directory for DHT prefetch (ADR 022) + `decdn publish assign` target. Set WITH publisher_registry_address.
# publisher_registry_address = ""    # OPTIONAL: 0x-prefixed hex; pairs with origin_assignment_address; also the `decdn publish namespace create` / `claim` target (#1029)
